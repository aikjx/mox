...
| workspace | `workspace.history` | `workspace_history.json` | 单集合 `Vec<HistoryItem>`；**原 `save_workspace_history` 未被调用（dead code），持久化此前实际未生效，归一化后存储后端已统一，建议补调用点真正落地** |
| kb_ext | `kb_ext.entity_relations` | `kb_entity_relations.json` | 单集合 `Vec<DocEntityRelation>` |
| misc | `misc.tasks` / `misc.projects` | `misc_data.json` | 双集合，旧 JSON 为 `{tasks, projects}` 对象，迁移时拆分 |
