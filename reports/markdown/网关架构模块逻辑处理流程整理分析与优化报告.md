...
| 优化 | 本轮落地 5 项零/低风险收口（§六），新增 3 个回归用例，全量测试 74 + 8 + 10 全绿 |
| 路线 | 剩余 17 项按 P0/P1/P2 排出，含风险与工作量，待确认后实施（§七） |
...
| ID | 问题 | 证据 |
|----|------|------|
| P1-01 | **9 套独立状态容器**：`GatewayState`、`AllianceGatewayState`、`MonitorState`、`WorkspaceState`、`ProjectsState`、`MiscState`、`KbExtState`、`NotificationState`、`ExpertsSharedState`，无统一状态注册中心 | 各模块文件 |
...
| P1-05 | **归一化映射表三处重复**：`alliance.rs:347-370`、`alliance_remote.rs:180-214`、内联 match（`alliance.rs:783-788, 883-890`） | 同前 |
...
| P2-04 | **`config.rs` 死配置**：`RoutingConfig` 三个字段无读取点 | `config.rs:116-133` | 待清理 |
...
1. **状态注册中心**：统一 `AppState` 持有 9 套子状态，路由构建器从中心取用，消除 `with_state(())` 重复。
...
当前主要短板集中在两点：**安全边界**（JWT 不验签、管理面裸奔）与**状态/持久化碎片化**（9 套状态、3 类存储）。本轮已完成专家联盟域的流程收口（收藏归一、预约校验、死代码清理），下一轮建议优先处理 P0 安全三项，再推进状态与持久化归一。
