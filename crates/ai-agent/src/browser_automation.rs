...
pub enum BrowserAction {
    ...
    SwitchFrame { selector: Option<String> },
    GoBack,
    GoForward,
    Refresh,
    Close,
...
impl BrowserAutomationEngine {
    ...
    fn register_default_tasks(tasks: &mut HashMap<String, AutomationTask>) {
        ...
        let defaults = vec![
            AutomationTask {
                ...
                steps: vec![
                    BrowserAction::Navigate { url: "https://www.bing.com/search?q={{query}}".into() },
                    BrowserAction::WaitFor { selector: "body".into(), timeout_ms: Some(8000) },
                    BrowserAction::GetTitle,
                    BrowserAction::GetUrl,
                    BrowserAction::ExtractText { selector: "body".into() },
                ],
            ...
            AutomationTask {
                ...
                steps: vec![
                    ...
                    BrowserAction::WaitFor { selector: "body".into(), timeout_ms: Some(10000) },
                    BrowserAction::GetTitle,
                    BrowserAction::ExtractText { selector: "body".into() },
                ],
            ...
            AutomationTask {
                ...
                steps: vec![
                    BrowserAction::Navigate { url: "{{url}}".into() },
                    BrowserAction::WaitFor { selector: "form".into(), timeout_ms: Some(10000) },
                    BrowserAction::Type { selector: "input".into(), text: "{{text}}".into(), clear_first: Some(true) },
                    BrowserAction::Click { selector: "submit".into(), timeout_ms: Some(5000) },
    ...
    pub async fn execute_action(&mut self, session_id: &str, action: BrowserAction) -> Result<ActionResult, BrowserError> {
        ...
        let session_title = self.sessions.get(session_id).map(|s| s.title.clone()).unwrap_or_default();
        let session_history_len = self.sessions.get(session_id).map(|s| s.history.len()).unwrap_or(0);
        ...
        let ar: ActionResult = match &action {
            ...
            BrowserAction::ExtractText { selector } => {
                ...
                let text = cached_page.as_ref()
                    .map(|p| {
                        if selector == "body" || selector == "#b_results" {
                            p.text.chars().take(10000).collect::<String>()
                        } else {
                            format!("[选择器: {}]\n{}", selector, p.text.chars().take(2000).collect::<String>())
                        }
                    })
            ...
            BrowserAction::ExtractHtml => {
                let (html, title, url, truncated, full_len, has_page) = match &cached_page {
                    Some(p) => {
                        ...
                        (h, p.title.clone(), p.url.clone(), p.html.len() > 100000, p.html.len(), true)
                    }
            ...
            BrowserAction::GetTitle => {
                let title = cached_page.as_ref()
                    .map(|p| p.title.clone())
                    .filter(|t| !t.is_empty())
                    .unwrap_or_else(|| {
                        if session_title.is_empty() { "[未获取到标题]".to_string() } else { session_title.clone() }
                    });
            ...
            BrowserAction::WaitFor { selector, timeout_ms } => {
                tracing::info!("[{}] 等待元素: {} (超时: {}ms)", session_id, selector, timeout_ms.unwrap_or(5000));
            ...
            BrowserAction::Screenshot => {
                ...
                self.screenshots.insert(shot_id.clone(), "[screenshot - 需要Headless Chrome]".into());
    ...
    pub async fn execute_task(&mut self, task_id: &str, variables: Option<HashMap<String, String>>) -> Result<TaskExecutionResult, BrowserError> {
        let task = self.saved_tasks.get(task_id).cloned()
        ...
        if let Some(user_vars) = variables { vars.extend(user_vars); }

        if let Some(start_url) = &task.start_url {
            let resolved_url = replace_vars(start_url, &vars);
            let _ = self.execute_action(&session_id, BrowserAction::Navigate { url: resolved_url }).await;
        }
        ...
        for step in &task.steps {
            ...
            if steps_results.is_empty() && matches!(step, BrowserAction::Navigate { .. }) && task.start_url.is_some() {
                // 已经导航过了，直接取缓存结果
                if let Some(nav_result) = self.sessions.get(&session_id).and_then(|s| s.action_log.last().cloned()) {
                    if nav_result.action_type == "navigate" {
                        if let Some(d) = &nav_result.data { extracted_data.insert("page_info".into(), d.clone()); }
            ...
            match self.execute_action(&session_id, resolved).await {
                Ok(r) => {
                    if r.action_type.contains("extract") || r.action_type == "get_title" || r.action_type == "get_url" || r.action_type == "navigate" {
                        if let Some(d) = &r.data {
                            extracted_data.insert(format!("step_{}", steps_results.len()), d.clone());
                        }
                    }
    ...
    pub async fn execute_custom_steps(&mut self, steps: Vec<BrowserAction>, start_url: Option<String>) -> Result<TaskExecutionResult, BrowserError> {
        let tid = format!("custom-{}", &uuid::Uuid::new_v4().to_string()[..8]);
        let task = AutomationTask {
            id: tid.clone(), name: "自定义任务".into(), description: "用户自定义浏览器自动化".into(),
            start_url, steps, variables: HashMap::new(), created_at: chrono::Utc::now(),
        };
    ...
    pub fn parse_natural_language(prompt: &str) -> (Option<String>, Vec<BrowserAction>) {
        ...
        if p.contains("搜索") || p.contains("search") || p.contains("查一下") || p.contains("搜一下") {
            ...
            steps.push(BrowserAction::WaitFor { selector: "body".into(), timeout_ms: Some(8000) });
            steps.push(BrowserAction::GetTitle);
            steps.push(BrowserAction::GetUrl);
            steps.push(BrowserAction::ExtractText { selector: "body".into() });
            return (None, steps);
        }
        ...
        if p.contains("截图") || p.contains("screenshot") {
            if let Some(u) = &extracted_url {
                ...
                steps.push(BrowserAction::WaitFor { selector: "body".into(), timeout_ms: Some(10000) });
            }
    ...
    while i < chars.len() {
        ...
        if !in_script && !in_style {
            if c.is_whitespace() || c == '\n' || c == '\r' || c == '\t' {
                if !last_was_space { text.push(' '); last_was_space = true; }
            } else {
                text.push(c);
...
fn resolve_step(step: &BrowserAction, vars: &HashMap<String, String>) -> BrowserAction {
    match step {
        BrowserAction::Navigate { url } => BrowserAction::Navigate { url: replace_vars(url, vars) },
        BrowserAction::Click { selector, timeout_ms } => BrowserAction::Click { selector: replace_vars(selector, vars), timeout_ms: *timeout_ms },
        BrowserAction::Type { selector, text, clear_first } => BrowserAction::Type { selector: replace_vars(selector, vars), text: replace_vars(text, vars), clear_first: *clear_first },
        BrowserAction::ExtractText { selector } => BrowserAction::ExtractText { selector: replace_vars(selector, vars) },
        BrowserAction::WaitFor { selector, timeout_ms } => BrowserAction::WaitFor { selector: replace_vars(selector, vars), timeout_ms: *timeout_ms },
        BrowserAction::ScrollTo { selector } => BrowserAction::ScrollTo { selector: replace_vars(selector, vars) },
        other => other.clone(),
    }
}
...
fn extract_url(text: &str) -> Option<String> {
    ...
    for word in text.split_whitespace() {
        let w = word.trim_matches(|c: char| c == '.' || c == ',' || c == '!' || c == '?' || c == '"' || c == '\'' || c == ')' || c == '(' || c == '，' || c == '。' || c == '、');
        if w.contains('.') && !w.starts_with('.') && !w.ends_with('.') && w.len() > 6 {
            let wl = w.to_lowercase();
            if wl.starts_with("www.") || wl.ends_with(".com") || wl.ends_with(".cn") || wl.ends_with(".net") || wl.ends_with(".org") || wl.ends_with(".io") || wl.ends_with(".dev") {
...
fn extract_search_query(text: &str) -> String {
    ...
    for kw in &["搜索", "search", "帮我", "请", "查询", "查找", "一下", "搜", "查", "bing", "百度", "google", "在", "上", "网"] {
        q = q.replace(kw, "");
    }
    let q = q.trim().to_string();
    if q.is_empty() { "算子统一系统".into() } else { q }
}
