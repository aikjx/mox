...
//!
//! 所有配置项均可通过环境变量（`ALLIANCE_*` 前缀）覆盖，便于容器化部署与多环境切换，
//! 无需重新编译即可调整配额、绑定地址、限流与 CORS 策略。
use serde::Deserialize;

/// 资源配额（BR-03 / I-03）：防止单一联盟内的无界增长拖垮整体
...
impl AppConfig {
    ...
    pub fn load() -> Self {
        ...
        let get = |k: &str| std::env::var(format!("ALLIANCE_{}", k)).ok();
    ...
    pub fn db_path(&self) -> String {
        format!("{}/alliance.db", self.data_dir.trim_end_matches('/'))
    }
