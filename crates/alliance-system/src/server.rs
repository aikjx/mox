...
use crate::error::*;
use crate::model::*;
use crate::orchestrator::AllianceSystem;
...
pub struct CreateChannelReq {
    alliance_id: String,
    kind: String, // "alliance" | "task:<id>" | "direct"
    name: String,
    members: Vec<String>,
}
...
async fn rate_limit_mw(
    State(sys): State<Arc<AllianceSystem>>,
    req: Request,
    next: Next,
) -> Response {
...
// ---------------- 路由处理器 ----------------
async fn me(Extension(AuthUser(id)): Extension<AuthUser>, State(sys): State<Arc<AllianceSystem>>) -> Result<Json<Member>> {
...
async fn list_members(
    ...
    let alliance_id = q.get("alliance_id").cloned().ok_or_else(|| AppError::BadRequest("缺少 alliance_id".into()))?;
    sys.require(&id, Permission::AuditView, &crate::rbac::ResourceCtx { alliance_id: alliance_id.clone(), task: None }).await?;
    Ok(Json(sys.member.list(&alliance_id).await))
}
...
async fn invite(
    ...
    let m = sys
        .invite_member(
            &id,
            &InviteInput {
                alliance_id: req.alliance_id.clone(),
                name: req.name.clone(),
...
async fn create_task(
    ...
    let t = sys
        .create_task(&id, &req.alliance_id, &req.title, &req.description, parse_priority(&req.priority))
        .await?;
    Ok(Json(t))
}
...
async fn list_tasks(
    ...
    let alliance_id = q.get("alliance_id").cloned().ok_or_else(|| AppError::BadRequest("缺少 alliance_id".into()))?;
    sys.require(&id, Permission::TaskViewAll, &crate::rbac::ResourceCtx { alliance_id: alliance_id.clone(), task: None }).await?;
    Ok(Json(sys.task.list(&alliance_id).await))
}
...
async fn create_channel(
    ...
    sys.require(&id, Permission::CommSendAlliance, &crate::rbac::ResourceCtx { alliance_id: req.alliance_id.clone(), task: None }).await?;
    let ch = sys.comm.create_channel(&req.alliance_id, kind, &req.name, req.members).await;
    Ok(Json(ch))
}
...
async fn list_channels(
    ...
    let alliance_id = q.get("alliance_id").cloned().ok_or_else(|| AppError::BadRequest("缺少 alliance_id".into()))?;
    sys.require(&id, Permission::AuditView, &crate::rbac::ResourceCtx { alliance_id: alliance_id.clone(), task: None }).await?;
    Ok(Json(sys.store.list_channels(&alliance_id).await))
}
...
async fn list_messages(
    ...
    if !is_member {
        sys.require(&id, Permission::AuditView, &crate::rbac::ResourceCtx { alliance_id: ch.alliance_id.clone(), task: None }).await?;
    }
...
async fn my_notifications(
    ...
    sys.require(&id, Permission::AuditView, &crate::rbac::ResourceCtx { alliance_id: String::new(), task: None }).await?;
...
async fn grant_role(
    ...
    sys.require(&id, Permission::MemberManage, &crate::rbac::ResourceCtx { alliance_id: req.alliance_id.clone(), task: None }).await?;
    let role = match req.role.as_str() {
        "AllianceAdmin" => Role::AllianceAdmin,
    ...
    let scope = match &req.scope {
        ...
        Some(s) if s.starts_with("alliance:") => Scope::Alliance(s["alliance:".len()..].to_string()),
        _ => Scope::Global,
    };
...
async fn list_audit(
    ...
    let alliance_id = q
        .get("alliance_id")
        .cloned()
        .ok_or_else(|| AppError::BadRequest("缺少 alliance_id".into()))?;
    let mut records = sys.list_audit(&id, &alliance_id).await?;
...
async fn health(State(sys): State<Arc<AllianceSystem>>) -> Response {
    (
        StatusCode::OK,
        serde_json::json!({
            "status": "ok",
...
async fn metrics_handler(State(sys): State<Arc<AllianceSystem>>) -> Response {
    // 动态刷新「活跃成员」瞬时量
    let active = {
...
async fn handle_socket(sys: Arc<AllianceSystem>, socket: WebSocket, member_id: String) {
...
// ---------------- 应用装配 ----------------
pub fn app(sys: Arc<AllianceSystem>) -> Router {
