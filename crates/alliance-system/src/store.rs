...
impl Store {
    ...
    pub async fn alliance_count(&self) -> usize {
        self.state.read().await.alliances.len()
    }
    ...
    fn p_alliance(&self, a: &Alliance) {
        self.exec(
            "INSERT OR REPLACE INTO alliances (id, data) VALUES (?1, ?2)",
            params![a.id, serde_json::to_string(a).unwrap_or_default()],
        );
    }
    fn p_member(&self, m: &Member) {
        self.exec(
            "INSERT OR REPLACE INTO members (id, alliance_id, data) VALUES (?1, ?2, ?3)",
            params![
                m.id,
                m.alliance_id,
    ...
    fn p_task(&self, t: &Task) {
        self.exec(
            "INSERT OR REPLACE INTO tasks (id, alliance_id, data) VALUES (?1, ?2, ?3)",
            params![
                t.id,
                t.alliance_id,
    ...
    fn p_channel(&self, c: &Channel) {
        self.exec(
            "INSERT OR REPLACE INTO channels (id, alliance_id, data) VALUES (?1, ?2, ?3)",
            params![
                c.id,
                c.alliance_id,
    ...
    // ---------- 联盟 ----------
    pub async fn create_alliance(&self, a: Alliance) {
        self.state.write().await.alliances.insert(a.id.clone(), a.clone());
        self.p_alliance(&a);
    }
    pub async fn get_alliance(&self, id: &str) -> Option<Alliance> {
        self.state.read().await.alliances.get(id).cloned()
    }
    ...
    pub async fn list_members(&self, alliance_id: &str) -> Vec<Member> {
        self.state
            .read()
            .await
            .members
            .values()
            .filter(|m| m.alliance_id == alliance_id)
            .cloned()
            .collect()
    }
    ...
    pub async fn list_tasks(&self, alliance_id: &str) -> Vec<Task> {
        self.state
            .read()
            .await
            .tasks
            .values()
            .filter(|t| t.alliance_id == alliance_id)
            .cloned()
            .collect()
    }
    ...
    pub async fn list_channels(&self, alliance_id: &str) -> Vec<Channel> {
        self.state
            .read()
            .await
            .channels
            .values()
            .filter(|c| c.alliance_id == alliance_id)
            .cloned()
            .collect()
    }
    pub async fn task_channel(&self, alliance_id: &str, task_id: &str) -> Channel {
        {
    ...
    pub async fn ensure_alliance_channel(&self, alliance_id: &str) -> Channel {
        {
            let s = self.state.read().await;
            for c in s.channels.values() {
                if let ChannelKind::Alliance = &c.kind {
                    if c.alliance_id == alliance_id {
                        return c.clone();
                    }
                }
            }
...
fn create_tables(conn: &Connection) -> rusqlite::Result<()> {
    conn.execute_batch(
        "CREATE TABLE IF NOT EXISTS alliances (id TEXT PRIMARY KEY, data TEXT NOT NULL);
         CREATE TABLE IF NOT EXISTS members (id TEXT PRIMARY KEY, alliance_id TEXT NOT NULL, data TEXT NOT NULL);
         CREATE TABLE IF NOT EXISTS tasks (id TEXT PRIMARY KEY, alliance_id TEXT NOT NULL, data TEXT NOT NULL);
         CREATE TABLE IF NOT EXISTS channels (id TEXT PRIMARY KEY, alliance_id TEXT NOT NULL, data TEXT NOT NULL);
...
fn load_state(conn: &Connection) -> State {
    ...
    if let Ok(mut stmt) = conn.prepare("SELECT data FROM alliances") {
        if let Ok(rows) = stmt.query_map([], |r| r.get::<_, String>(0)) {
            for r in rows.flatten() {
                if let Ok(v) = serde_json::from_str::<Alliance>(&r) {
                    st.alliances.insert(v.id.clone(), v);
                }
            }
        }
    }
