...
impl MemberService {
    ...
    pub async fn invite(
        ...
        if self.store.get_alliance(&input.alliance_id).await.is_none() {
        ...
        // 配额（I-03）：单联盟成员数上限
        let count = self.store.list_members(&input.alliance_id).await.len();
        if count >= self.config.quotas.max_members {
            return Err(AppError::Conflict(format!(
                "联盟 {} 已达成员上限（{}）",
                input.alliance_id, self.config.quotas.max_members
            )));
        }
        // BR-04 邀请幂等：同一联盟内 email 唯一（大小写不敏感），
        // 避免重复邀请产生多个成员实体导致权限绑定与通知分裂。
        let email_key = input.email.trim().to_lowercase();
        if let Some(existing) = self
            .store
            .list_members(&input.alliance_id)
            .await
            .into_iter()
        ...
        let m = Member {
            id: new_id("mem"),
            alliance_id: input.alliance_id.clone(),
        ...
        // 专家联盟中，受邀参与者默认获得「专家」角色（作用域限定本联盟）
        let mut bindings = self.store.get_bindings(&m.id).await;
        bindings.push(RoleBinding::alliance(Role::Expert, &m.id, &input.alliance_id));
    ...
    pub async fn set_status(&self, member_id: &str, status: MemberStatus) -> Result<Member> {
        ...
        self.store
            .append_audit(AuditRecord {
                ...
                permission: None,
                scope: format!("alliance={}", updated.alliance_id),
    ...
    pub async fn list(&self, alliance_id: &str) -> Vec<Member> {
        self.store.list_members(alliance_id).await
    }

    pub async fn search(&self, alliance_id: &str, expertise: &str) -> Vec<Member> {
        self.store
            .list_members(alliance_id)
            .await
            .into_iter()
            .filter(|m| {
                m.expertise
...
impl TaskService {
    ...
    pub async fn create(
        ...
        if self.store.get_alliance(alliance_id).await.is_none() {
            return Err(AppError::BadRequest(format!("联盟 {alliance_id} 不存在")));
        }
        // 配额（I-03）：单联盟任务数上限
        let count = self.store.list_tasks(alliance_id).await.len();
        if count >= self.config.quotas.max_tasks {
            return Err(AppError::Conflict(format!(
                "联盟 {alliance_id} 已达任务上限（{}）",
    ...
    pub async fn assign(&self, task_id: &str, by: &str, assignees: Vec<String>) -> Result<Task> {
        ...
        // 若此处不校验，可写入任意 ID —— 包括他联盟成员 —— 造成跨租户权限提升。
        ...
        let updated = self
            ...
            .ok_or_else(|| AppError::NotFound(format!("任务 {task_id}")))?;
        // 确保每位被分派者进入任务频道
        let ch = self.store.task_channel(&updated.alliance_id, task_id).await;
        for a in &assignees {
    ...
    /// BR-07：被分派者必须是「存在 + 同联盟 + Active」的成员
    async fn validate_assignees(&self, task: &Task, assignees: &[String]) -> Result<()> {
        ...
        for a in assignees {
            ...
            if m.alliance_id != task.alliance_id {
                return Err(AppError::Forbidden(format!(
                    "跨联盟分派被拒：成员 {} 属于联盟 {}，任务属于联盟 {}",
                    a, m.alliance_id, task.alliance_id
                )));
            }
    ...
    pub async fn watch(&self, task_id: &str, watcher_id: &str) -> Result<Task> {
        ...
        if m.alliance_id != before.alliance_id {
            return Err(AppError::Forbidden(format!(
                "跨联盟关注被拒：成员 {} 属于联盟 {}，任务属于联盟 {}",
                watcher_id, m.alliance_id, before.alliance_id
            )));
        }
    ...
    /// 追加前置依赖（BR-11）：依赖图必须保持为 DAG，且不得跨联盟
    ///
    ...
    pub async fn add_dependency(&self, task_id: &str, dep_id: &str) -> Result<Task> {
        ...
        if dep.alliance_id != task.alliance_id {
            return Err(AppError::Forbidden(format!(
                "跨联盟依赖被拒：任务属于联盟 {}，依赖任务属于联盟 {}",
                task.alliance_id, dep.alliance_id
            )));
        }
    ...
    pub async fn list(&self, alliance_id: &str) -> Vec<Task> {
        self.store.list_tasks(alliance_id).await
    }
}
