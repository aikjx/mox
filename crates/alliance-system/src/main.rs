...
//!
//! 运行方式：
//! - `cargo run -p alliance-system -- --demo` 运行端到端演示（无需网络）
//! - `cargo run -p alliance-system` 启动 HTTP + WebSocket 服务（:3000）
use std::sync::Arc;
...
/// 端到端演示：引导联盟 → 邀请专家 → 创建/分派/推进任务 → 评论 → 验证通知与权限
async fn run_demo() {
    ...
    let (aln, admin, token) = sys
        .bootstrap("量子计算专家联盟", "璇玑(管理员)", "admin@alliance.io")
        .await
        .expect("bootstrap");
    println!("  联盟: {} ({})", aln.name, aln.id);
    println!("  管理员令牌: {}", token);
    ...
    sys.perm
        .assign_role(alliance_system::rbac::RoleBinding::alliance(
            alliance_system::rbac::Role::Coordinator,
            &e2.id,
            &aln.id,
        ))
        .await;
...
async fn run_server() {
    ...
    // 内存 / 持久化统一按 12-Factor 配置构建：内存模式同样尊重 ALLIANCE_BIND / RATE_LIMIT / CORS
    // （持久化由 config.persist 决定；即使持久化打开失败也回退为内存，但保留同一份运行配置）
    let sys = Arc::new(AllianceSystem::with_config(config.clone()).expect("按配置构建联盟系统失败"));

    // 持久化模式下，若数据库中已存在联盟则不重复引导（幂等启动）
    if sys.store.alliance_count().await == 0 {
        let (aln, _admin, token) = sys
            .bootstrap("默认专家联盟", "管理员", "admin@alliance.io")
            .await
            .expect("bootstrap");
        println!("联盟ID: {}", aln.id);
        println!("管理员令牌: {}", token);
        ...
        println!("（已有联盟的管理员令牌未打印；请使用首次引导时保存的令牌）");
    }
    let _reactor = sys.start_reactor();
