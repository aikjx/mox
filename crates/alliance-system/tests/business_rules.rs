//! 企业级业务规则验收测试
//!
//! 与 `docs/expert-alliance-business-requirements.md` 的业务规则编号一一对应。
//! 每条规则至少覆盖一条正向路径与一条负向路径。
use alliance_system::config::{AppConfig, Quotas};
use alliance_system::model::{
    AuditAction, InviteInput, MemberStatus, Priority, TaskStatus, Tier,
};
use alliance_system::orchestrator::AllianceSystem;

/// 便捷：组建联盟并返回 (系统, 联盟ID, 管理员ID)
async fn setup(name: &str, email: &str) -> (AllianceSystem, String, String) {
...
// ─────────────────────────────────────────────────────────────
// BR-04 邀请幂等：同联盟内 email 唯一
// ─────────────────────────────────────────────────────────────
#[tokio::test]
async fn br04_invite_is_idempotent_per_email() {
    let (sys, aln, admin) = setup("幂等联盟", "admin@i.io").await;
    let input = InviteInput {
        alliance_id: aln.clone(),
        name: "专家A".into(),
        email: "dup@i.io".into(),
        title: "算法".into(),
        expertise: vec![],
    ...
    assert!(matches!(
        again,
        Err(alliance_system::error::AppError::Conflict(_))
    ), "同联盟同 email 重复邀请应返回 Conflict，实际: {again:?}");
...
async fn br07_assign_validates_assignee_identity() {
    let (sys, aln, admin) = setup("联盟A", "admin@a.io").await;
    ...
    assert!(matches!(r, Err(alliance_system::error::AppError::BadRequest(_))),
        "分派不存在成员应 BadRequest，实际: {r:?}");

    // 负向②：分派他联盟成员（跨租户权限提升的核心攻击面）
    let (sys_b, aln_b, admin_b) = setup("联盟B", "admin@b.io").await;
    let outsider = active_expert(&sys_b, &aln_b, &admin_b, "外人", "x@b.io").await;
    // 把外人「搬」进 A 的存储，模拟持有合法成员 ID 但归属其他联盟的场景
    ...
    assert!(matches!(r, Err(alliance_system::error::AppError::Forbidden(_))),
        "跨联盟分派应 Forbidden，实际: {r:?}");
    ...
    assert!(matches!(r, Err(alliance_system::error::AppError::InvalidState(_))),
    ...
    assert!(!t.assignees.contains(&outsider), "他联盟成员不得出现在分派名单");
...
async fn br10_done_gate_blocks_incomplete_work() {
    let (sys, aln, admin) = setup("门禁联盟", "admin@g.io").await;
...
async fn br11_dependency_graph_is_dag() {
    let (sys, aln, admin) = setup("依赖联盟", "admin@d.io").await;
    ...
    // 负向⑤：跨联盟依赖
    let (sys2, aln2, admin2) = setup("依赖联盟2", "admin@d2.io").await;
    ...
    assert!(matches!(r, Err(alliance_system::error::AppError::Forbidden(_))),
        "跨联盟依赖应 Forbidden，实际: {r:?}");
}

#[tokio::test]
async fn br12_terminal_task_status_cannot_transition_out() {
    let (sys, aln, admin) = setup("终态联盟", "admin@t2.io").await;
...
async fn br21_member_status_machine_enforced() {
    let (sys, aln, admin) = setup("状态机联盟", "admin@s.io").await;
    let m = sys
        .invite_member(&admin, &InviteInput {
            alliance_id: aln.clone(),
            name: "专家".into(),
...
async fn br18_authz_denial_is_audited() {
    let (sys, aln, admin) = setup("审计联盟", "admin@au.io").await;
...
/// 便捷：以指定配额配置组建联盟，返回 (系统, 联盟ID, 管理员ID)
async fn setup_with(
    name: &str,
    email: &str,
    quotas: Quotas,
) -> (AllianceSystem, String, String) {
...
// ─────────────────────────────────────────────────────────────
// NFR-09 资源配额约束（I-03）：防止单一联盟/任务无界增长
// 六个维度全部以负向边界用例固化
// ─────────────────────────────────────────────────────────────
#[tokio::test]
async fn nfr09_max_members_enforced() {
    ...
    let r = sys
        ...
        .await;
    assert!(matches!(r, Err(alliance_system::error::AppError::Conflict(_))),
