//! Step 2：Hermes 工具调用 ↔ flow_ai::FlowNode 规范化映射。
//!
//! 设计：bridge 只认 Hermes 中间件上下文里的 `tool_name` + `args`（均为 serde_json::Value），
//! 把它们转成 flow_ai 的统一流程图节点，使任何 Hermes 工具（含 MCP 第三方工具）都能成为流程图节点。
...
/// 把 Hermes 工具名映射到 flow_ai 的 ToolKind。
