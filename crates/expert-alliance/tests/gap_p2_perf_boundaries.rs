...
//!
//! 覆盖三类压力/边界场景：
//!  P2.1  1000+ 节点 CPM 性能：深链/扇出/全链路联盟优化均需在预算内完成，且关键路径正确；
...
use expert_alliance::executor::run_report;
use expert_alliance::programming::{programming_pipeline, ProgrammingReport};
use expert_alliance::verify::verify;
...
fn cpm_deep_chain_preserves_real_data_dependency() {
    ...
    // 1000+ 节点的规模/性能由下方 fanout / independent / alliance 测试覆盖。
    let n: u32 = 400;
...
#[test]
fn alliance_optimize_1000_nodes_scales() {
    // 全链路联盟优化（含七专家派发 + 裁决 + 验证）在 1000 节点下仍应在预算内完成
    let n: u32 = 1000;
    let mut g = FlowGraph::new("all1000", "千级联盟优化");
    g.add_node(FlowNode::new("s", "开始", NodeKind::Start));
    g.add_node(FlowNode::new("e", "结束", NodeKind::End));
    let mut prev = "s".to_string();
    for i in 0..n {
...
fn boundary_empty_graph_no_panic() {
    ...
    // 联盟层面对空图也不应 panic
    let rep2 = expert_alliance::pipeline::alliance_optimize(&g, &safe_ctx());
    assert!(!rep2.algo.vetoed, "空图联盟优化不应否决");
}
