...
use expert_alliance::context::{GovernContext, LoopGuard, LoopPolicy, McpTool, Principal, Tenant};
use expert_alliance::govern::{AuditChain};
use expert_alliance::ir::{auto_dimension};
use expert_alliance::pipeline::alliance_optimize;
use expert_alliance::reconcile::reconcile;
...
fn gov_flow() -> FlowGraph {
    ...
    g.pools.push(flow_ai::model::ResourcePool { name: "browser".into(), capacity: 1 });
    g
}

#[test]
fn alliance_optimize_passes_and_audit_clean() {
    let g = gov_flow();
    ...
    ctx.registry.register_mcp("fs", vec![McpTool {
        server: "fs".into(),
        name: "write".into(),
        input_schema: "{}".into(),
        pool: "mcp_fs".into(),
    }]);
    ctx.registry.register_skill(expert_alliance::context::SkillRef {
        id: "rpa-citizen".into(),
...
fn missing_desensitize_blocked_by_gate() {
    ...
    g.add_node(FlowNode::new("s", "开始", NodeKind::Start));
    g.add_node(FlowNode::task("read", "读公民库", ToolKind::Database, 100)
        .with_access(Access::read("db:citizen_info")));
    g.add_node(FlowNode::task("send", "外发", ToolKind::Http, 100)
        .with_access(Access::read("db:citizen_info")));
    g.add_node(FlowNode::new("e", "结束", NodeKind::End));
    g.add_edge(FlowEdge::seq("s", "read"));
    ...
    if !rep.optimization.optimized_graph.nodes.iter().any(|n| n.kind == NodeKind::Guard) {
...
fn rbac_denied_experts_skipped_not_panicked() {
    let g = gov_flow();
    let tenant = Tenant::new("t", "ns").regulated(true).with_pool("browser", 1);
...
fn browser_mutex_injected_from_resource_conflict() {
    ...
    let experts = expert_alliance::experts::all_experts();
    let tenant = Tenant::new("t", "ns").with_pool("browser", 1);
    let principal = Principal::new("admin").with_roles(vec!["admin".into(), "editor".into()]);
    let ctx = GovernContext::new(tenant, principal);
    let ectx = expert_alliance::context::ExpertContext::new(&df.base, &ctx);
    let opinions = expert_alliance::expert::dispatch(&ectx, &experts);
