...
- 在 `pipeline::alliance_optimize` 中插入：步骤 [5] 调用 `verify(&raw_base, &opt)`，结果写入 `GovernanceReport.algo`（新增字段），并作为 `govern` 的硬输入：`if algo.vetoed { gate = Blocked(reason="algorithm_veto") }`
