...
impl Expert for ResourceExpert {
    fn id(&self) -> String { "resource".into() }
    fn dimension(&self) -> Dimension { Dimension::Resource }
    ...
    fn analyze(&self, ctx: &ExpertContext) -> ExpertOpinion {
        ...
        for (pool, cap) in &ctx.tenant.pool_caps {
            o.constraints.push(Constraint::ResourceCap(pool.clone(), *cap));
        }
        o.constraints.push(Constraint::ResourceCap("cpu".into(), ctx.quota.max_parallel));
            ...
            if users.len() > 1 {
                ...
                if cap < users.len() as u32 {
                    o.push_risk(
                        ...
                        format!("{} 个浏览器任务被并发调度，但实例容量仅 {}，将互相抢占页面", users.len(), cap),
                        Some("自动插入串行互斥边".into()),
                    );
                    // 建议两两串行（由裁决器物化为 Mutex 边）
                    for w in users.windows(2) {
                        o.constraints.push(Constraint::MustSerialize(crate::expert::NodeEdge {
                            from: w[0].clone(),
                            to: w[1].clone(),
                        }));
                    }
        ...
        if total > ctx.quota.sla_ms {
            o.push_risk(
                ...
                vec![],
                format!("串行耗时 {}ms 超出 SLA {}ms，需并行优化", total, ctx.quota.sla_ms),
