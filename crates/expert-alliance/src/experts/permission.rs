...
impl Expert for PermissionExpert {
    ...
    fn dimension(&self) -> Dimension { Dimension::Permission }

    fn analyze(&self, ctx: &ExpertContext) -> ExpertOpinion {
        if !ctx.can(crate::context::Capability::EditFlow) {
            return ExpertOpinion::skipped("permission", Dimension::Permission, "无 edit-flow 权限");
        }
        ...
        for n in &g.nodes {
            ...
            let has_guard = has_guard
                || g
                    .edges
                    .iter()
                    .any(|e| e.from == n.id && g.node(&e.to).map(|s| s.kind == NodeKind::Guard).unwrap_or(false));
            if !has_guard {
                o.constraints.push(Constraint::MustGuard(n.id.clone(), vec!["desensitize".into()]));
                o.push_risk(
                    flow_ai::model::Severity::Blocking,
                    vec![n.id.clone()],
                    format!("节点「{}」触碰敏感数据但缺少脱敏/鉴权前置拦截（政务等保）", n.name),
        ...
        for n in &g.nodes {
            ...
            if writes_prod {
                o.push_veto(
                    vec![n.id.clone()],
                    format!("节点「{}」写生产/敏感数据但缺少 authz 鉴权且无安全审批（数据不可回退）", n.name),
                    Some("补充 authz Guard 并由 safety_approver 审批".into()),
                );
            } else {
                o.constraints.push(Constraint::MustGuard(n.id.clone(), vec!["authz".into()]));
                o.push_risk(
