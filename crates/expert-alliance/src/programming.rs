...
use crate::pipeline::{alliance_optimize, GovernanceReport};
use flow_ai::codegen::{generate, CodeBundle};
use flow_ai::model::{FlowGraph, FlowNode, NodeKind};
...
pub fn programming_pipeline(
    ...
    //      此处不再重复检查。
    // ②-⑥⑨ 复用已验证的 alliance_optimize（含七专家/裁决/优化/verify/治理）
    let gov = alliance_optimize(graph, ctx);
...
/// 循环护栏（兼容性：消费 `ctx.registry.loops`，弥补既有 `alliance_optimize`
...
mod tests {
    ...
    use flow_ai::codegen::{CodeBundle, GeneratedFile};
    use flow_ai::model::{Access, FlowEdge, ToolKind};
