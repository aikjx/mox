...
//!   alliance optimize <flow.json> [--out DIR]   跑全维优化并输出报告 JSON / 生成的 Python
//!   alliance demo                          内置政务场景端到端演示（含可视化 DTO 校验）

use expert_alliance::context::{GovernContext, Principal, Tenant};
use expert_alliance::pipeline::alliance_optimize;
use expert_alliance::server::{demo_topology, to_viz};
...
fn cmd_bench() -> anyhow::Result<()> {
    let rows = expert_alliance::bench::run_benchmarks();
    println!("{}", expert_alliance::bench::bench_table(&rows));
    let csv = expert_alliance::bench::bench_csv(&rows);
    let path = std::env::temp_dir().join("alliance_bench.csv");
    fs::write(&path, &csv)?;
...
fn cmd_demo() -> anyhow::Result<()> {
    ...
    println!("=== 可视化 DTO ===");
    println!("  流程图节点: {}  边: {}", viz.flow_nodes.len(), viz.flow_edges.len());
    println!("  关系网实体: {}  关系: {}", viz.entities.len(), viz.relations.len());
    println!("  关键路径: {:?}", viz.critical_path);
    println!("  复用路径: {:?}", viz.reuse_path);
    println!("  冲突标红节点: {}", viz.flow_nodes.iter().filter(|n| n.highlight == "conflict").count());
    println!("  关键路径高亮节点: {}", viz.flow_nodes.iter().filter(|n| n.highlight == "critical").count());
    Ok(())
}
...
fn cmd_optimize(args: &[String]) -> anyhow::Result<()> {
    ...
    if let Some(dir) = out {
        ...
        let topo = demo_topology();
        let viz = to_viz(&rep, Some(&topo));
        fs::write(format!("{dir}/viz.json"), serde_json::to_string_pretty(&viz)?)?;
...
fn cmd_serve(args: &[String]) -> anyhow::Result<()> {
    ...
    rt.block_on(async move {
        let topo = demo_topology();
        let state = expert_alliance::server::AppState {
        ...
        let app = expert_alliance::server::router(state)
