...
> 让业务流程图 / 算法流程图 / 权限流程图 / 资源流程图在**同一套归一化 IR** 上被全维度联合优化。
...
1. **归一化（Single Source of Truth）**
   ...
   不存在"四套图各自优化再合并"——那会产生数据孤岛与冲突。全维视角 = 同一张图被七个镜头同时审视。

2. **全维（Full-Dimension）**
   每个节点同时可被业务、算法、权限、资源、安全、数据、可观测七个维度评估；优化结论必须**同时满足全维约束**，不做单维最优而牺牲其他维。

3. **最优算法处理（Optimal）**
   最终求解交给 `flow-ai` 已验证的算法栈（冒险分析并行化 + CPM 关键路径 + RCPSP 资源调度 + 冲突自动修复 + Dijkstra 关系网最短路）。本层不重造算法，只负责把全维约束**正确翻译**成引擎能吃的输入。
...
归一化关键：**物理节点唯一，维度只是标签**。这样"改一个节点，全维同步"天然成立，无需任何跨图同步逻辑。

---

## 2. 专家联盟（Expert Council）
...
> 设计要点：裁决器**只翻译、不求解**。它把全维约束物化为 flow-ai 能识别的边/规则，
...
```rust
pub fn alliance_optimize(raw: &RawFlow, ctx: &GovernContext) -> GovernanceReport {
...
```
crates/expert-alliance/
├── Cargo.toml
├── src/
│   ├── lib.rs              # 汇总导出 + alliance_optimize 入口
...
- **契约**：`alliance_optimize` 对任意合法输入不产生 panic；`ReconciledPlan` 永远是可拓扑排序的 DAG（Mutex 边已保证不环）。
...
**开发专家联盟 = 七个领域专家在归一化 IR 上并行诊断，裁决器按「权限/安全优先」全维归一，
flow-ai 引擎做已验证的最优求解，治理层把关后出码——一张图，全维最优，企业可治理。**

---

## 11. 兼容性设计（MCP / Skills / Loops / 大模型）

> 本层不重写任何外部协议，而是把外部能力**归一化进同一张 FlowGraph**，让专家联盟对它们一视同仁地做全维优化。
...
- 与 `flow-ai::topology` 关系网打通：`SkillRef.keywords` 即拓扑网中 Skill 实体的语义词，
  语音/自然语言指令先过拓扑最短路检索，命中即**跳过完整 ReAct 推理**（快路径），未命中才走全维推理兜底。
- 专家联盟产出的 `ReconciledPlan` 可反向沉淀为 `SkillRef.flow_template` → 存入记忆图谱，实现「执行即提炼、复用即剪枝」。
...
- 多模型结果经「算法联盟」式融合（多数投票 / 置信度加权）写入节点 `write_set`，参与下游依赖。
