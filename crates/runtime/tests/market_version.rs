...
fn make_pkg(id: &str, version: &str, requirement: &str) -> OperatorPackage {
    OperatorPackage {
        ...
        features: vec![],
        tags: vec![],
        created_at: runtime::market_migration::now_rfc3339(),
        updated_at: runtime::market_migration::now_rfc3339(),
        clone_count: 0,
        forked_from: None,
...
fn tenant_owner_permission_filters() {
    ...
    runtime::market::reload_index_sync(&state);
...
async fn init_state_creates_seed_and_index() {
    ...
    let state = runtime::market::init_market_state().await;
