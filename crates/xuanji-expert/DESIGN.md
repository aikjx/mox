...
```rust
pub fn xuanji_optimize(raw: &RawFlow, ctx: &GovernContext) -> GovernanceReport {
...
```
crates/xuanji-expert/
├── Cargo.toml
├── src/
│   ├── lib.rs              # 汇总导出 + xuanji_optimize 入口
...
- **契约**：`xuanji_optimize` 对任意合法输入不产生 panic；`ReconciledPlan` 永远是可拓扑排序的 DAG（Mutex 边已保证不环）。
...
> 本层不重写任何外部协议，而是把外部能力**归一化进同一张 FlowGraph**，让璇玑对它们一视同仁地做全维优化。
