...
- 在 `pipeline::xuanji_optimize` 中插入：步骤 [5] 调用 `verify(&raw_base, &opt)`，结果写入 `GovernanceReport.algo`（新增字段），并作为 `govern` 的硬输入：`if algo.vetoed { gate = Blocked(reason="algorithm_veto") }`
...
- [x] **Step 2** `pipeline.rs`：`GovernanceReport` 加 `pub algo: AlgoVerification`；`xuanji_optimize` 调用 `verify` 并让治理尊重 veto
...
- [x] **Step 5** 前端 `xuanji.html`：新增「⛨ 璇玑验证」卡片
...
- [x] **Step 7** CLI `xuanji verify <flow.json>` 子命令（退出码 0=通过 / 2=否决）；`demo` 展示验证结论
