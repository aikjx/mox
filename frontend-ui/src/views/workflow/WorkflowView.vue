<template>
  <div class="page-container" v-loading="loading" element-loading-text="工作流数据加载中...">
    ...
    <!-- 插件中心 / MCP / 自动化 Tab 内容（嵌套路由渲染） -->
    <router-view v-if="outerTab !== 'flows'" v-slot="{ Component }">
...
// 使用嵌套路由驱动：从子路由名推断 outerTab
const outerTab = computed(() => {
  const name = route.name?.toString() || 'WorkflowFlows'
  if (name.includes('Plugin')) return 'plugins'
  if (name.includes('Mcp') || name.includes('MCP')) return 'mcp'
  if (name.includes('Automation')) return 'automation'
  // 兼容旧的 query.tab 链接
  const q = route.query.tab
  if (q === 'plugins' || q === 'mcp' || q === 'automation') return q
  return 'flows'
})
function onOuterTabChange(tab) {
  ...
  router.push(routes[tab] || routes.flows)
}
