...
const PHASES = [
  ...
  { key: 'architecture', label: '🏗️ 架构阶段', desc: '知识图谱 / 专家联盟 / 架构融合', color: '#06b6d4' },
...
const QUICK_QUESTIONS = [
  { icon: '🏢', label: '生成公司官网的需求图谱', hint: '覆盖角色/目标/用例/数据/技术', prompt: '生成「公司官网」的架构需求图谱：含角色、目标、用例、数据、技术选型与验收清单。', chip: '需求知识图谱' },
  { icon: '🧾', label: '需求知识图谱', hint: '生成项目需求的结构化知识图谱', prompt: '请生成当前项目的需求知识图谱，并输出节点与关系边列表。', chip: '需求知识图谱' },
  { icon: '🏗', label: '需求架构（S1）', hint: '输入自定义问题，自动生成架构草案', prompt: '请对当前项目做架构的需求架构分析：业务场景、角色、核心用例、非功能需求。', chip: '需求知识图谱' },
...
const customPlaceholder = computed(() => {
  ...
  if (selectedChip.value === '需求架构（S1）') return '需求架构：输入自定义问题，系统将架构分析处理…'
  if (selectedChip.value === '血清空（初始化会话）') return '血清空：请确认要重置上下文，并输入新项目的描述…'
  return '自定义问题：输入你的问题，系统将架构分析处理（支持生成需求图谱 / 架构 / 算法 / 任务拆解 / 创建项目）…'
})
