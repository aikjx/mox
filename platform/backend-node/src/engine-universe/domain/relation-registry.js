...
const ENGINE_EDGES = [
  ...
  { from: 'orchestration-engine', to: 'flow-engine', type: 'delegates_to', note: '复杂流程状态机委托 Rust flow-engine（mox-system 协同）' },
  ...
  { from: 'engine-rust-mox-expert',    to: 'engine::mox_expert',      type: 'equivalent_to', note: 'Rust 璇玑专家平台 JS 视图 ↔ Rust crate' },
  { from: 'engine-rust-mox-system',    to: 'engine::mox_system',      type: 'equivalent_to', note: 'Rust 系统底座 ↔ Rust crate 正式条目' },
  ...
  // engine::* 中另外 5 条 engine-rust 条目尚未建（business_catalog / hermes_flow_bridge / operator_wasm / template_market / mox_common_meta），这里补接入
  { from: 'engine-rust-mox-system',    to: 'engine::business_catalog',   type: 'delegates_to', note: '业务目录作为 mox-system 的子能力（跨源数据统一目录）' },
  ...
  { from: 'engine-rust-mox-system',    to: 'engine::mox_common_meta', type: 'depends_on', note: '系统底座依赖公共元数据常量与 URN 解析' },
  ...
  // ========== [P1-2 V6 孤岛消除] FlowAI / Hermes / mox_expert 跨层桥（主分量 → Rust 分量）==========
  ...
  { from: 'expert-alliance-engine', to: 'engine-rust-mox-expert', type: 'delegates_to', note: '联盟咨询的验证管线（拓扑/冲突/数据依赖）委托 Rust' },
  ...
  { from: 'engine-kernel',          to: 'engine-rust-mox-system', type: 'depends_on', note: '内核 RBAC / 加密 / 限流 依赖 Rust mox-system 组件' },
