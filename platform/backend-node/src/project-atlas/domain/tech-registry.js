...
const ALGORITHMS = [
  {
    ...
    singleSource: true, impl_kind: 'co_impl', primary_impl: 'RUST',
    main: 'rust',
    co_impl: ['node:GraphFormulas'],
    main_crate: 'graph-algorithms',
  ...
  },
  {
    ...
    codePath: 'src/ai-flow-graph.js',
    principle: '无向边统一 RAW_EXPAND 输入库内展开，度 = 出度 + 入度；Rust primary + Node secondary（co_impl）',
    singleSource: true, impl_kind: 'co_impl', primary_impl: 'RUST',
    main: 'rust',
    co_impl: ['node:GraphFormulas'],
    main_crate: 'graph-algorithms',
  ...
  {
    id: 'algo-raw-expand', name: 'RAW_EXPAND 原始边展开（度中心性前置处理）',
    codePath: '../services/graph-algorithms/src/lib.rs',
    principle: 'RAW_EXPAND：无向边双向展开 + 自环归一，作为度中心性/Brandes 输入预处理；Rust primary + Node GraphFormulas secondary（co_impl）',
    singleSource: true, impl_kind: 'co_impl', primary_impl: 'RUST',
    main: 'rust',
    co_impl: ['node:GraphFormulas'],
    main_crate: 'graph-algorithms',
    ...
    primary_impl_codePath: 'platform/services/graph-algorithms/src/lib.rs#L439-L458',
    secondary_impl_codePath: 'src/ai-flow-graph.js',
    category: '图算法', consumers: ['ai-engine', 'graph', 'engine-rust-graph-algorithms']
  },
  {
...
const DATA_ASSETS = [
  ...
  { file: 'alliance_learned_skills.json', domain: 'expert-alliance', desc: '联盟学习技能（门禁通过沉淀 · 原子写）' },
  ...
  { file: 'atlas_auto_registry_rust.json', domain: 'atlas', desc: 'Rust crate 自同步扫描产物（16 crate 的文件/fn/struct/const 索引）' },
...
const DOCS = [
  ...
  { file: 'docs/standards/mox-studio.md', domain: 'studio', desc: '璇玑工作台（豆包式低门槛交互 + JS 沙箱在线运行）' },
  ...
  { file: 'docs/standards/expert-alliance-flow-standard.md', domain: 'mcp', desc: 'EAF-STD-001 V1.2 §9：MCP 标准协议暴露（JSON-RPC 2.0 七大工具）与 SSE 流式契约' },
  ...
  { file: 'docs/modules/mox-expert-alliance-fusion-flows.md', domain: 'expert-alliance', desc: '联盟融合流程' },
  { file: 'docs/modules/mox-expert-normalization.md', domain: 'expert-alliance', desc: '专家归一化设计' },
