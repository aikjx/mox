...
fn tr7_6_cdc_batch_aggregation_gathers_flush() {
    ...
    for i in 0..50 {
        cdc.emit(
            "default",
            xuanji_graph_storage::CdcEventType::EdgeCreated,
            format!("{{\"i\":{i}}}"),
        );
    }
...
fn hot_cache_case(total: usize, cap: usize) {
    ...
    let fresh = xuanji_graph_storage::HotNeighborCache::new(cap);
...
impl ReadVertexPubHelper
    for xuanji_graph_storage::storage_api::LruCache<
        String,
        Vec<xuanji_graph_storage::storage_api::Neighbor>,
    >
{
...
impl ReadVertexPubHelper for StorageServer {
    fn read_vertex_pub(&self, vid: &str) -> BTreeMap<String, PropValue> {
        ...
        let shard = xuanji_graph_storage::graph_codec::vid_hash_shard(vid, sc);
        ...
        let rows = self
            ...
            .unwrap_or_default();
        for (k, v) in rows {
            if let Ok((_, _, vv)) = xuanji_graph_storage::graph_codec::decode_vertex_key(&k) {
                if vv == vid {
                    if let Ok((_t, p)) = xuanji_graph_storage::graph_codec::decode_vertex_value(&v)
                    {
                        return p;
                    }
                }
