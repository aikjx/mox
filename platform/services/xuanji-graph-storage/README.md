# xuanji-graph-storage（璇玑关系图 R2 Storage Service）
...
use xuanji_graph_storage::{Direction, PropValue, StorageServer};
...
上层通过 `xuanji-graph-meta` 的 partition store 路由到具体 storage 主机 → 本 StorageServer。
