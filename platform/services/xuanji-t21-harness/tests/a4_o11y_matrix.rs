...
use xuanji_t21_harness::o11y::{BenchSamples, XuanjiMetrics, interpolated_percentile, METRIC_BASE_NAMES};
...
fn check_observe_n(n: u64) {
    ...
    assert!(text.contains(&format!("xuanji_mountpath_faulty_total 2")));
    assert!(text.contains(&format!("xuanji_legalhold_active_objects 5")));
    // All counters multiplied by n
    let expect = n.to_string();
    for (i, name) in ["xuanji_mpu_parts_total", "xuanji_ec_shard_rebuild_total",
                      "xuanji_miji_denied_read_total", "xuanji_miji_denied_write_total",
                      "xuanji_crc_mismatch_total"].iter().enumerate() {
