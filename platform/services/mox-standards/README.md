# mox-standards — Mox V5 §3 10 标准矩阵测试骨架
...
本 crate 定义了 Mox V5 平台对外承诺的 **10 项行业/国际标准接口契约**，并以纯 Rust
单元测试矩阵的方式独立可运行（`cargo test -p mox-standards --test t14_standards_matrix`）。
...
`mox-domain-abstractions` 中的 `Mock*Provider` 为底层 in-memory 实现。
里程碑到来时按以下方式替换零侵入：
...
`platform/services/mox-standards/tasks.md` 有 TR14 子任务可追溯。

**Q4：为什么 workspace 成员里没有 `mox-graph-meta`？**
...
Dual-licensed under **MIT OR Apache-2.0** at your option (matching the Mox V5 workspace
