//! Xuanji Cloud Drive M3: POSIX Filer crate.
//!
//! 三个元数据后端：
//! - SQLite（内存：`rusqlite`）
...
//! FUSE 客户端为 Xuanji 自研（模拟 mount/ls/write/s3 list，避免跨平台依赖）。
//!
//! ## 自研边界
...
pub use meta_trait::{
    Attr, DirEntry, MetaBackend, MetaStorageProvider, META_BACKENDS, PJD_CASES_TOTAL,
    PJD_PASS_THRESHOLD,
};
