...
use xuanji_cloud_drive_master::MasterServer;
use xuanji_domain_abstractions::PartETag;
...
fn op_list_buckets(storage: &InMemoryStorage) -> Response<Body> {
    ...
    let owner_id = "xuanji-owner-id";
    let owner_disp = "xuanji";
    let body = format!(
...
fn list_objects_render(
    ...
    for (k, m) in &contents {
        ...
        xml.push_str("    <Owner><ID>xuanji</ID><DisplayName>xuanji</DisplayName></Owner>\n");
        xml.push_str("  </Contents>\n");
    }
...
fn op_get_object_acl(storage: &InMemoryStorage, bucket: &str, key: &str) -> Response<Body> {
    ...
    let xml = meta.acl.to_acl_xml("xuanji-owner-id", "xuanji");
    S3Server::xml_response(StatusCode::OK, xml)
}
...
fn op_list_multipart_uploads(mpu: &MultipartManager, bucket: &str, query: &str) -> Response<Body> {
    ...
    for up in list.iter().take(1000) {
        ...
        inner.push_str(
            "    <Initiator><ID>xuanji</ID><DisplayName>xuanji</DisplayName></Initiator>\n",
        );
        inner.push_str("    <Owner><ID>xuanji</ID><DisplayName>xuanji</DisplayName></Owner>\n");
    ...
    let body = format!(
        "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n\
         <ListPartsResult xmlns=\"http://s3.amazonaws.com/doc/2006-03-01/\">\n\
           ...
           <Owner><ID>xuanji</ID><DisplayName>xuanji</DisplayName></Owner>\n\
           <Initiator><ID>xuanji</ID><DisplayName>xuanji</DisplayName></Initiator>\n\
