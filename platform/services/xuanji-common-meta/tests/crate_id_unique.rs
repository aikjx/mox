...
use xuanji_common_meta::all_crate_metas;
...
fn test_crate_ids_well_formed_uuid() {
    ...
    for m in &metas {
        ...
        assert_eq!(parts.len(), 5, "id for {} should have 5 dash-separated parts: {}", m.name, m.id);
        ...
        assert_eq!(parts[4].len(), 12, "id[4] should be 12 chars for {}", m.name);
        // v5 marker: parts[2][0..1] should be '5'
        assert_eq!(&parts[2][0..1], "5", "id should be UUIDv5 (version byte 5) for {}", m.name);
    }
}
