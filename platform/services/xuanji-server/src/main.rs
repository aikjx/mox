...
//!
//! Parses argv via `clap` derive. When invoked as `xuanji-server server
...
//! summary produced by [`xuanji_server::cli_run`].
...
use xuanji_server::{cli_run, Cli, CliState, Command, ServerArgs, ServerState, serve_forever};
...
fn run_server_forever(args: ServerArgs) -> ExitCode {
        ...
        .build()
    {
        Ok(rt) => rt,
        Err(e) => {
            eprintln!("xuanji-server: tokio runtime build error: {e}");
    ...
    eprintln!(
        "[xuanji-server] 🚀 entering single-node mode: public={public} ctrl={ctrl} data={data}"
    );
    eprintln!(
        "[xuanji-server] 🔌 ctrl & data endpoints piggyback on public listener in this build. \
    ...
    rt.block_on(async move {
        match serve_forever(args, state).await {
            Ok(()) => ExitCode::from(0),
            Err(e) => {
                eprintln!("xuanji-server: fatal: {e}");
                ExitCode::from(1)
            }
        }
    })
