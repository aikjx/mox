...
use rand::Rng;
use xuanji_fusion::audit_sync::{AuditChain, AuditRecordKind};
use xuanji_fusion::cdc_stage::tag_cdc_graph_stage;
use xuanji_fusion::graph_writer::{self, GraphWriter};
use xuanji_fusion::tag_parser::{Tag, TagSet};
...
fn put_via_pipeline(
...
) -> std::result::Result<(), xuanji_fusion::graph_writer::Error> {
    let ct = headers
        .iter()
    ...
    if alarm.as_deref() == Some("truncated") {
        use xuanji_fusion::audit_sync::AuditEvent;
