...
use mox_expert::server::{demo_topology, to_viz};
...
fn cmd_bench() -> anyhow::Result<()> {
    let rows = mox_expert::bench::run_benchmarks();
    println!("{}", mox_expert::bench::bench_table(&rows));
    let csv = mox_expert::bench::bench_csv(&rows);
...
fn cmd_serve(args: &[String]) -> anyhow::Result<()> {
    ...
    rt.block_on(async move {
        let topo = demo_topology();
        let state = mox_expert::server::AppState {
        ...
        let app =
            mox_expert::server::router(state).layer(tower_http::cors::CorsLayer::permissive());
