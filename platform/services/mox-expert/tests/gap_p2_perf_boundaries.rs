...
use mox_expert::executor::run_report;
use mox_expert::programming::{programming_pipeline, ProgrammingReport};
use mox_expert::verify::verify;
...
fn mox_optimize_1000_nodes_scales() {
    ...
    let start = Instant::now();
    let rep = mox_expert::pipeline::mox_optimize(&g, &ctx);
    let elapsed = start.elapsed();
...
fn boundary_empty_graph_no_panic() {
    ...
    // 璇玑层面对空图也不应 panic
    let rep2 = mox_expert::pipeline::mox_optimize(&g, &safe_ctx());
