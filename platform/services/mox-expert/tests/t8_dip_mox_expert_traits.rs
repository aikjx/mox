...
//!   凡 `use mox_expert::X` 形式只允许 X ∈ {expert_traits, types, domain}。
...
use std::sync::Arc;
use mox_expert::domain::{
    GovernContext, GovernExpert, GovernLevel, GovernVerdict, MinimalGovernContext, MockGovernExpert,
};
use mox_expert::expert_traits::{AllianceOrchestrator, ExpertConsultant, ExpertRegistry};
use mox_expert::types::{ConsultQuery, ConsultReport, TaskSpec};
...
fn scan_mox_use_violations(file: &Path) -> Vec<String> {
    ...
    for (idx, line) in content.lines().enumerate() {
        ...
        let rest_after_keyword = match trimmed.find("mox_expert::") {
            Some(p) => &trimmed[p..],
            None => continue,
        };
        let after = &rest_after_keyword["mox_expert::".len()..];
        let end = after
        ...
        if !allowed.contains(&mod_name) {
            violations.push(format!(
                "{}:{} 非白名单模块 use mox_expert::{} (期望 expert_traits/types/domain) —— 行内容：{}",
                file.display(),
                idx + 1,
                mod_name,
                line.trim()
...
impl ExpertConsultant for MockExpertEmpty {
    async fn consult(&self, _q: &ConsultQuery) -> mox_expert::types::Result<ConsultReport> {
        unreachable!("sync-only mock")
    }
    fn consult_blocking(&self, q: &ConsultQuery) -> mox_expert::types::Result<ConsultReport> {
        Ok(ConsultReport {
...
async fn tr_08_09_alliance_orchestrator_trait_object_safe() {
    ...
    let reg: Arc<dyn ExpertRegistry> = mox_expert::expert_traits::default_registry();
    let router: Arc<dyn AllianceOrchestrator> =
        mox_expert::expert_traits::default_orchestrator(reg);
