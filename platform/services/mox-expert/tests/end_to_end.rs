...
use mox_expert::context::{GovernContext, LoopGuard, LoopPolicy, McpTool, Principal, Tenant};
use mox_expert::govern::AuditChain;
use mox_expert::ir::auto_dimension;
use mox_expert::pipeline::mox_optimize;
use mox_expert::reconcile::reconcile;
...
fn mox_optimize_passes_and_audit_clean() {
    ...
    ctx.registry
        .register_skill(mox_expert::context::SkillRef {
            id: "rpa-citizen".into(),
...
fn browser_mutex_injected_from_resource_conflict() {
    ...
    let experts = mox_expert::experts::all_experts();
    let tenant = Tenant::new("t", "ns").with_pool("browser", 1);
    let principal = Principal::new("admin").with_roles(vec!["admin".into(), "editor".into()]);
    let ctx = GovernContext::new(tenant, principal);
    let ectx = mox_expert::context::ExpertContext::new(&df.base, &ctx);
    let opinions = mox_expert::expert::dispatch(&ectx, &experts);
