...
fn parse_create(sql: &str) -> Option<String> {
    ...
    let p = rest.find('(')?;
    Some(rest[..p].trim().trim_matches('"').trim_matches('`').to_string())
}
fn parse_insert(sql: &str) -> Option<(String, Vec<String>)> {
    ...
    let cols = rest[p + 1..p + 1 + end].split(',').map(|c| c.trim().to_string()).collect();
    Some((table, cols))
}
fn parse_select(sql: &str) -> Option<SelectReturn> {
    ...
    let cols: Vec<String> = rest[..from].split(',').map(|c| c.trim().to_string()).collect();
...
impl PersistenceProvider for MockPersistence {
    fn exec(&self, sql: &str, params: &[SqlValue]) -> anyhow::Result<usize> {
        ...
        if lo.starts_with("create table") {
            ...
            let (tbl, cols) = parse_insert(sql).ok_or_else(|| anyhow::anyhow!("INSERT parse fail"))?;
            let mut g = self.data.lock().unwrap();
            ...
            let tbl = tbl_part.split_whitespace().next().unwrap_or("").trim_end_matches(';').to_string();
            if !tbl.is_empty() {
    ...
    fn query(&self, sql: &str, params: &[SqlValue]) -> anyhow::Result<Vec<SqlRow>> {
        let (cols, table, wc) = parse_select(sql).ok_or_else(|| anyhow::anyhow!("SELECT parse fail: {sql}"))?;
        let g = self.data.lock().unwrap();
        let Some(rows) = g.store.get(&table) else { return Ok(vec![]); };
        let mut out = Vec::new();
        for (rcols, rvals) in rows {
            if let Some((wc_col, wc_idx)) = &wc {
                let Some(pos) = rcols.iter().position(|c| c == wc_col) else { continue };
                let want = &params[*wc_idx];
                if &rvals[pos] != want { continue; }
            }
...
/// ---------------- 5 case ----------------
fn arc() -> Arc<dyn PersistenceProvider> { Arc::new(MockPersistence::new()) }
...
fn case2_insert_then_query() {
    let db = arc();
    db.exec("CREATE TABLE flows (id TEXT, name TEXT)", &[]).unwrap();
    db.exec("INSERT INTO flows(id, name) VALUES (?,?)",
...
fn case3_exec_batch_seed() {
    let db = arc();
    db.exec_batch(
        "CREATE TABLE seeds (id INT, val TEXT);"
    ).unwrap();
    db.exec("INSERT INTO seeds(id, val) VALUES (?,?)",
        &[SqlValue::Int(1), SqlValue::Text("a".into())]).unwrap();
    db.exec("INSERT INTO seeds(id, val) VALUES (?,?)",
        &[SqlValue::Int(2), SqlValue::Text("b".into())]).unwrap();
...
fn case4_update_returns_affected() {
    let db = arc();
    db.exec("CREATE TABLE tasks (id TEXT, status TEXT)", &[]).unwrap();
    db.exec("INSERT INTO tasks(id, status) VALUES (?,?)",
        &[SqlValue::Text("t1".into()), SqlValue::Text("todo".into())]).unwrap();
    let n = db.exec("UPDATE tasks SET status = ? WHERE id = ?",
        &[SqlValue::Text("done".into()), SqlValue::Text("t1".into())]).unwrap();
...
fn case5_delete_then_list_empty() {
    let db = arc();
    db.exec("CREATE TABLE docs (id TEXT, title TEXT)", &[]).unwrap();
    db.exec("INSERT INTO docs(id, title) VALUES (?,?)",
        &[SqlValue::Text("d1".into()), SqlValue::Text("x".into())]).unwrap();
    assert_eq!(db.query("SELECT id FROM docs", &[]).unwrap().len(), 1);
    let n = db.exec("DELETE FROM docs WHERE id = ?",
