...
//! - `ExpertConsultant`：单次咨询接口（替代直接依赖 `xuanji_optimize` / `GovernanceReport`）。
...
///
/// 实现可以是：
/// - 真实 xuanji-expert 内置注册表（把 `experts::all_experts()` 转为 `ExpertMeta` 返回）
...
///
/// 真实实现：把 ConsultQuery 映射到 FlowGraph + GovernContext，调用 `xuanji_optimize`，
/// 再把 `GovernanceReport` 归一化为 `ConsultReport`。
/// 下游（hermes-flow-bridge / business-catalog）改为持有 `Arc<dyn ExpertConsultant>`，
/// 即可在测试时替换为 MockExpert，不再需要引入 xuanji-expert 的 full engine 构建开销。
#[async_trait]
...
// 从而无需 `use xuanji_expert::services::{RegistryImpl, ...}` 等具体名字。
// 下游只会 `use xuanji_expert::expert_traits::default_consultant`，走 trait 模块，合规。
// ---------------------------------------------------------------------------
...
/// 默认咨询实现（包装真实 `xuanji_optimize`，同步快路径见 `ExpertServiceImpl::consult_sync`）。
