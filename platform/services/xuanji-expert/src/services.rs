//! Concrete 服务实现：把 xuanji-expert 现有函数引擎包装为对外 domain trait 的实现。
//!
//! DIP 关系：
//! - `RegistryImpl`   impl `ExpertRegistry`   →  包装 `experts::all_experts()`，把内部 Expert 映射到 `ExpertMeta`。
//! - `ExpertServiceImpl` impl `ExpertConsultant` → 包装 `xuanji_optimize()`，把 `GovernanceReport` 归一化为 `ConsultReport`。
...
// ============================================================================
// ExpertServiceImpl：把 xuanji_optimize 包装为 ExpertConsultant trait
// ============================================================================
    ...
    pub fn consult_sync(&self, query: &ConsultQuery) -> Result<ConsultReport> {
        ...
        match flow_opt {
            ...
            Some(flow) => {
                ...
                // 3) 调用璇玑优化
                let rep = xuanji_optimize(&flow, &ctx);
