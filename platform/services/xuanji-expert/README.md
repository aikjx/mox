# xuanji-expert · 双璇玑十四维专家联盟（14 Experts + Verify）
...
pub const ENGINE_NAME: &str = "xuanji::xuanji_expert";
...
| 文件/目录 | 职责 |
|-----------|------|
| `src/lib.rs` | 三常量 + 对外总入口：`XuanjiExpert::evaluate_project` 聚合、harness、pipeline、reconcile 统一导出 |
...
| `src/bin/xuanji.rs` | CLI 二进制 `xuanji` 专家联盟命令行 |
...
## §5 · 跑单测指引
```bash
cargo test -p xuanji-expert
cargo test -p xuanji-expert expert_unit_tests          # 14 专家单元
cargo test -p xuanji-expert end_to_end                 # 端到端评估→裁决→审计
cargo test -p xuanji-expert t8_dip_xuanji_expert_traits # T8 DIP 合规
cargo test -p xuanji-expert t9_deep_chain_p99          # T9 P99 深度链 ≤2s
cargo run -p xuanji-expert --bin xuanji -- eval <dir>  # CLI 评估目录
```
...
```
domain id      : domain-rust-xuanji-expert
engine id      : engine-rust-xuanji-expert
code_graph unit: xuanji-expert
```
