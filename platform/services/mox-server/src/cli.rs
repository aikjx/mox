...
use mox_cloud_drive_volume::{EcProfile, EcManifest, crc64_ecma};
use mox_compliance::legal_hold::LegalHold;
...
use mox_etl_wasm::{PluginKind, PluginRegistry};
use mox_fusion::{TagSet, tag_cdc_graph_stage};
    ...
    match &args.op {
        EtlOp::ListPlugins => {
            ...
            for id in ["md5", "upper"] {
                if state
                    .etl
                    .run_inline_get(id, b"probe", &mox_etl_wasm::EtContext::new("u", "b"))
                    .is_ok()
                {
        ...
        EtlOp::Run { plugin, data } => {
            let ctx = mox_etl_wasm::EtContext::new("cli-user", "cli-bucket");
            let out = state
                .etl
