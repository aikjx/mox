# mox-cloud-drive-master

## Intro
Mox Cloud Drive L4 Master — 云盘控制面 (Control Plane) 服务。
...
- **Volume (mox-cloud-drive-volume)**: 数据面。存真实 chunk 数据，处理读写、自研 RS(2+1) 编码、重建。

## Example 起 3 Volume + Master
```rust,ignore
use mox_cloud_drive_master::*;
use std::sync::Arc;
...
## 扩展点（L5 impl 替换）
- Volume 节点内部 chunk store 可替换 `mox_domain_abstractions::ChunkManagerProvider` 的 Mock 实现，接入真实磁盘/对象存储。
