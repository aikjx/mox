...
//!
//! 在不修改 `xuanji-domain-abstractions` 的前提下（避免 crate 循环依赖），
//! 本模块提供 `StsServiceSm2`，它包装 `xuanji_domain_abstractions::sts_ttl900::StsService`
...
//!                 || user_id.as_bytes()
//! ```
//! 使用 `id = b"xuanji-sts"`（ZA 标识），签名 OS RNG。
//!
//! SM2 hex 签名字段长度固定为 128 字符（r||s 各 32 bytes = 64 hex chars × 2）。

use std::collections::HashMap;

use xuanji_domain_abstractions::sts_ttl900::{
