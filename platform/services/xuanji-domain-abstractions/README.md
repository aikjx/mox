# xuanji-domain-abstractions

## 1. Intro — L5 定位

xuanji-domain-abstractions 是 Xuanji V5 统一算子平台 **AIS L5（纯 trait 定义层）**。
...
`ust
use xuanji_domain_abstractions::{ObjectStorageProvider, MockObjectStorageProvider};
use bytes::Bytes;
...
1. 在独立 crate（比如 xuanji-l4-minio）的 Cargo.toml 加依赖：
   `	oml
   xuanji-domain-abstractions = { path = "../xuanji-domain-abstractions" }
   `
2. 在代码中 #[async_trait] impl ObjectStorageProvider for MinioAdapter { ... }
3. 用 cargo test -p xuanji-l4-minio 跑 L4 自己的集成测试；
