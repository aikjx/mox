...
//! `mox_expert::expert_traits::ExpertConsultant` trait，不依赖 concrete。
//!
...
//!   `use mox_expert::X` 仅允许 X ∈ {expert_traits, types, domain}。

use async_trait::async_trait;
use hermes_flow_bridge::bridge::optimize_session_with;
use hermes_flow_bridge::normalize::ToolCall;
use hermes_flow_bridge::state::{BridgeState, GateState};
use serde_json::json;
...
fn small_graph() -> (std::sync::Arc<BridgeState>, flow_ai::model::FlowGraph) {
...
fn tr_h8_03_default_factory_returns_trait_object() {
    ...
    let c: Arc<dyn ExpertConsultant> = mox_expert::expert_traits::default_consultant();
    ...
    for f in &files {
        ...
        for (idx, line) in content.lines().enumerate() {
            ...
            if !allowed.contains(&mod_name) {
                panic!(
                    "tr_h8_05 FAIL: {}:{} use mox_expert::{}（仅允许 expert_traits/types/domain）;\n原行: {}",
