...
//! `xuanji_expert::expert_traits::ExpertConsultant` trait，不依赖 concrete。
//!
//! tr_h8_01_bridge_state_with_mock：BridgeState 构造时注入 MockHealthy →
//!   optimize_session_with 返回 mock 数据，不触发 xuanji-expert 引擎。
...
//! tr_h8_05_no_xuanji_concrete_in_lib_use：静态扫描 src/，确保 hermes 生产代码
//!   `use xuanji_expert::X` 仅允许 X ∈ {expert_traits, types, domain}。
...
#[test]
fn tr_h8_05_no_xuanji_concrete_in_lib_use() {
    ...
    for f in &files {
        ...
        for (idx, line) in content.lines().enumerate() {
            ...
            if !allowed.contains(&mod_name) {
                panic!(
                    "tr_h8_05 FAIL: {}:{} use xuanji_expert::{}（仅允许 expert_traits/types/domain）;\n原行: {}",
