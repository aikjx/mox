...
> 目标：把已验证的 `flow-ai`（流程图优化内核）+ `mox-expert`（七专家+治理+算法验证网关）
...
**关键约束**：两个中间件都是**同步闭包**。不能在闭包内跑 `mox_optimize`（async + 重计算），
也不能阻塞 agent loop。→ 解法：优化内核作为**独立常驻服务**（`mox-expert` 已有 HTTP 服务），
...
- 调用 `mox-expert` 的 `TopologyGraph::route(query, threshold)` 做 fast-path 点亮
...
impl Plugin for FlowBridgePlugin {
    fn register(&self, ctx: &mut PluginContext) {
        ...
        // 异步侧：另起后台任务把累积图推给 mox-expert 服务做 optimize+verify
    }
}
```
- **算法否决拦截**：`ToolExecutionMiddleware` 中，若 `mox-expert` 服务的 `verify` 结论为
...
name: flow-bridge
version: 0.1.0
description: "注入 flow-ai + mox-expert 流程图/关系网优化内核（璇玑验证）"
author: mox
kind: plugin
dependencies:
  - mox-expert-server   # 独立常驻服务
```
...
3. 回合结束 → 后台 spawn `optimize(session_graph)` → `mox-expert` 服务返回 `VizBundle`：
...
5. `mox.html`（复用 mox-expert 的 Three.js 面板）实时高亮：关键路径金黄、冲突标红、
...
- **Skills**：`hermes-skills` 提炼的技能写入 `mox-expert::TopologyGraph`（六维关系网 `Skill` 实体），
...
- **多模型**：`mox-expert` 的 `ModelRouting`（Light/Standard/Heavy）直接复用 Hermes
  `smart_model_routing` 的逐轮 cheap-route 结论，算力分配一致。

## 七、开发步骤（跟文档逐步，先骨架后接入）

- [x] **Step 1** 新建 crate `hermes-flow-bridge` + `Cargo.toml`（依赖 flow-ai / mox-expert / serde_json / tokio）
...
- [x] **Step 7** `bridge.rs`：后台 optimize 推送 + `verify` 否决拦截（直接复用 `mox_expert::mox_optimize`，1 单测）
...
- [x] **Step 10** 前端实时联动：mox-expert `server.rs` 新增 `POST /api/ingest` + `GET /api/live`（对实时会话图跑 `mox_optimize` 返回带高亮 VizBundle）；`frontend/mox.html` 新增「实时联动」按钮（1.5s 轮询 `/api/live`）。已实测：ingest 接受 FlowGraph → live 返回 gate=Approved、算法验证 5 项、7 专家评分、关键路径/冲突高亮。
- [x] **Step 11** 会话级端到端集成测试 `tests/session_e2e.rs`（6 用例）+ 闭环演示 bin `src/bin/bridge_demo.rs`：模拟 Hermes 多轮工具调用 → 录制 FlowGraph → 复用路由命中 → 后台 `mox_optimize` → 算法网关 → 否决拦截接线。`cargo run -p hermes-flow-bridge --bin bridge_demo` 实测：speedup 2.60×（省 61.5%），复用模板命中，否决位可强制阻断。
...
- `cargo test -p mox-expert`：23 lib + 5 integration 通过；`cargo test -p flow-ai`：50 + 1 通过。
- 璇玑服务 `mox serve --port 3079` 实测：
  - `POST /api/ingest` 接受 `{id,name,nodes[{id,name,kind,tool,accesses[{resource,mode}]}],edges[{from,to,kind}]}` → ok
  - `GET /api/live` 跑完整 `mox_optimize`：gate=Approved，algorithm.vetoed=false（topology/data_dep/conflict 三项阻断检查全过，code_rt 仅非阻断告警），critical_path=2 节点，conflicts=0 阻断，expert_scores 七专家全 1.0/0.8。
- 融合架构闭环：Hermes 工具调用 → bridge 录制为 FlowGraph → 后台调 `mox_optimize` → `verify()` 算法网关 → 否决时 `ToolExecutionMiddleware` 强制拦截。

## 八、优先级铁律（与 mox-expert 一致）
```
