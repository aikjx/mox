...
//!   DIP：中间件（同步）只依赖 trait，不依赖 mox-expert 任何 concrete struct 名字。
...
pub struct BridgeState {
    ...
    /// 默认由 mox_expert::expert_traits::default_consultant() 注入真实引擎；
    /// 测试可替换为 MockExpert。
...
impl BridgeState {
    ...
    pub fn new() -> Arc<Self> {
        Self::with_consultant(mox_expert::expert_traits::default_consultant())
    }
