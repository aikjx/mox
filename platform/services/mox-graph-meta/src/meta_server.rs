...
pub use mox_domain_abstractions::GraphMetaProvider;
...
impl GraphMetaProvider for MetaServer {
    ...
    async fn list_spaces(
        &self,
    ) -> Result<Vec<mox_domain_abstractions::SpaceInfo>, Box<dyn std::error::Error + Send + Sync>>
    {
        let list = MetaServer::list_spaces(self)?;
        Ok(list
            .into_iter()
            .map(|s| mox_domain_abstractions::SpaceInfo {
                name: s.space_id,
    ...
    async fn show_hosts(
        &self,
    ) -> Result<Vec<mox_domain_abstractions::HostInfo>, Box<dyn std::error::Error + Send + Sync>>
    {
        let list = MetaServer::show_hosts(self)?;
        Ok(list
            .into_iter()
            .map(|h| mox_domain_abstractions::HostInfo {
    ...
    async fn list_tags(
        &self,
        space: &str,
    ) -> Result<Vec<mox_domain_abstractions::TagDef>, Box<dyn std::error::Error + Send + Sync>>
    {
        let list = MetaServer::list_tags(self, space)?;
        Ok(list
            .into_iter()
            .map(|t| mox_domain_abstractions::TagDef {
                name: t.tag_name,
                fields: t
                    .fields
                    .into_iter()
    ...
    async fn list_edge_types(
        ...
        Vec<mox_domain_abstractions::EdgeTypeDef>,
        Box<dyn std::error::Error + Send + Sync>,
    > {
        let list = MetaServer::list_edge_types(self, space)?;
        Ok(list
            .into_iter()
            .map(|e| mox_domain_abstractions::EdgeTypeDef {
                name: e.edge_name,
                fields: e
                    .fields
                    .into_iter()
