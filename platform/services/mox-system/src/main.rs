...
//!
//! 运行方式：
//! - `cargo run -p mox-system -- --demo` 运行端到端演示（无需网络）
//! - `cargo run -p mox-system` 启动 HTTP + WebSocket 服务（:3000）
use std::sync::Arc;

use mox_system::config::AppConfig;
use mox_system::model::{InviteInput, Priority, TaskStatus, Tier};
use mox_system::orchestrator::MoxSystem;
use mox_system::server;
...
async fn run_demo() {
    ...
    let (aln, admin, token) = sys
        .bootstrap("量子计算璇玑", "璇玑(管理员)", "admin@mox.io")
        .await
        .expect("bootstrap");
    ...
    sys.perm
        .assign_role(mox_system::rbac::RoleBinding::mox(
            mox_system::rbac::Role::Coordinator,
            &e2.id,
            &aln.id,
        ))
        .await;
...
async fn run_server() {
    ...
    // 内存 / 持久化统一按 12-Factor 配置构建：内存模式同样尊重 MOX_BIND / RATE_LIMIT / CORS
    // （持久化由 config.persist 决定；即使持久化打开失败也回退为内存，但保留同一份运行配置）
    let sys = Arc::new(
        MoxSystem::with_config(config.clone())
            .await
            .expect("按配置构建璇玑系统失败"),
    );
    ...
    if sys.store.mox_count().await == 0 {
        let (aln, _admin, token) = sys
            .bootstrap("默认璇玑", "管理员", "admin@mox.io")
            .await
            .expect("bootstrap");
        println!("璇玑ID: {}", aln.id);
        println!("管理员令牌: {}", token);
