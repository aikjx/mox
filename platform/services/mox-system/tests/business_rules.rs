...
use mox_system::config::{AppConfig, Quotas};
use mox_system::model::{AuditAction, InviteInput, MemberStatus, Priority, TaskStatus, Tier};
use mox_system::orchestrator::MoxSystem;

/// 便捷：组建璇玑并返回 (系统, 璇玑ID, 管理员ID)
async fn setup(name: &str, email: &str) -> (MoxSystem, String, String) {
    let sys = MoxSystem::new();
...
async fn br04_invite_is_idempotent_per_email() {
    ...
    assert!(
        matches!(again, Err(mox_system::error::AppError::Conflict(_))),
        "同璇玑同 email 重复邀请应返回 Conflict，实际: {again:?}"
    );
    assert_eq!(
        sys.member.list(&aln).await.len(),
        2,
        "重复邀请不得产生新成员"
    );

    // 大小写与首尾空格不敏感
    let mixed = InviteInput {
        email: "  DUP@I.IO ".into(),
        ..input.clone()
    };
    assert!(
        sys.invite_member(&admin, &mixed).await.is_err(),
        "email 比较应忽略大小写与空格"
    );
}
...
async fn br07_assign_validates_assignee_identity() {
    ...
    let r = sys
        ...
        .await;
    assert!(
        matches!(r, Err(mox_system::error::AppError::BadRequest(_))),
    ...
    let r = sys
        ...
        .await;
    assert!(
        matches!(r, Err(mox_system::error::AppError::Forbidden(_))),
    ...
    let r = sys
        ...
        .await;
    assert!(
        matches!(r, Err(mox_system::error::AppError::InvalidState(_))),
        "分派 Invited 成员应 InvalidState，实际: {r:?}"
    );

    // 负向④：分派已停权成员
    sys.member
        .set_status(&expert, MemberStatus::Suspended)
        .await
        .unwrap();
    let r = sys
    ...
    // 负向⑤：名单内重复
    sys.member
        .set_status(&expert, MemberStatus::Active)
        .await
        .unwrap();
    let r = sys
        .assign_task(&admin, &task.id, vec![expert.clone(), expert.clone()])
        .await;
    ...
    assert!(
        !t.assignees.contains(&outsider),
        "他璇玑成员不得出现在分派名单"
    );
    let escalate = sys
        .transition_task(&outsider, &t.id, TaskStatus::InProgress)
        .await;
...
async fn br10_done_gate_blocks_incomplete_work() {
    ...
    // 前置依赖任务（故意不完成）
    let dep = sys
        .create_task(&admin, &aln, "前置任务", "d", Priority::Medium)
        .await
        .unwrap();
    // 主任务
    let main = sys
        .create_task(&admin, &aln, "主任务", "d", Priority::High)
        .await
        .unwrap();
    ...
    // 推到 InReview
    sys.assign_task(&admin, &main.id, vec![expert.clone()])
        .await
        .unwrap();
    sys.transition_task(&admin, &main.id, TaskStatus::InProgress)
        .await
        .unwrap();
    sys.transition_task(&admin, &main.id, TaskStatus::InReview)
        .await
        .unwrap();
    ...
    // 完成前置依赖
    sys.assign_task(&admin, &dep.id, vec![expert.clone()])
        .await
        .unwrap();
    sys.transition_task(&admin, &dep.id, TaskStatus::InProgress)
        .await
        .unwrap();
    sys.transition_task(&admin, &dep.id, TaskStatus::InReview)
        .await
        .unwrap();
    sys.transition_task(&admin, &dep.id, TaskStatus::Done)
        .await
        .unwrap();
...
async fn br11_dependency_graph_is_dag() {
    ...
    let t1 = sys
        .create_task(&admin, &aln, "T1", "d", Priority::Low)
        .await
        .unwrap();
    let t2 = sys
        ...
        .unwrap();
    let t3 = sys
        .create_task(&admin, &aln, "T3", "d", Priority::Low)
        .await
        .unwrap();

    // 负向①：自依赖
    assert!(
        sys.task.add_dependency(&t1.id, &t1.id).await.is_err(),
        "不得自依赖"
    );

    // 负向②：依赖不存在的任务
    assert!(
        sys.task.add_dependency(&t1.id, "task_ghost").await.is_err(),
        "不得依赖不存在任务"
    );
    ...
    assert!(
        sys.task.add_dependency(&t1.id, &t2.id).await.is_err(),
        "不得形成直接环"
    );
    ...
    let other = sys2
        .create_task(&admin2, &aln2, "外部任务", "d", Priority::Low)
        .await
        .unwrap();
...
async fn br12_terminal_task_status_cannot_transition_out() {
    ...
    let t = sys
        .create_task(&admin, &aln, "T", "d", Priority::Low)
        .await
        .unwrap();
    // Draft → Cancelled（终态）
    let t = sys
        .transition_task(&admin, &t.id, TaskStatus::Cancelled)
        .await
        .unwrap();
    ...
    for to in [
        ...
        assert!(
            sys.transition_task(&admin, &t.id, to).await.is_err(),
            "终态 Cancelled 不得迁出到 {}",
            to.label()
        );
    }
...
async fn br21_member_status_machine_enforced() {
    ...
    assert_eq!(
        sys.member.activate(&m.id, &admin).await.unwrap().status,
        MemberStatus::Active
    );
    assert_eq!(
        sys.member
            .set_status(&m.id, MemberStatus::Suspended)
            .await
            .unwrap()
            .status,
    ...
    assert_eq!(
        sys.member
            .set_status(&m.id, MemberStatus::Active)
            .await
            .unwrap()
            .status,
        MemberStatus::Active
    ...
    assert_eq!(
        sys.member
            .set_status(&m.id, MemberStatus::Left)
            .await
            .unwrap()
            .status,
        MemberStatus::Left
...
async fn br18_authz_denial_is_audited() {
    ...
    let t = sys
        .create_task(&admin, &aln, "T", "d", Priority::High)
        .await
        .unwrap();
    let t = sys
        .assign_task(&admin, &t.id, vec![e1.clone()])
        .await
        .unwrap();

    let before = sys
        .store
        .list_audit_by_action(AuditAction::AuthzDenied)
        .await
        .len();

    // 越权尝试：e2 未被分派，试图推进 e1 的任务
    let denied = sys
        .transition_task(&e2, &t.id, TaskStatus::InProgress)
        .await;
    ...
    let n = sys
        ...
        .await
        .len();
    sys.transition_task(&e1, &t.id, TaskStatus::InProgress)
        .await
        .unwrap();
    assert_eq!(
        sys.store
    ...
    assert!(
        !sys.store
            .list_audit_by_action(AuditAction::TaskStatusChange)
            .await
            .is_empty(),
        "任务状态迁移应留痕"
    );
    assert!(
        !sys.store
            .list_audit_by_action(AuditAction::TaskAssign)
            .await
            .is_empty(),
        "任务分派应留痕"
    );
    assert!(
        !sys.store
            .list_audit_by_action(AuditAction::MemberStatusChange)
            .await
            .is_empty(),
        "成员状态迁移应留痕"
    );
    ...
    assert!(
        sys.list_audit(&e1, &aln).await.is_err(),
        "专家无 audit:view，不可查审计"
    );
}

/// 便捷：以指定配额配置组建璇玑，返回 (系统, 璇玑ID, 管理员ID)
async fn setup_with(name: &str, email: &str, quotas: Quotas) -> (MoxSystem, String, String) {
    let cfg = AppConfig {
        quotas,
        ..AppConfig::default()
    };
    let sys = MoxSystem::with_config(cfg).await.unwrap();
    let (aln, admin, _tok) = sys.bootstrap(name, "管理员", email).await.unwrap();
    (sys, aln.id, admin.id)
}
...
async fn nfr09_max_members_enforced() {
    ...
    let r = sys
        ...
        .await;
    assert!(
        matches!(r, Err(mox_system::error::AppError::Conflict(_))),
