...
use std::sync::Arc;
use mox_system::config::{AppConfig, Backend};
use mox_system::event::DomainEvent;
use mox_system::model::{InviteInput, Priority, TaskStatus, Tier};
use mox_system::orchestrator::{Reactor, MoxSystem};
use mox_system::rbac::{Permission, ResourceCtx, Role, RoleBinding, Scope};
use mox_system::store::Store;

/// 分配一个临时 SQLite 数据库路径。
///
/// 关键点：同一测试二进制在多个线程并行执行，若仅用 `process::id()` 作目录名，
/// 所有调用会共享同一个 `mox.db` 文件，导致并发 `remove_dir_all` / `Connection::open`
...
fn temp_db() -> String {
    ...
    let dir = std::env::temp_dir().join(format!("mox_it_{}_{}", std::process::id(), n));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
    dir.join("mox.db").to_string_lossy().into_owned()
}
...
async fn full_lifecycle_and_rbac() {
    ...
    let e1 = sys
        ...
        .await
        .unwrap();
    assert_eq!(e1.status, mox_system::model::MemberStatus::Invited);
    let e1 = sys.member.activate(&e1.id, &admin.id).await.unwrap();
    assert_eq!(e1.status, mox_system::model::MemberStatus::Active);
    ...
    assert!(sys
        ...
        .await
        .is_ok());
    let task_res = mox_system::rbac::TaskResource {
        id: t.id.clone(),
        mox_id: aln.id.clone(),
    ...
    assert!(sys
        ...
        .is_ok());
    // 专家对未分派的任务不可编辑
    let other = mox_system::rbac::TaskResource {
        id: "other".into(),
        mox_id: aln.id.clone(),
...
async fn role_inheritance_and_scope() {
    ...
    let other = "aln_other";
    let ctx_other = ResourceCtx {
        mox_id: other.to_string(),
        task: None,
    };
    let scoped = RoleBinding {
        member_id: m.id.clone(),
        role: Role::Coordinator,
        scope: Scope::Mox(other.to_string()),
    };
...
async fn persistence_survives_restart() {
    ...
    let member_id;
    {
        let sys = Arc::new(MoxSystem::with_config(cfg.clone()).await.unwrap());
    ...
    // 重启：从 SQLite 重放
    let sys2 = Arc::new(MoxSystem::with_config(cfg).await.unwrap());
    assert!(
...
async fn quota_enforcement() {
    ...
    let sys2 = MoxSystem::with_config(cfg2).await.unwrap();
...
// ---------------- PostgreSQL 端到端（可选） ----------------
// 需设置环境变量 MOX_TEST_PG_URL（如 postgres://postgres:pass@127.0.0.1:35432/mox_test），
// 未设置时测试自动跳过（便于无 PG 环境的 CI）。

fn pg_test_url() -> Option<String> {
    std::env::var("MOX_TEST_PG_URL").ok()
}

#[tokio::test]
async fn postgres_store_persists_and_replays() {
    let Some(url) = pg_test_url() else {
        eprintln!("SKIP postgres_store_persists_and_replays: 未设置 MOX_TEST_PG_URL");
        return;
    };
...
async fn postgres_system_survives_restart() {
    let Some(url) = pg_test_url() else {
        eprintln!("SKIP postgres_system_survives_restart: 未设置 MOX_TEST_PG_URL");
        return;
    };
    let cfg = AppConfig {
        persist: true,
