...
impl Metrics {
    ...
    pub fn render_prometheus(&self) -> String {
        ...
        s.push_str("# HELP xuanji_requests_total 总 HTTP 请求数\n");
        s.push_str("# TYPE xuanji_requests_total counter\n");
        s.push_str(&format!("xuanji_requests_total {}\n", req));

        s.push_str("# HELP xuanji_errors_total 4xx/5xx 响应数\n");
        s.push_str("# TYPE xuanji_errors_total counter\n");
        s.push_str(&format!("xuanji_errors_total {}\n", err));

        s.push_str("# HELP xuanji_events_published_total 领域事件发布总数\n");
        s.push_str("# TYPE xuanji_events_published_total counter\n");
        s.push_str(&format!("xuanji_events_published_total {}\n", ev));

        s.push_str("# HELP xuanji_audit_records_total 审计记录总数\n");
        s.push_str("# TYPE xuanji_audit_records_total counter\n");
        s.push_str(&format!("xuanji_audit_records_total {}\n", aud));

        s.push_str("# HELP xuanji_notifications_total 生成的通知总数\n");
        s.push_str("# TYPE xuanji_notifications_total counter\n");
        s.push_str(&format!("xuanji_notifications_total {}\n", ntfs));

        s.push_str("# HELP xuanji_active_members 当前活跃成员数（瞬时量）\n");
        s.push_str("# TYPE xuanji_active_members gauge\n");
        s.push_str(&format!("xuanji_active_members {}\n", active));

        s.push_str("# HELP xuanji_http_latency_avg_ms 平均 HTTP 延迟（毫秒）\n");
        s.push_str("# TYPE xuanji_http_latency_avg_ms gauge\n");
        s.push_str(&format!("xuanji_http_latency_avg_ms {}\n", avg));
        s
    }
}
