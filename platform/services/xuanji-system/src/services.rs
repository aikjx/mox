...
impl MemberService {
    ...
    pub async fn invite(
        ...
        if count >= self.config.quotas.max_members {
            return Err(AppError::Conflict(format!(
                "璇玑 {} 已达成员上限（{}）",
                input.xuanji_id, self.config.quotas.max_members
            )));
        }
        ...
        let m = Member {
            id: new_id("mem"),
            xuanji_id: input.xuanji_id.clone(),
        ...
        bindings.push(RoleBinding::xuanji(Role::Expert, &m.id, &input.xuanji_id));
    ...
    pub async fn set_status(&self, member_id: &str, status: MemberStatus) -> Result<Member> {
        ...
        self.store
            .append_audit(AuditRecord {
                ...
                permission: None,
                scope: format!("xuanji={}", updated.xuanji_id),
    ...
    pub async fn list(&self, xuanji_id: &str) -> Vec<Member> {
        self.store.list_members(xuanji_id).await
    }

    pub async fn search(&self, xuanji_id: &str, expertise: &str) -> Vec<Member> {
        self.store
            .list_members(xuanji_id)
            .await
            .into_iter()
            .filter(|m| {
                m.expertise
...
impl TaskService {
    ...
    pub async fn create(
        ...
        if self.store.get_xuanji(xuanji_id).await.is_none() {
            return Err(AppError::BadRequest(format!("璇玑 {xuanji_id} 不存在")));
        }
        // 配额（I-03）：单璇玑任务数上限
        let count = self.store.list_tasks(xuanji_id).await.len();
    ...
    pub async fn assign(&self, task_id: &str, by: &str, assignees: Vec<String>) -> Result<Task> {
        ...
        let updated = self
            ...
            .ok_or_else(|| AppError::NotFound(format!("任务 {task_id}")))?;
        // 确保每位被分派者进入任务频道
        let ch = self.store.task_channel(&updated.xuanji_id, task_id).await;
        for a in &assignees {
    ...
    async fn validate_assignees(&self, task: &Task, assignees: &[String]) -> Result<()> {
        ...
        for a in assignees {
            ...
            if m.xuanji_id != task.xuanji_id {
                return Err(AppError::Forbidden(format!(
                    "跨璇玑分派被拒：成员 {} 属于璇玑 {}，任务属于璇玑 {}",
                    a, m.xuanji_id, task.xuanji_id
                )));
            }
    ...
    pub async fn watch(&self, task_id: &str, watcher_id: &str) -> Result<Task> {
        ...
        if m.xuanji_id != before.xuanji_id {
            return Err(AppError::Forbidden(format!(
                "跨璇玑关注被拒：成员 {} 属于璇玑 {}，任务属于璇玑 {}",
                watcher_id, m.xuanji_id, before.xuanji_id
            )));
        }
    ...
    pub async fn add_dependency(&self, task_id: &str, dep_id: &str) -> Result<Task> {
        ...
        if dep.xuanji_id != task.xuanji_id {
            return Err(AppError::Forbidden(format!(
                "跨璇玑依赖被拒：任务属于璇玑 {}，依赖任务属于璇玑 {}",
                task.xuanji_id, dep.xuanji_id
            )));
        }
    ...
    pub async fn list(&self, xuanji_id: &str) -> Vec<Task> {
        self.store.list_tasks(xuanji_id).await
    }
}
