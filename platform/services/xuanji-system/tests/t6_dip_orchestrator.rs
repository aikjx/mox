...
//! TR-06-04 (rule)：XuanjiSystem 结构体 4 个服务字段为 trait object（非 concrete struct）。
...
mod t6_dip_tests {
    ...
    // ========================================================================
    // TR-06-04：XuanjiSystem 4 字段必须是 trait object（dyn XxxTrait）
    // ========================================================================
    #[test]
    fn tr_06_04_xuanji_fields_are_dyn_traits() {
        let code = fs::read_to_string(ORCH_PATH).expect("orchestrator.rs readable");
        // 找到 `pub struct XuanjiSystem {` 到匹配 `}` 之间的字段声明区
        let struct_start = code
            .find("pub struct XuanjiSystem")
            .expect("XuanjiSystem struct must exist");
        ...
        for dyn_name in required_dyn.iter() {
            assert!(
                ...
                "TR-06-04 FAIL: XuanjiSystem 字段区未包含 `{}`；当前字段声明：\n{}",
                dyn_name,
                fields
            );
        }
        ...
        for fc in forbidden_concrete.iter() {
            assert!(
                !fields.contains(fc),
                "TR-06-04 FAIL: XuanjiSystem 仍包含 concrete 字段 `{}`（应为 Box/Arc<dyn Trait>）",
                fc
            );
        }
    }
    ...
    impl MemberServiceTrait for MockMember {
        ...
        async fn get(&self, id: &str) -> Result<Member> {
            Ok(Member {
                id: id.into(),
                xuanji_id: "mock_x".into(),
                name: format!("Member-{id}"),
                email: format!("{id}@xuanji.local"),
    ...
    async fn tr_06_10_task_and_comm_mocks_independent() {
        ...
        assert_eq!(t.xuanji_id, "x"); // Mock 硬编码
