...
use xuanji_system::model::{AuditAction, InviteInput, MemberStatus, Priority, TaskStatus, Tier};
...
async fn br04_invite_is_idempotent_per_email() {
    ...
    assert!(
        matches!(again, Err(xuanji_system::error::AppError::Conflict(_))),
...
async fn br07_assign_validates_assignee_identity() {
    ...
    let r = sys
        ...
        .await;
    assert!(
        matches!(r, Err(xuanji_system::error::AppError::BadRequest(_))),
    ...
    let r = sys
        ...
        .await;
    assert!(
        matches!(r, Err(xuanji_system::error::AppError::Forbidden(_))),
    ...
    let r = sys
        ...
        .await;
    assert!(
        matches!(r, Err(xuanji_system::error::AppError::InvalidState(_))),
...
async fn setup_with(name: &str, email: &str, quotas: Quotas) -> (XuanjiSystem, String, String) {
    let cfg = AppConfig {
        quotas,
        ..AppConfig::default()
    };
...
async fn nfr09_max_members_enforced() {
    ...
    let r = sys
        ...
        .await;
    assert!(
        matches!(r, Err(xuanji_system::error::AppError::Conflict(_))),
