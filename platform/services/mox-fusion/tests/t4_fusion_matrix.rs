...
use rand::Rng;
use mox_fusion::audit_sync::{AuditChain, AuditRecordKind};
use mox_fusion::cdc_stage::tag_cdc_graph_stage;
use mox_fusion::graph_writer::{self, GraphWriter};
use mox_fusion::tag_parser::{Tag, TagSet};
...
fn put_via_pipeline(
...
) -> std::result::Result<(), mox_fusion::graph_writer::Error> {
    let ct = headers
        .iter()
    ...
    if alarm.as_deref() == Some("truncated") {
        use mox_fusion::audit_sync::AuditEvent;
