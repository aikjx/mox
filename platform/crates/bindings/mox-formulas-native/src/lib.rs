...
fn rank_to_map(csr: &mox_formulas_core::CsrGraph, r: &[f64]) -> StdMap<String, f64> {
    let n = csr.n;
...
pub fn formulas_density(env: Env, input: JsUnknown) -> Result<serde_json::Value> {
    ...
    let d = if directed {
        ...
        DensityView {
            ...
            interpretation: mox_formulas_core::density_interpretation(v).to_string(),
        }
    } else {
