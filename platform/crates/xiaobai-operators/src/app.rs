...
use xiaobai_core::errors::XiaobaiResult;
...
impl AppOperator {
   pub(crate) fn open_app_impl(&self, app_name: &str) -> Result<(Vec<&'static str>, String), xiaobai_core::XiaobaiError> {
       ...
       Err(xiaobai_core::XiaobaiError::OperatorUnsupported {
   ...
   pub(crate) fn list_running_impl(&self) -> Result<(Vec<&'static str>, Vec<BTreeMap<String, String>>), xiaobai_core::XiaobaiError> {
       let mut fallbacks = Vec::new();
       let rows = if cfg!(windows) {
           let (stdout, _, _) = run_command("tasklist", &["/FO", "CSV", "/NH"]).map_err(|e| {
               xiaobai_core::XiaobaiError::ExecutionError {
   ...
   pub(crate) fn close_app_impl(&self, app_name: &str) -> Result<(Vec<&'static str>, String), xiaobai_core::XiaobaiError> {
       let exec_name = normalize_app_exec(app_name);
       let mut fallbacks = Vec::new();
       ...
       if code != 0 {
           return Err(xiaobai_core::XiaobaiError::ExecutionError {
               category: OperatorCategory::App.as_str().into(),
               action: "close_app".into(),
...
impl SystemOperator for AppOperator {
   ...
   async fn execute(
       ...
       match action {
           ...
           "open_app" => {
               let name = param.get_str("app_name").ok_or_else(|| xiaobai_core::XiaobaiError::InvalidArgument {
                   action: "open_app".into(),
                   param: "app_name".into(),
                   value: "<missing>".into(),
           ...
           "open_file_with_app" => {
               let path = param.get_str("path").ok_or_else(|| xiaobai_core::XiaobaiError::InvalidArgument {
           ...
           "shell_exec" => {
               let cmd = param.get_str("command").ok_or_else(|| xiaobai_core::XiaobaiError::InvalidArgument {
               ...
               let (stdout, stderr, code) = run_command(cmd, &args_refs).map_err(|e| xiaobai_core::XiaobaiError::ExecutionError {
                   category: OperatorCategory::App.as_str().into(),
                   action: "shell_exec".into(),
                   detail: format!("{cmd} {args_refs:?} failed: {e}"),
               })?;
           ...
           other => Err(xiaobai_core::XiaobaiError::IntentUnknown(other.into())),
       }
   }
}
// ============ Helpers ============
