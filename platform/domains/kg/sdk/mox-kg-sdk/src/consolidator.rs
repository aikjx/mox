...
//! 安全接入。计数是候选信息量；没有存储适配器时，成功标志必须保持 false，不能冒充持久化。
...
/// 后续可扩展为持有 a knowledge-store adapter 引用以实现真正的图谱写入。
...
impl TraceConsolidator {
    ...
    pub fn consolidate(&self, trace: &EngineTrace) -> ConsolidationResult {
        ...
        let summary = format!(
            "候选情景 {} 条 | 候选图谱 {} 实体 / {} 关系 | 候选程序性 {} 条（尚未写入知识库）",
...
mod tests {
    ...
    #[test]
    fn empty_trace_does_not_claim_persistence() {
