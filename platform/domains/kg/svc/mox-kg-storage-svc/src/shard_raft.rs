...
//! 所有写入操作必须经过 Raft apply 后才能生效，保证单分片内状态机顺序一致。
//! 注意：当前为单进程最小实现（无选举/无网络复制），持久性依赖底层
//! RocksDB WAL 的 ack 前 fsync（见 `kv_engine::wal_sync_enabled`），而非副本多数派。
//!
//! ## 快照机制
//!
