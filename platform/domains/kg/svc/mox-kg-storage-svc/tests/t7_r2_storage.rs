...
fn tr7_6_cdc_batch_aggregation_gathers_flush() {
    ...
    for i in 0..50 {
        cdc.emit(
            "default",
            mox_kg_storage_svc::CdcEventType::EdgeCreated,
            format!("{{\"i\":{i}}}"),
        );
    }
...
fn hot_cache_case(total: usize, cap: usize) {
    ...
    let fresh = mox_kg_storage_svc::HotNeighborCache::new(cap);
...
impl ReadVertexPubHelper
    for mox_kg_storage_svc::storage_api::LruCache<
        String,
        Vec<mox_kg_storage_svc::storage_api::Neighbor>,
    >
{
