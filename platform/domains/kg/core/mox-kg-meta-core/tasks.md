# T5 关系图 R1 Meta Service — 交付任务追溯（mox-graph-meta）

> 对应交付点：T5 关图 R1 Meta Service。
> crate：`mox-graph-meta`（L4 服务层，`platform/services/mox-graph-meta/`）
>
...
- 测试总数：**25 / 25**
- GREEN 结果：**25 passed，0 failed，0 ignored**（`cargo test -p mox-graph-meta --test t5_r1_meta_raft`）
- RED 阶段锚定：`cfg(red_phase)` 打开时 25/25 全部强制失败。
- `cargo check -p mox-graph-meta --tests`：0 errors。
...
cargo test -p mox-graph-meta --test t5_r1_meta_raft --features red_phase

# GREEN 阶段（主交付验证）
cargo test -p mox-graph-meta --test t5_r1_meta_raft -- --test-threads=1

# 编译级健康度
cargo check -p mox-graph-meta --tests

# 可选：启用 rocksdb 持久化快照
cargo test -p mox-graph-meta --features persist-rocksdb --test t5_r1_meta_raft -- --test-threads=1
```
