# mox-graph-meta（T5 关系图 R1 Meta Service）

L4 服务层实现：3 节点 Raft 共识 + 图元数据（Schema / 权限 / 分区路由）
状态机，对接 L5 `mox-domain-abstractions` 中定义的 `GraphMetaProvider` 语义。
...
- **GREEN 阶段**：`cargo test -p mox-graph-meta --test t5_r1_meta_raft -- --test-threads=1`
  - 25 passed, 0 failed。
...
```sh
# 基础（无 rocksdb 持久化）
cargo test -p mox-graph-meta --test t5_r1_meta_raft

# 启用 rocksdb 持久化快照
cargo test -p mox-graph-meta --features persist-rocksdb --test t5_r1_meta_raft
```

## 与 L5 对齐（`mox-domain-abstractions::GraphMetaProvider`）
