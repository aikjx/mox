...
use mox_kg_algo_core::{Community, KnowledgeEdge, KnowledgeGraph, KnowledgeNode};
...
fn datasets() -> HashMap<&'static str, KnowledgeGraph> {
    ...
    for i in 0..5 {
        for j in 0..5 {
            if i != j {
                k5_edges.push((nodes5[i], nodes5[j]));
            }
        }
    }
    ...
    for i in 1..=3 {
        for j in 1..=3 {
            if i < 3 {
                grid_edges.push((g(i, j), g(i + 1, j)));
                grid_edges.push((g(i + 1, j), g(i, j)));
            }
            if j < 3 {
                grid_edges.push((g(i, j), g(i, j + 1)));
                grid_edges.push((g(i, j + 1), g(i, j)));
            }
        }
    }
    ...
    let mut ln = vec!["hub"];
    for i in 1..=10 {
        ln.push(Box::leak(format!("u{i}").into_boxed_str()));
    }
    let mut tail = vec![];
    for i in 1..=10 {
        tail.push(("hub", ln[i]));
        tail.push((ln[i], "hub"));
        if i > 1 {
            // 幂律：部分 hubs 之间还有 1-2 连接，长尾不互连
            tail.push((ln[i - 1], ln[i]));
        }
    }
    m.insert("T7", mk(&ln, &tail));
    // T8 加权环（8 节点双向环；权重仍 1.0，用于社区 4 vs 4 划分的 modularity 测试）
    let ring: Vec<&str> = (1..=8)
        .map(|i| Box::leak(format!("r{i}").into_boxed_str()) as &str)
        .collect();
    ...
    ring_edges.push(("r1", "r3"));
    ring_edges.push(("r3", "r1"));
    ring_edges.push(("r5", "r7"));
    ring_edges.push(("r7", "r5"));
    m.insert("T8", {
        let mut g = KnowledgeGraph::new();
        for v in &ring {
            g.add_node(KnowledgeNode {
...
fn main() -> Result<()> {
    ...
    for a in std::env::args().skip(1) {
        if let Some(v) = a.strip_prefix("iterations=") {
            iterations = v.parse().unwrap_or(20);
        }
        ...
        for &algo in ALGORITHMS {
            let rec = match algo {
                ...
                "modularity" => {
                    ...
                    Record {
                        dataset: d.to_string(),
                        algorithm: "modularity".to_string(),
                        primary_impl: "RUST",
                        params: serde_json::json!({}),
                        result: serde_json::json!({
                            "modularity": q,
