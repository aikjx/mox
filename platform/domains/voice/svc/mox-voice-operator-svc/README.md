...
- **30010 端口服务**（feature `voice-server`）：完整 HTTP 服务、路由构建与阻塞运行
...
- `voice-server` — 启用 30010 端口 HTTP 服务
...
启动 HTTP 服务（需 feature `voice-server`）：
...
### 30010 服务（feature = "voice-server"）
- `voice_server::XiaobaiVoiceService` — 小白语音服务
- `voice_server::VoiceServiceConfig` — 服务配置
- `voice_server::serve` — 启动服务（async）
- `voice_server::build_router` — 构建 Axum 路由
- `voice_server::run_service_blocking` — 阻塞式运行服务
