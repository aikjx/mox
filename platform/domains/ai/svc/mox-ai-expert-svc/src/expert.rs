...
pub enum Constraint {
    ...
    Compliance(PolicyId),
    /// 建议绑定的算力档位
    RouteModel(String, mox_ai_flow_svc::schedule::ModelTier),
}
...
pub struct Risk {
    ...
    /// 由 `mox_optimize` 汇总后并入算法验证否决（algo.vetoed），治理闸门不可覆盖。
...
pub enum Suggestion {
    ...
    Split,
    Merge,
    Offload(mox_ai_flow_svc::schedule::ModelTier),
    Retry,
    Debounce,
}
...
mod tests {
    ...
    use mox_ai_flow_svc::schedule::ModelTier;
