...
//! HTTP 服务：把架构治理报告转为前端可视化 DTO，供 Three.js 力导向图实时联动高亮
//!
...
pub struct OptimizeRequest {
    /// 流程图 JSON（mox_ai_flow_svc::FlowGraph 序列化）
    pub flow: FlowGraph,
    /// 租户 ID（缺省 gov-tenant）
        ...
        .map(|n| {
            ...
            VizFlowNode {
                ...
                injected: n.tags.iter().any(|t| t == "desensitize" || t == "authz")
                    || n.kind == mox_ai_flow_svc::model::NodeKind::Guard,
            }
        })
        .collect();
    ...
    VizBundle {
        ...
        conflicts: opt
            ...
            .map(|c| VizConflict {
                ...
                blocking: matches!(c.severity, mox_ai_flow_svc::model::Severity::Blocking),
                nodes: c.nodes.clone(),
                message: c.message.clone(),
                remedy: c.remedy.as_ref().map(|r| format!("{:?}", r)),
            })
            .collect(),
        mermaid: mox_ai_flow_svc::to_mermaid(g),
        summary: opt.summary(),
    }
}
...
/// 独立服务首页：本 crate 作为纯治理库被主后端 mox_platform_orchestrator_svc 调用，独立 `mox serve`
/// 仅作演示。前端主入口是 Vue SPA（`frontend/`，由 mox_platform_orchestrator_svc 托管 `/`），此处不内嵌
/// 已移除的单文件 HTML，改为引导到主前端，避免重复维护两份前端产物。
const INDEX_HTML: &str = r#"<!doctype html>
<html lang="zh-CN">
<head><meta charset="utf-8"><title>璇玑架构治理</title></head>
<body style="font-family:system-ui,sans-serif;padding:2rem">
  <h1>璇玑 · 架构处理工具流程图</h1>
  <p>治理内核已就绪。完整可视化前端由 Vue SPA 提供（由主后端 mox_platform_orchestrator_svc 托管）。</p>
  <p>返回 <a href="/">主控制台</a> 查看架构治理台。</p>
</body>
</html>"#;
    ...
    fn sample_req() -> OptimizeRequest {
        ...
        g.add_node(
            FlowNode::task("read", "读取公民库", ToolKind::Database, 300)
                .with_access(mox_ai_flow_svc::model::Access::read("db:citizen_info"))
                .with_access(mox_ai_flow_svc::model::Access::write("var:citizen")),
        );
        g.add_node(
            FlowNode::task("guard", "脱敏", ToolKind::Compute, 50)
                .with_tag("desensitize")
                .with_access(mox_ai_flow_svc::model::Access::read("var:citizen"))
                .with_access(mox_ai_flow_svc::model::Access::write("var:citizen_safe")),
        );
        ...
        g.add_node(
            FlowNode::task("merge", "汇总", ToolKind::Compute, 100)
                .with_access(mox_ai_flow_svc::model::Access::read("var:citizen_safe")),
        );
        g.add_node(FlowNode::new("e", "结束", NodeKind::End));
