    ...
    fn def_to_graph(name: &str, def: &FlowDef) -> Result<FlowGraph, FlowLoadError> {
        ...
        for rd in &def.rules {
            let sev = match rd.severity.as_str().unwrap_or("warning") {
                "blocking" => mox_ai_flow_svc::model::Severity::Blocking,
                "info" => mox_ai_flow_svc::model::Severity::Info,
                _ => mox_ai_flow_svc::model::Severity::Warning,
            };
            let tool_kinds: Vec<_> = rd
                ...
                .map(|tk| vec![tk])
                .unwrap_or_default();
            graph.rules.push(mox_ai_flow_svc::model::ExpertRule {
                id: rd.id.clone(),
