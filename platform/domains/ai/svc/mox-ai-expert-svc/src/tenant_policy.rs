...
/// 治理 8 闸门标识（GR-STD）：与 `mox_flow_fusion_svc::full_gate` 的 8 闸对齐，
...
fn is_sensitive_write(a: &mox_ai_flow_svc::model::Access) -> bool {
    if !a.mode.writes() {
        return false;
    }
...
mod tests {
    ...
    fn regulated_tenant_blocks_raw_sensitive_write() {
        ...
        let mut graph = mox_ai_flow_svc::model::FlowGraph::new("x", "t");
        let mut n = mox_ai_flow_svc::model::FlowNode::new("n1", "敏感写库", mox_ai_flow_svc::model::NodeKind::Task);
        n.accesses.push(mox_ai_flow_svc::model::Access::write("db:secret"));
        graph.nodes.push(n);
        let opt =
            mox_ai_flow_svc::pipeline::optimize(&graph, &mox_ai_flow_svc::pipeline::OptimizeConfig::default());
        let gates = evaluate_gates(
            &ctx,
            &opt,
            crate::govern::FlowStatus::Approved,
    ...
    fn dual_acceptance_requires_both_sides() {
        ...
        let ctx = GovernContext::new(g, Principal::new("u"));
        let rep = mox_optimize(&mox_ai_flow_svc::model::FlowGraph::new("x", "t"), &ctx);
