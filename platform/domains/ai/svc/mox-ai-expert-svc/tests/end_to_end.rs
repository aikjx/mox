...
use mox_ai_flow_svc::model::{Access, EdgeKind, FlowEdge, FlowNode, NodeKind, ToolKind};
use mox_ai_expert_svc::context::{GovernContext, LoopGuard, LoopPolicy, McpTool, Principal, Tenant};
use mox_ai_expert_svc::govern::AuditChain;
use mox_ai_expert_svc::ir::auto_dimension;
use mox_ai_expert_svc::pipeline::mox_optimize;
use mox_ai_expert_svc::reconcile::reconcile;
...
fn gov_flow() -> FlowGraph {
    ...
    g.pools.push(mox_ai_flow_svc::model::ResourcePool {
        name: "browser".into(),
        capacity: 1,
    });
    g
...
#[test]
fn mox_optimize_passes_and_audit_clean() {
    let g = gov_flow();
    ...
    ctx.registry
        .register_skill(mox_ai_expert_svc::context::SkillRef {
            id: "rpa-citizen".into(),
...
fn browser_mutex_injected_from_resource_conflict() {
    ...
    let experts = mox_ai_expert_svc::experts::all_experts();
    let tenant = Tenant::new("t", "ns").with_pool("browser", 1);
    let principal = Principal::new("admin").with_roles(vec!["admin".into(), "editor".into()]);
    let ctx = GovernContext::new(tenant, principal);
    let ectx = mox_ai_expert_svc::context::ExpertContext::new(&df.base, &ctx);
    let opinions = mox_ai_expert_svc::expert::dispatch(&ectx, &experts);
