...
use mox_ai_expert_svc::executor::run_report;
use mox_ai_expert_svc::programming::{programming_pipeline, ProgrammingReport};
...
fn cpm_deep_chain_preserves_real_data_dependency() {
    ...
    // 1000+ 节点的规模/性能由下方 fanout / independent / mox 测试覆盖。
    let n: u32 = 400;
...
#[test]
fn mox_optimize_1000_nodes_scales() {
    ...
    let start = Instant::now();
    let rep = mox_ai_expert_svc::pipeline::mox_optimize(&g, &ctx);
    let elapsed = start.elapsed();
...
fn boundary_empty_graph_no_panic() {
    ...
    // 璇玑层面对空图也不应 panic
    let rep2 = mox_ai_expert_svc::pipeline::mox_optimize(&g, &safe_ctx());
