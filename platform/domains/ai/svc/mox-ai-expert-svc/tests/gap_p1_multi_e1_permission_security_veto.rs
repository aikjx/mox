...
//! 且璇玑编排层（mox_optimize）把 Permission 的 `push_veto` 并入算法验证否决，
//! 整体闸门 BLOCK。
//!
//! 正交性子用例：
...
use mox_ai_flow_svc::model::{Access, AccessMode, FlowEdge, FlowGraph, FlowNode, NodeKind, ToolKind};
use mox_ai_expert_svc::context::{GovernContext, Principal, Tenant};
use mox_ai_expert_svc::expert::dispatch;
use mox_ai_expert_svc::experts::all_experts;
use mox_ai_expert_svc::govern::FlowStatus;
...
fn dual_violation_both_experts_object_and_flow_blocked() {
    ...
    let ectx = mox_ai_expert_svc::context::ExpertContext::new(&g, &ctx);
    ...
    assert!(
        perm.risks
            .iter()
            .any(|r| r.veto && r.severity == mox_ai_flow_svc::model::Severity::Blocking),
    ...
    assert!(
        sec.risks
            .iter()
            .any(|r| r.severity == mox_ai_flow_svc::model::Severity::Blocking),
        "Security 专家应对 regulated 租户 PII 外发给出 Blocking 风险，实际 risks={:?}",
        sec.risks
    );

    // 2) 璇玑编排层：Permission 的 veto 并入算法否决 → 整体 BLOCK
