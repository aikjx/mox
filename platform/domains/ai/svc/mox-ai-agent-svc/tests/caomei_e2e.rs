...
use mox_ai_agent_svc::requirement_compiler::{RequirementCompiler, SystemBlueprint};
use mox_ai_flow_svc::codegen::{generate, generate_full_stack};
use mox_ai_flow_svc::model::{Access, FlowEdge, FlowGraph, FlowNode, NodeKind};
use mox_ai_flow_svc::{conflict, dataflow, schedule};
use serde_json::json;
use mox_market_template_svc::{Domain, SystemTemplate, TemplateMarket};
...
fn blueprint_to_flowgraph(bp: &SystemBlueprint) -> FlowGraph {
    ...
    for n in &bp.flow.nodes {
        ...
        if n.node_type == mox_ai_agent_svc::flow_engine::NodeType::Start
            || n.node_type == mox_ai_agent_svc::flow_engine::NodeType::End
        {
            // 起点/终点不加库访问
        }
        g.add_node(node);
