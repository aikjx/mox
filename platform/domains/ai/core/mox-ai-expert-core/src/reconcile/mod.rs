...
//! 归一化裁决：把十四位专家的观点合并为可喂给 flow-ai 的图
//!
//! 铁律：裁决器只翻译约束为 flow-ai 能识别的边/规则，**不求解**。
//! 硬约束（Blocking / 资源上限）一律落地；冲突按维度优先级仲裁。
//!
//! P2 架构解耦 · 阶段 4：
//! - 约束/观点/维度等领域类型统一使用 proto 定义（SSOT）
//! - 与 flow-svc 交互时通过类型转换函数桥接
...
pub struct ReconciledPlan {
    ...
    pub adopted_suggestions: Vec<Suggestion>,
}
...
pub fn reconcile(
    ...
    for o in opinions {
        ...
        for s in &o.suggestions {
            if matches!(s, Suggestion::Parallelize) {
    ...
    for o in ordered {
        for c in &o.constraints {
            match c {
                ...
                Constraint::MustSerialize(node_ref) => {
                ...
                Constraint::Compliance(pid) => {
                    rules.push(ExpertRule {
                        ...
                        severity: to_flow_severity(mox_ai_expert_proto::Severity::Blocking),
                ...
                Constraint::RouteModel(node, tier) => {
                    model_routes.push((node.clone(), to_flow_model_tier(*tier)));
                }
            }
        }
    }
    ...
    // 采纳建议：与硬串行约束(MustSerialize)语义相反的 Parallelize 不采纳（已记入 semantic 冲突）；
    // 其余（Cache/Offload/Merge 等）一律采纳，交由流水线落地。
    let adopted_suggestions: Vec<Suggestion> = if serialize_nodes.is_empty() {
        all_suggestions
    } else {
        all_suggestions
            .into_iter()
            .filter(|s| !matches!(s, Suggestion::Parallelize))
            .collect()
    };
    ReconciledPlan {
        graph,
...
mod tests {
    ...
    fn mutex_is_hard_not_pruned() {
        ...
        o.constraints
            .push(Constraint::MustSerialize(NodeEdge {
                from: "a".into(),
                to: "b".into(),
            }));
        let plan = reconcile(&[o], &g, &[]);
    ...
    fn same_priority_conflict_escalates() {
        // P2 验收：Permission(100) 与 Security(100) 对同一节点施加**同类别**约束（都 MustGuard，参数互斥）
        ...
        // 安架构维度对同节点也要求 MustGuard（不同脱敏策略）→ 同类别冲突
        s.constraints
    ...
    fn serialize_vs_parallelize_recorded() {
        ...
        algo.suggestions.push(Suggestion::Parallelize);
    ...
    fn non_conflicting_suggestions_adopted() {
        // 与硬约束不冲突的建议（Cache/Merge）应被裁决器采纳进 ReconciledPlan
        let g = base();
        let mut algo = ExpertOpinion::empty("algorithm", Dimension::Algorithm);
        algo.suggestions.push(Suggestion::Cache);
        let mut biz = ExpertOpinion::empty("business", Dimension::Business);
        biz.suggestions.push(Suggestion::Merge);
        let plan = reconcile(&[algo, biz], &g, &[]);
        assert!(
            plan.adopted_suggestions.contains(&Suggestion::Cache),
            "Cache 建议应被采纳"
        );
        assert!(
            plan.adopted_suggestions.contains(&Suggestion::Merge),
            "Merge 建议应被采纳"
        );
    }
    ...
    fn parallelize_not_adopted_when_serialize_conflict() {
        // 存在 MustSerialize 硬串行约束时，语义相反的 Parallelize 建议不应被采纳
        let g = base();
        let mut r = ExpertOpinion::empty("resource", Dimension::Resource);
        r.constraints
            .push(Constraint::MustSerialize(NodeEdge {
        ...
        assert!(
            !plan
                .adopted_suggestions
                .iter()
                .any(|s| matches!(s, Suggestion::Parallelize)),
    ...
    #[test]
    fn route_model_converts_tier() {
        // RouteModel 约束中的 proto::ModelTier 应正确转换为 flow-svc 的 ModelTier
        let g = base();
        let mut o = ExpertOpinion::empty("algorithm", Dimension::Algorithm);
        o.constraints.push(Constraint::RouteModel(
            "a".into(),
            mox_ai_expert_proto::ModelTier::Heavy,
        ));
        let plan = reconcile(&[o], &g, &[]);
        assert_eq!(plan.model_routes.len(), 1);
