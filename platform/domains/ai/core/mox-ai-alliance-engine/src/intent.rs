...
//!
//! # 设计
//! - `IntentClassifier` 为意图分类器结构体，支持可配置的关键词模式
//! - 通过 `KgConnector` trait 注入图谱激活扩散能力
//! - 图谱不可用时自动降级为纯关键词模式（degraded=true）
...
/// 意图分类器
///
/// 封装关键词模式 + 图谱激活扩散，提供意图分类能力。
/// 可独立使用，也可被 AllianceEngine 内部组合。
#[derive(Debug, Clone)]
pub struct IntentClassifier {
    /// 关键词模式列表：(pattern, intent_id, base_weight)
    patterns: Vec<(String, IntentId, f64)>,
}
...
impl IntentClassifier {
    ...
    /// 自定义关键词模式（高级用法）
    pub fn with_patterns(patterns: Vec<(String, IntentId, f64)>) -> Self {
        Self { patterns }
    }

    /// 执行意图分类（同步版本，用于非 async 上下文）
    ///
    /// - `query`: 非空用户输入
    /// - `graph_spread_fn`: 可选的图谱激活扩散函数
    pub fn classify<F>(&self, query: &str, graph_spread_fn: Option<F>) -> IntentResult
    where
        F: FnOnce(&[String], f64, u32) -> Result<BTreeMap<String, f64>, String>,
    {
        classify_intent_inner(query, &self.patterns, graph_spread_fn)
    }

    /// 异步意图分类（使用 KgConnector trait）
    ///
    /// 推荐在 async 上下文中使用此方法。
    pub async fn classify_async<C: ?Sized + crate::kg::KgConnector>(
        &self,
        query: &str,
        kg: Option<&C>,
    ) -> IntentResult {
        ...
        // Stage 1: 关键词匹配
        let (keyword_raw, seeds_hit) = self.match_keywords(&query_lower);

        // Stage 2: 激活扩散（如果 KG 可用）
        let (spread_raw, degraded, degrade_reason) = if let Some(kg) = kg {
            if !kg.is_available() {
                (empty_7class(), true, Some("kg_connector_unavailable".to_string()))
            } else {
                let seeds = build_seeds(query, &keyword_raw);
                match kg.spread(&seeds, SPREAD_DAMPING, SPREAD_ROUNDS).await {
                    Ok(raw) => (normalize_to_7class(&raw), false, None),
                    Err(e) => (empty_7class(), true, Some(format!("spread_error: {}", e))),
                }
            }
        } else {
            (
                empty_7class(),
                true,
                Some("spread_graph_unavailable: no kg connector injected".to_string()),
            )
        };
    ...
    /// 仅关键词匹配（不使用图谱，性能模式）
    pub fn classify_keyword_only(&self, query: &str) -> IntentResult {
        let none_fn: Option<fn(&[String], f64, u32) -> Result<BTreeMap<String, f64>, String>> = None;
        self.classify(query, none_fn)
    }

    // ---- 内部方法 ----

    fn match_keywords(&self, query_lower: &str) -> (BTreeMap<IntentId, f64>, Vec<String>) {
        let mut keyword_raw: BTreeMap<IntentId, f64> = BTreeMap::new();
        let mut seeds_hit = Vec::<String>::new();
        for (pat, intent, base_weight) in &self.patterns {
            let re = regex_lite(pat);
            if re.is_match(query_lower) {
                seeds_hit.push(format!("pattern:{}→{}", truncate(pat, 32), intent));
                let entry = keyword_raw.entry(intent.clone()).or_insert(0.0);
                *entry = (*entry + base_weight).min(1.0);
            }
        }
        // 对 7 类强制补 0（确保 7 键齐全）
    ...
    F: FnOnce(&[String], f64, u32) -> Result<BTreeMap<String, f64>, String>,
{
    ...
    classifier.classify(query, graph_spread_fn)
}

// ================== 内部实现 ==================

fn classify_intent_inner<F>(
    query: &str,
    patterns: &[(String, IntentId, f64)],
    graph_spread_fn: Option<F>,
) -> IntentResult
where
    F: FnOnce(&[String], f64, u32) -> Result<BTreeMap<String, f64>, String>,
{
    ...
    let mut keyword_raw: BTreeMap<IntentId, f64> = BTreeMap::new();
    let mut seeds_hit = Vec::<String>::new();
    for (pat, intent, base_weight) in patterns {
        let re = regex_lite(pat);
        if re.is_match(&query_lower) {
            seeds_hit.push(format!("pattern:{}→{}", truncate(pat, 32), intent));
            let entry = keyword_raw.entry(intent.clone()).or_insert(0.0);
...
fn build_seeds(query: &str, keyword_raw: &BTreeMap<IntentId, f64>) -> Vec<String> {
...
fn empty_7class() -> BTreeMap<IntentId, f64> {
    let mut m = BTreeMap::new();
    for cls in INTENT_CLASSES {
        m.insert(cls.to_string(), 0.0);
    }
    m
}

fn normalize_to_7class(raw: &BTreeMap<String, f64>) -> BTreeMap<IntentId, f64> {
    ...
    for cls in INTENT_CLASSES {
        ...
        out.insert(cls.to_string(), best);
    }
    out
}

fn rrf_fusion(keyword: &BTreeMap<IntentId, f64>, spread: &BTreeMap<IntentId, f64>) -> BTreeMap<IntentId, f64> {
    let rrf_keyword = rrf_scores(keyword, RRF_K as f64);
    let rrf_spread = rrf_scores(spread, RRF_K as f64);
...
fn find_winner(rrf: &BTreeMap<IntentId, f64>) -> (IntentId, f64) {
...
fn compute_confidence(winner: &str, keyword: &BTreeMap<IntentId, f64>, spread: &BTreeMap<IntentId, f64>) -> f64 {
    let kw_conf = keyword.get(winner).copied().unwrap_or(0.0);
    let sp_conf = spread.get(winner).copied().unwrap_or(0.0);
...
/// 极简 RRF：把 {id: raw_score} → {id: rrf_score}
...
/// 默认关键词模式（7 类）
fn default_patterns() -> Vec<(String, IntentId, f64)> {
    vec![
        // --- math ---
...
mod tests {
    ...
    /// IntentClassifier 结构体测试
    #[test]
    fn intent_classifier_struct_works() {
        let clf = IntentClassifier::new();
        let r = clf.classify_keyword_only("写一个 Rust 函数");
        assert_eq!(r.intent_id, "code");
        assert!(r.degraded); // 没传 KG，应降级
    }
