...
//! - **具体实现** [`store_admin::StoreAdmin`] 与 [`store_admin::assemble_backend`]
//!   在 feature `admin` 下启用（装配 `mox-cloud-store-core` 真实后端）。
...
/// store-core 后端的具体管理实现。
#[cfg(feature = "admin")]
pub mod store_admin {
    use super::{GcReport, MigrateReport, StorageAdmin, StorageStats, StorageStatus, VerifyReport};
    use crate::{CloudApiError, CloudApiResult};
    use async_trait::async_trait;
