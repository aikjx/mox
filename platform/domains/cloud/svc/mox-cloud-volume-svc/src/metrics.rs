...
//! - `mox_ec_rebuild_count` (counter)
//! - `mox_ec_shards_lost_total` (counter)
//! - `mox_ec_encode_us` (histogram samples)
//! - `mox_ec_encode_avx2_bytes_total` (counter, feature=simd)
//! - `mox_ec_encode_neon_bytes_total` (counter, feature=simd)
//! - `mox_ec_encode_scalar_bytes_total` (counter)
//! - `mox_ec_decode_scalar_bytes_total` / `_avx2_` / `_neon_` (counter)
...
/// Counter: `mox_ec_rebuild_count` – every successful rebuild job bumps.
pub static REBUILD_COUNT: AtomicU64 = AtomicU64::new(0);

/// Counter: `mox_ec_shards_lost_total` – every detected missing shard.
...
/// Push a latency sample (microseconds) for `mox_ec_encode_us`.
...
mod tests {
    ...
    fn t22_metrics_prometheus_text_contains_avx2_line() {
        ...
        assert!(
            snap.contains("mox_ec_encode_scalar_bytes_total 2048"),
            "snapshot missing scalar: {snap}"
        );
        #[cfg(feature = "simd")]
        assert!(
            snap.contains("mox_ec_encode_avx2_bytes_total 4096"),
            "snapshot missing avx2: {snap}"
        );
        // Standard header present.
        assert!(snap.starts_with("# HELP mox_ec_rebuild_count"));
    }
