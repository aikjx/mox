...
use mox_flow_operator_core::conservation::{
    ConservationChecker, ConservationLaw, GraphNode, GuardedGraph, L1Conservation, L2Conservation,
};
use mox_flow_operator_core::engine::OperatorPipeline;
...
use mox_flow_operator_core::types::{builtin, TypeCheck, TypeIdentifier, TypePair};
use mox_flow_operator_core::{OperatorMetadata, SystemConfig};
...
fn conservation_residual_monitor_convergence() {
    let mut monitor = mox_flow_operator_core::conservation::ResidualMonitor::new(1e-6);
...
fn pipeline_error_propagation() {
    let failing = Arc::new(FunctionOperator::new("fail", |_s, _ctx| {
        Err(mox_flow_operator_core::OperatorError::ExecutionError(
