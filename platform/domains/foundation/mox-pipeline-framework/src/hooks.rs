...
pub mod builtin_hooks {
    ...
    pub fn metrics_hook() -> WaterfallHook {
        Arc::new(|event, ctx, next| {
...
mod tests {
    ...
    #[test]
    fn hook_event_names() {
        assert_eq!(HookEvent::PrePipeline.name(), "pre_pipeline");
        assert_eq!(HookEvent::PostPipeline.name(), "post_pipeline");
        assert_eq!(HookEvent::PrePhase(Phase::Analyze).name(), "pre_analyze");
        assert_eq!(HookEvent::PostPhase(Phase::Gate).name(), "post_gate");
    }
    ...
    fn hook_propagates_error() {
        ...
        let mut ctx = make_ctx();
        let result = chain.run(HookEvent::PrePhase(Phase::Analyze), &mut ctx);
    ...
    fn hook_registry_manages_all_events() {
        ...
        registry.on_pre_phase(Phase::Analyze, builtin_hooks::audit_hook());
        registry.on_post_phase(Phase::Analyze, builtin_hooks::audit_hook());
        ...
        assert!(registry.run_pre_phase(Phase::Analyze, &mut ctx).is_ok());
        assert!(registry.run_post_phase(Phase::Analyze, &mut ctx).is_ok());

        // 未注册钩子的阶段也应返回 Ok
        assert!(registry.run_pre_phase(Phase::Gate, &mut ctx).is_ok());
    }
    ...
    fn audit_hook_records_events() {
        ...
        // pre_phase 钩子应记录开始事件
        registry
            .run_pre_phase(Phase::Analyze, &mut ctx)
            .unwrap();
        let pre_count = ctx.audit.len();

        // post_phase 钩子应记录结束事件
        ctx.mark_phase_end(Phase::Analyze, crate::phase::PhaseStatus::Success, 100);
        registry
            .run_post_phase(Phase::Analyze, &mut ctx)
            .unwrap();
    ...
    fn metrics_hook_stores_latency() {
        let mut registry = HookRegistry::new();
        registry.on_post_phase(Phase::Analyze, builtin_hooks::metrics_hook());

        let mut ctx = make_ctx();
        ctx.mark_phase_start(Phase::Analyze);
        ctx.mark_phase_end(Phase::Analyze, crate::phase::PhaseStatus::Success, 42);
    ...
    fn hook_error_display() {
        let err = HookError::new(HookEvent::PrePipeline, "test error");
        let display = format!("{err}");
        assert!(display.contains("PrePipeline"));
