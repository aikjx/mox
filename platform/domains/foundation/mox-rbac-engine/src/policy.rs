...
impl RbacPolicy {
    ...
    /// 添加一个角色定义
    pub fn add_role(&mut self, role: RoleDef) {
...
mod tests {
    ...
    #[test]
    fn wildcard_match_everything() {
        let p = Permission::new("admin", "*");
        assert!(p.matches("read", "anything/at/all"));
        assert!(p.matches("write", "db:prod/secret"));
    }
    ...
    #[test]
    fn cyclic_inheritance_is_safe() {
        let mut pol = RbacPolicy::new();
        pol.add_role(RoleDef::new("a").extends("b").grant("read", "x"));
        pol.add_role(RoleDef::new("b").extends("a").grant("write", "y"));
        let perms = BuiltinRoles::resolve(&pol, "a");
        // 不应无限递归，且两个角色的权限都应出现
        assert!(perms.iter().any(|p| p.action == "read"));
        assert!(perms.iter().any(|p| p.action == "write"));
    }

    #[test]
    fn policy_builder_pattern() {
        let pol = RbacPolicy::new()
            .with_roles(vec![RoleDef::new("custom").grant("read", "custom/*")]);
        assert!(pol.has_role("custom"));
        assert!(!pol.has_role("nonexistent"));
    }

    #[test]
    fn default_policy_has_all_builtin_roles() {
        let pol = RbacPolicy::default();
        assert!(pol.has_role("admin"));
        assert!(pol.has_role("editor"));
        assert!(pol.has_role("viewer"));
        assert!(pol.has_role("safety_approver"));
        assert!(pol.has_role("operator"));
        assert!(pol.has_role("auditor"));
    }
}
