# mox-system · 璇玑业务 & 基础设施层（AIS L7Infrastructure）
...
pub const ENGINE_NAME: &str = "mox::mox_system";
pub const CRATE_META: mox_common_meta::CrateMeta = mox_common_meta::CrateMeta {
    id: CRATE_ID,
    name: env!("CARGO_PKG_NAME"),
    version: env!("CARGO_PKG_VERSION"),
    layer: mox_common_meta::AisLayer::L7Infrastructure,
    owner: "mox-core",
};
```
...
| `src/persistence_provider.rs` + `src/store.rs` | `trait PersistenceProvider` + 内存态 Store（MOX_PERSIST=false 时） |
| `src/rbac.rs` | RBAC：5 角色 + 14 原子权限 + 作用域 Global/Mox/Task + 所有权 `*Own` 规则 |
...
- **`pub trait PersistenceProvider`**（persistence_provider.rs）：`fn get_repo() -> Box<dyn Repository>`；按 `MOX_DB_BACKEND` 路由。
...
```bash
# 默认 SQLite 内存态
cargo test -p mox-system
# 启用 Postgres 后端（需要本地 PG 连接）
MOX_DB_BACKEND=postgres DATABASE_URL=postgres://... cargo test -p mox-system integration
# MySQL
MOX_DB_BACKEND=mysql DATABASE_URL=mysql://... cargo test -p mox-system
# T6 DIP
cargo test -p mox-system t6_dip_orchestrator
# 运行演示
cargo run -p mox-system -- --demo
```
...
```
domain id      : domain-rust-mox-system
engine id      : engine-rust-mox-system
code_graph unit: mox-system
```
