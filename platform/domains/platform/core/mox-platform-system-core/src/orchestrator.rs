...
//!
//! `MoxSystem` 聚合四个服务，并在每次写操作前做 RBAC 鉴权（require），
...
#[derive(Clone)]
// 说明：struct MoxSystem —— 企业级数据/实现项，按 AIS 契约要求提供幂等接口
// 设计：保持单一职责；相关字段变更需同步修改对应序列化 / 反序列化结构
pub struct MoxSystem {
    pub store: Arc<Store>,
    pub bus: EventBus,
...
// 设计：保持单一职责；相关字段变更需同步修改对应序列化 / 反序列化结构
impl Default for MoxSystem {
    fn default() -> Self {
        Self::new()
    }
}

// 说明：impl MoxSystem —— 企业级数据/实现项，按 AIS 契约要求提供幂等接口
// 设计：保持单一职责；相关字段变更需同步修改对应序列化 / 反序列化结构
impl MoxSystem {
        ...
        match self.perm.authorize(member_id, perm, ctx).await {
            ...
            Err(e) => {
                let scope = match &ctx.task {
                    ...
                    None => format!("mox={}", ctx.mox_id),
                };
                let resource = ctx
                    .task
                    .as_ref()
                    .map(|t| t.id.clone())
                    .unwrap_or_else(|| ctx.mox_id.clone());
    ...
    pub async fn list_audit(&self, actor: &str, mox_id: &str) -> Result<Vec<AuditRecord>> {
        self.require(actor, Permission::AuditView, &self.ctx_mox(mox_id))
            .await?;
        Ok(self.store.list_audit().await)
    }

    fn ctx_mox(&self, mox_id: &str) -> ResourceCtx {
        ResourceCtx {
    ...
    pub async fn bootstrap(
        ...
        admin_name: &str,
        admin_email: &str,
    ) -> Result<(Mox, Member, String)> {
        let mox = Mox {
            id: new_id("aln"),
            name: mox_name.to_string(),
            description: String::new(),
            created_at: chrono::Utc::now(),
        };
        self.store.create_mox(mox.clone()).await;
        let admin = Member {
            id: new_id("mem"),
            mox_id: mox.id.clone(),
        ...
        // 授予全局管理员角色
        self.perm
            .assign_role(RoleBinding::global(Role::MoxAdmin, &admin.id))
            .await;
        // 璇玑大厅频道
        self.store.ensure_mox_channel(&mox.id).await;
        // 签发登录令牌
    ...
    pub async fn create_task(
        ...
        self.require(actor, Permission::TaskCreate, &self.ctx_mox(mox_id))
            .await?;
        self.task
            .create(mox_id, actor, title, description, priority)
            .await
    }
    ...
    pub async fn send_channel_message(
        ...
        let perm = match &ch.kind {
            ChannelKind::Mox => Permission::CommSendMox,
        ...
        match e {
            DomainEvent::MemberInvited {
                ...
                self.comm
                    ...
                    .await;
                self.sys_msg(mox_id, &format!("{name} 邀请了新成员加入璇玑"))
                    .await;
            }
            ...
            DomainEvent::TaskCreated {
                ...
                let name = self.name_of(by).await;
                self.sys_msg(mox_id, &format!("{name} 创建了任务《{title}》"))
                    .await;
            }
