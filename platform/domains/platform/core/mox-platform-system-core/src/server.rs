...
use crate::error::*;
use crate::model::*;
use crate::orchestrator::MoxSystem;
...
pub struct CreateChannelReq {
    mox_id: String,
    kind: String, // "mox" | "task:<id>" | "direct"
    name: String,
    members: Vec<String>,
}
...
async fn rate_limit_mw(State(sys): State<Arc<MoxSystem>>, req: Request, next: Next) -> Response {
...
async fn metrics_mw(State(sys): State<Arc<MoxSystem>>, req: Request, next: Next) -> Response {
...
// ---------------- 鉴权中间件 ----------------
async fn auth_mw(State(sys): State<Arc<MoxSystem>>, mut req: Request, next: Next) -> Response {
    let token = req
        .headers()
        .get("authorization")
        .and_then(|v| v.to_str().ok())
...
async fn list_members(
    ...
    let mox_id = q
        .get("mox_id")
        .cloned()
        .ok_or_else(|| AppError::BadRequest("缺少 mox_id".into()))?;
    sys.require(
        &id,
    ...
    .await?;
    Ok(Json(sys.member.list(&mox_id).await))
}
...
async fn list_tasks(
    ...
    .await?;
    Ok(Json(sys.task.list(&mox_id).await))
}
...
async fn create_channel(
    ...
    let ch = sys
        .comm
        .create_channel(&req.mox_id, kind, &req.name, req.members)
        .await;
    Ok(Json(ch))
}
...
async fn list_channels(
    ...
    .await?;
    Ok(Json(sys.store.list_channels(&mox_id).await))
}
...
async fn grant_role(
    ...
    let scope = match &req.scope {
        ...
        Some(s) if s.starts_with("mox:") => Scope::Mox(s["mox:".len()..].to_string()),
        _ => Scope::Global,
    };
    sys.perm
...
async fn list_audit(
    ...
    let mox_id = q
        ...
        .cloned()
        .ok_or_else(|| AppError::BadRequest("缺少 mox_id".into()))?;
    let mut records = sys.list_audit(&id, &mox_id).await?;
...
async fn health(State(sys): State<Arc<MoxSystem>>) -> Response {
    (
        StatusCode::OK,
        serde_json::json!({
            "status": "ok",
...
async fn metrics_handler(State(sys): State<Arc<MoxSystem>>) -> Response {
    // 动态刷新「活跃成员」瞬时量
    let active = {
...
async fn handle_socket(sys: Arc<MoxSystem>, socket: WebSocket, member_id: String) {
...
// ---------------- 应用装配 ----------------
pub fn app(sys: Arc<MoxSystem>) -> Router {
