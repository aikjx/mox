...
use mox_platform_system_core::config::AppConfig;
...
use mox_platform_system_core::server;
...
async fn run_demo() {
    ...
    sys.perm
        .assign_role(mox_platform_system_core::rbac::RoleBinding::mox(
            mox_platform_system_core::rbac::Role::Coordinator,
            &e2.id,
            &aln.id,
        ))
        .await;
