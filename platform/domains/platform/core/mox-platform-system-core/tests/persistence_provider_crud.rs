...
fn crud_member_1_save() {
    ...
    let n = db
        .exec(
            "INSERT INTO members(id, mox_id, name, email, status) VALUES (?,?,?,?,?)",
            &[
...
fn crud_member_2_get() {
    ...
    let row = db
        .query_one(
            "SELECT id, mox_id, name, email, status FROM members WHERE id = ?",
            &[SqlValue::Text("m1".into())],
        )
        .unwrap()
        .unwrap();
...
fn crud_task_1_save() {
    ...
    let n = db
        .exec(
            "INSERT INTO tasks(id, mox_id, title, status, priority) VALUES (?,?,?,?,?)",
            &[
