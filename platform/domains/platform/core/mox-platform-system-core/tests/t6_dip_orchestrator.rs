...
mod t6_dip_tests {
    ...
    // ========================================================================
    use mox_platform_system_core::domain_traits::{
        CommServiceTrait, MemberServiceTrait, PermissionServiceTrait, TaskServiceTrait,
    };
    use mox_platform_system_core::error::Result;
    use mox_platform_system_core::model::*;
    use mox_platform_system_core::rbac::{Permission, ResourceCtx};
    use mox_platform_system_core::rbac::{Role, RoleBinding};

    struct MockMember;
    #[async_trait::async_trait]
    impl MemberServiceTrait for MockMember {
        async fn invite(&self, _by: &str, _input: &InviteInput) -> Result<Member> {
            Err(mox_platform_system_core::error::AppError::NotFound(
                "MockMember".into(),
            ))
        }
