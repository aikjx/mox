...
use mox_data_catalog_svc::spiral::{analyze_spiral, SpiralParams};
...
struct AppState {
    ...
    // 会话存储
    chat_sessions: Arc<Mutex<HashMap<String, Vec<mox_ai_agent_svc::ChatMessage>>>>,
    // 工作流存储
...
async fn ai_chat(
    ...
    let response = match state.ai_agent.chat(&session_id, &req.message).await {
        Ok(resp) => resp,
        Err(e) => ChatResponse {
            message: mox_ai_agent_svc::ChatMessage::assistant(format!("AI处理错误: {}", e)),
            suggestions: vec![],
    ...
    // 保存会话历史
    {
        ...
        history.push(mox_ai_agent_svc::ChatMessage::user(&req.message));
                        ...
                        Err(e) => {
                            return api_ok(WorkflowResult {
                                instance: mox_ai_agent_svc::WorkflowInstance {
                                    ...
                                    status: mox_ai_agent_svc::WorkflowStatus::Failed,
                                    current_nodes: vec![],
    ...
    match req.method.as_str() {
        ...
        "tools/call" => {
            ...
            if let Some(op_id) = name.strip_prefix("operator_") {
                ...
                let bus = state.ai_agent.plugin_bus();
                let bus_guard = bus.read().await;
                let msg = mox_ai_agent_svc::PluginMessage::new("mcp-client", &message, payload);
                match bus_guard.route_message(msg).await {
                    Ok(Some(r)) => mcp_ok(
                        id,
                        serde_json::json!({
