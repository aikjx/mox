...
/// 架构融合落盘：接收璇玑归一化产出的优化流程图（mox_ai_flow_sdk::FlowNode/FlowEdge）+
...
pub fn publish_unified(
    ...
    description: String,
    requirement: String,
    nodes: Vec<mox_ai_flow_sdk::model::FlowNode>,
    edges: Vec<mox_ai_flow_sdk::model::FlowEdge>,
    tags: Vec<String>,
    // 来源璇玑架构治理报告（I-07 产物来源追溯）
    ...
    let id = gen_id();
    let ts = now_rfc3339();
    // mox_ai_flow_sdk 模型 -> 算子商城展示模型（字段结构不同，做归一化映射）
