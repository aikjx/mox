...
extern crate mox_kg_algo_core as c_graph_algorithms;
extern crate mox_flow_bridge_svc as c_hermes_flow_bridge;
extern crate mox_kg_hub_svc as c_kg_hub;
extern crate mox_flow_operator_core as c_operator_core;
extern crate mox_flow_operator_wasm_svc as c_operator_wasm;
extern crate mox_flow_optimizer_core as c_optimizer;
extern crate mox_flow_primiflow_svc as c_primiflow_core;
extern crate mox_flow_fusion_svc as c_primiflow_fusion;
extern crate mox_platform_orchestrator_svc as c_runtime;
extern crate mox_market_template_svc as c_template_market;
extern crate mox_platform_foundation as c_mox_common_meta;
extern crate mox_ai_expert_svc as c_mox_expert;
extern crate mox_platform_system_core as c_mox_system;
...
fn sixteen_crates_have_valid_crate_id_engine_name_and_crate_meta() {
    // --------- operator-core (L6Kernel) ---------
    {
        ...
        assert!(matches!(m.layer, mox_platform_foundation::AisLayer::L6Kernel));
    }
    ...
    // --------- mox_platform_orchestrator_svc (L3Orchestration) ---------
    {
        ...
        assert!(matches!(
            m.layer,
            mox_platform_foundation::AisLayer::L3Orchestration
        ));
    }
    // --------- mox-system (L7Infrastructure) ---------
    {
        ...
        assert!(matches!(
            m.layer,
            mox_platform_foundation::AisLayer::L7Infrastructure
        ));
    }
    ...
    // --------- mox-common-meta (L5Domain) ---------
    {
        ...
        assert_eq!(m.owner, "mox-core");
        assert!(matches!(m.layer, mox_platform_foundation::AisLayer::L5Domain));
    }
    ...
    let layers: [mox_platform_foundation::AisLayer; EXPECTED_COUNT] = [
    ...
    let l4_count = layers
        .iter()
        .filter(|l| matches!(l, mox_platform_foundation::AisLayer::L4Services))
        .count();
    assert_eq!(
        l4_count, 12,
    ...
    let l6_kernel_count = layers
        .iter()
        .filter(|l| matches!(l, mox_platform_foundation::AisLayer::L6Kernel))
        .count();
    assert_eq!(
        l6_kernel_count, 1,
    ...
    let l7_count = layers
        .iter()
        .filter(|l| matches!(l, mox_platform_foundation::AisLayer::L7Infrastructure))
        .count();
    assert_eq!(
        l7_count, 1,
    ...
    let l5_count = layers
        .iter()
        .filter(|l| matches!(l, mox_platform_foundation::AisLayer::L5Domain))
        .count();
    assert_eq!(
        l5_count, 1,
    ...
    let l3_count = layers
        .iter()
        .filter(|l| matches!(l, mox_platform_foundation::AisLayer::L3Orchestration))
        .count();
    assert_eq!(
        l3_count, 1,
