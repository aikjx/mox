...
//! 璇玑架构融合端到端集成测试（I-05 / I-06 / I-07 闭环验收）
//!
//! 不依赖服务器进程，纯库级串联：
//!   1. 构造业务流程图（受监管租户的真实场景）
//!   2. 走架构治理流水线 `mox_optimize` —— 触发治理 8 闸门（I-06）
//!   3. 双验收联动（I-05）：任务 Done ∧ 算法未否决 ∧ 闸门放行
//!   4. 架构融合落盘 `publish_unified` —— 固化产物溯源（I-07 provenance）
//!
...
fn e2e_governance_eight_gates_pipeline_publish_provenance() {
    ...
    // 1. 架构治理流水线：归一化 → 派发专家 → 裁决 → 求解 → 治理 8 闸门 → 出码
