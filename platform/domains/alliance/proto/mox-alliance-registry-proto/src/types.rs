...
mod tests {
    ...
    #[test]
    fn register_request_defaults_roundtrip() {
        let json = r#"{"name":"expert-code","endpoint":"http://127.0.0.1:3300"}"#;
        let req: RegisterRequest = serde_json::from_str(json).unwrap();
        assert_eq!(req.version, "0.1.0");
        assert_eq!(req.weight, 1.0);
        assert_eq!(req.load_capacity, 100);
        assert_eq!(req.lease_seconds, 15);
        let back = serde_json::to_string(&req).unwrap();
        let req2: RegisterRequest = serde_json::from_str(&back).unwrap();
        assert_eq!(req2.name, req.name);
    }

    #[test]
    fn instance_lease_and_discoverable_semantics() {
        let now = Utc::now();
        ...
        assert!(inst.is_alive_at(now + chrono::Duration::seconds(10)));
        assert!(inst.is_expired_at(now + chrono::Duration::seconds(20)));
        assert!(InstanceStatus::Draining.is_discoverable());
        assert!(!InstanceStatus::Expired.is_discoverable());
    }

    #[test]
    fn expert_status_serde_lowercase_contract() {
        assert_eq!(
            serde_json::to_string(&InstanceStatus::Unhealthy).unwrap(),
            "\"unhealthy\""
        );
        let s: InstanceStatus = serde_json::from_str("\"draining\"").unwrap();
        assert_eq!(s, InstanceStatus::Draining);
    }
}
