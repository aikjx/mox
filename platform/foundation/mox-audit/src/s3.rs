...
//! PutObject 请求，并手写 AWS Signature V4 请求签名（与 S3 兼容
...
impl S3Sink {
    ...
    pub(crate) fn upload(&self, key: &str, body: &[u8]) -> Result<(), AuditError> {
...
mod tests {
    ...
    #[test]
    fn with_object_lock_flag() {
        let sink = S3Sink::new("b", "r").with_object_lock();
        assert!(sink.object_lock);
    }

    #[test]
    fn with_prefix_sets_prefix() {
        let sink = S3Sink::new("b", "r").with_prefix("audit/");
        assert_eq!(sink.prefix, "audit/");
    }

    #[test]
    fn append_to_buffer_no_upload() {
        // 仅追加到缓冲，不触发上传（无网络）
        let sink = S3Sink::new("b", "r");
        let result = sink.append_sync(&test_event());
        // 由于无 S3 服务，append_sync 只写缓冲，不调用 upload
        // （只有小时切换或 flush 时才调用 upload）
        assert!(result.is_ok());
        assert_eq!(sink.hour_buffer.lock().unwrap().len(), 1);
    }
}
