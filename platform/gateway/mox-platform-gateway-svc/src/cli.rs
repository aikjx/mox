...
use mox_cloud_volume_svc::{EcProfile, EcManifest, crc64_ecma};
use mox_data_compliance_svc::legal_hold::LegalHold;
use mox_data_compliance_svc::miji::{Clearance, MijiLevel, judge_read};
use mox_data_plane_svc::{
    FshcScanner, MountpathRegistry, MountpathState, TripleListener, TripleListenerConfig,
};
use mox_data_etl_svc::{PluginKind, PluginRegistry};
use mox_kg_fusion_svc::{TagSet, tag_cdc_graph_stage};

use crate::o11y::{BenchSamples, MoxMetrics};

// =========================================================================
// Clap definition
// =========================================================================

/// Mox v2.0 AIS-grade fusion single-binary server.
#[derive(Debug, Parser, Clone)]
#[command(
    name = "mox-server",
    version,
    about = "Mox v2.0 unified single-binary (server/ec/mount/compliance/bench/etl)"
)]
pub struct Cli {
    #[command(subcommand)]
    pub command: Command,
...
pub enum Command {
    /// Start (or dry-run) the Mox triple-listener server.
    Server(ServerArgs),
...
pub struct EcEncodeArgs {
    ...
    #[arg(long, default_value = "/tmp/mox-ec-out")]
    pub out: String,
}
...
impl CliState {
    pub fn new() -> Arc<Self> {
        Arc::new(Self {
            ...
            metrics: MoxMetrics::new().expect("metrics registry build"),
        })
    }
}
    ...
    match &args.op {
        EtlOp::ListPlugins => {
            ...
            for id in ["md5", "upper"] {
                if state
                    .etl
                    .run_inline_get(id, b"probe", &mox_data_etl_svc::EtContext::new("u", "b"))
                    .is_ok()
                {
        ...
        EtlOp::Run { plugin, data } => {
            let ctx = mox_data_etl_svc::EtContext::new("cli-user", "cli-bucket");
            let out = state
                .etl
