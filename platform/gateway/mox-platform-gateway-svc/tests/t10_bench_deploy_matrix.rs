...
use std::sync::Arc;
use mox_cloud_volume_svc::crc64_ecma;
use mox_platform_gateway_svc::BenchSamples;
use mox_platform_gateway_svc::cli::{
...
use mox_platform_gateway_svc::cli_run;
...
fn repo_root_relative(sub: &str) -> PathBuf {
    // cwd when running tests: platform/services/mox-server (3 levels below repo root)
    // Up 3 levels: mox-server -> services -> platform -> repo_root (infotopograph)
...
// T10-07 deploy_helm_values_smoke: read deploy/helm/mox/templates/values.yaml,
//         assert replicaCount >= 1 via serde_yaml.
#[test]
fn t10_07_deploy_helm_values_smoke_replicacount_ge_1() {
    let path = repo_root_relative("deploy/helm/mox/templates/values.yaml");
