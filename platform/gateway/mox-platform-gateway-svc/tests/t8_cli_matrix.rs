...
//!
//! Exercices the `mox_platform_gateway_svc::cli_run` pure function with in-memory
...
//!
//! All tests stay in-process — no `Command::new("mox-server")` spawn,
//! no real TCP binding.
    ...
    let cli = Cli {
        command: Command::Ec(EcArgs {
            op: EcOp::Decode(EcDecodeArgs {
                manifest: "/tmp/mox-nosuch-manifest.json".to_string(),
            }),
        }),
    };
    let v = run(&cli, &s);
...
fn t8_10_etl_register_stub_custom() {
    ...
    let reg_cli = Cli {
        command: Command::Etl(EtlArgs {
            op: EtlOp::Register {
                kind: mox_platform_gateway_svc::cli::PluginKindArg::InlineGet,
                id: "custom".to_string(),
                code: "noop".to_string(),
            },
        }),
