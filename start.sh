...
if [ "$DO_VERIFY" -eq 1 ]; then
  ...
  if "$PY_BIN" scripts/service/server-manage.py verify; then
    echo " $tick 公理验证通过"
  else
...
if [ "$DO_STOP" -eq 1 ]; then
  echo
  echo "[STOP] 按拓扑停止全部服务..."
  "$PY_BIN" scripts/service/server-manage.py stop all --force
  exit $?
fi
...
if [ "$DO_RESTART" -eq 1 ]; then
  ...
  "$PY_BIN" scripts/service/server-manage.py restart all --strict
  rc=$?
  if [ "$rc" -ne 0 ]; then echo "${cross} 重启失败（code=$rc）" >&2; exit "$rc"; fi
  echo " $tick 重启完成"
  "$PY_BIN" scripts/service/server-manage.py list
  exit 0
fi
...
echo
echo "[MAIN] 调用 scripts/service/server-manage.py bootstrap（默认：仅面板 → 页面上按需启动服务）"
...
echo "   → $ $PY_BIN scripts/service/server-manage.py ${BOOT_ARGS[*]}"
"$PY_BIN" scripts/service/server-manage.py "${BOOT_ARGS[@]}"
rc=$?

if [ "$rc" -ne 0 ]; then
  echo "${cross} bootstrap 失败 exit=$rc" >&2
  if [ "$DRY_RUN" -eq 0 ]; then
    "$PY_BIN" scripts/service/server-manage.py status
    dump_logs
  fi
  exit "$rc"
fi
...
echo "   · 停止所有 ：${C_CYN}$0 --stop  或  $PY_BIN scripts/service/server-manage.py stop all --force${C_RST}"
echo "   · 运维 CLI ：${C_CYN}$PY_BIN scripts/service/server-manage.py list|status|logs|stop${C_RST}"
