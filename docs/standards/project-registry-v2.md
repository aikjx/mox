...
| P-03 MOX跟进 | 每个项目按「需求→架构→业务→算法→开发→验证→交付」七维全链路跟进 | 项目全景页展示七维进度 |
...
| 4 | proj-dev-expert-alliance | 开发专家联盟 | maintaining | MOX分析优化 · 企业级 · 算法验证最优需求业务处理流程图 |
...
| 状态 | maintaining |
| 核心定位 | MOX分析优化 · 企业级 · 通过算法验证最优需求业务处理流程图 |
| 版本 | v3.0（MOX优化版，2026-08-26） |

### 三大核心标签

| 标签 | 内涵 | 验证基准 |
|------|------|---------|
| MOX分析优化 | 双璇玑十四维（业务七维 + 开发七维）并行诊断；v3 架构 7 服务拆分；16 张 Mermaid MOX流程图 | expert-alliance/v3/03-business-flow-diagrams.md |
| 企业级 | 多租户三档隔离；RBAC 四权分离；审计哈希链；SLA + 成本预算治理闸门；K8s + HPA | expert-alliance/v3/01-architecture-optimization.md |
| 算法验证最优 | flow-ai 已验证算法栈：冒险分析并行化 + CPM + RCPSP + 冲突修复 + Dijkstra；璇玑验证网关 5 项阻断级检查 | 璇玑-MOX需求业务处理流程图-归一化企业级.md |
...
| domain-flow-ai | MOX流程优化 | CPM + RCPSP + 冲突修复 + 代码生成；已验证最优求解 |
...
| 2026-08-26 | v2.0 | 新增 proj-default-dialogue（AI对话 - 默认对话）；proj-expert-alliance 更名为 proj-dev-expert-alliance（开发专家联盟），定位升级为MOX分析优化·企业级·算法验证最优需求业务处理流程图；新增 P7 对话归属不变式；全项目描述优化 |
