...
2. **ChatView.vue（1773+ 行）**：
   - 顶栏（L12-81）：已有专家选择器、两个开关、恢复历史/导入/导出/转任务/创建项目/清空共 10+ 元素，但**未按 FR11 严格顺序"新建/清空/导出/导入/转任务/创建项目/MOX分析"7 按钮排列、缺少"MOX分析 φ 强调主按钮"和"分析阶段 5 chip 指示器"**。
...
- **顶栏 7 按钮重排**（FR11）：chat-tools 左侧加"新建对话"按钮；重排顺序为 新建/清空/导出/导入/转任务/创建项目/**MOX分析（主渐变 φ 按钮）**；在顶栏下方新增分析阶段 5 chip 行（需求→架构→实现→测试→验收），颜色 φ 递进（冷蓝→靛→紫→粉→金），点击切换 `requirementFlowMode` + 跳对应 stage。
