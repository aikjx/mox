...
3. s3→volume 解耦 + RustFS ecstore 后端骨架：移除死依赖，StorageBackend 依赖注入，ObjectMeta 用 chunk_id 替代内联 data
...
│  ┌──────┐ ┌──────────┐ ┌───────────┐ ┌────────────┐ ┌───────────┐  │
│  │reed_ │ │gf256_    │ │multi_     │ │hedged_     │ │backpressure│ │
│  │solomon│ │simd      │ │writer     │ │reader      │ │            │  │
│  └──────┘ └──────────┘ └───────────┘ └────────────┘ └───────────┘  │
│  ┌──────────┐ ┌────────────────┐ ┌───────┐ ┌────────┐ ┌──────────┐ │
│  │buffer_   │ │reader_capability│ │scanner│ │profile│ │metrics   │ │
│  │pool      │ │                │ │       │ │        │ │          │ │
│  └──────────┘ └────────────────┘ └───────┘ └────────┘ └──────────┘ │
│  零业务依赖：仅 tokio / async-trait / futures / thiserror /           │
│  parking_lot / serde / bytes / rand / tracing                          │
└──────────────────────────────┬────────────────────────────────────────┘
                               │
┌──────────────────────────────▼────────────────────────────────────────┐
│                        L6  存储后端层                                  │
...
│   └──────────────────┘  └──────────────────┘  └──────────────────┘  │
│   feature flag: rustfs_ecstore_backend (默认 false)                   │
└─────────────────────────────────────────────────────────────────────────┘
```
...
| 9 | `profile` | `profile.rs` (~3KB) | EcProfile 纠删码配置 | data_shards/parity_shards、DEFAULT_MIN_OBJ_SIZE |
| 10 | `metrics` | `metrics.rs` (~16KB) | 数据面指标 | encode_us 直方图、REBUILD_COUNT、SHARDS_LOST_TOTAL、全局原子指标 |
...
pub trait StorageBackend: Send + Sync {
    ...
    async fn list_chunks(&self, prefix: &str, marker: Option<&str>, limit: u32) -> Result<ChunkListPage, StorageError>;
...
**职责**：统一底层 chunk 级存取契约，所有存储后端（内存/本地FS/RustFS ecstore/S3兼容）均需实现。
...
- 新增 `mox-cloud-kernel` 依赖（scanner/buffer_pool 等算法）
...
pub struct S3Server {
    ...
    /// 读管线（可选，默认 None，op_get_object 走单后端）
...
| 方法 | 签名 | 用途 |
|------|------|------|
| `new(port, master)` | `pub fn new(port: u16, master: Option<Arc<MasterServer>>) -> Self` | 默认构造，使用 InMemoryStorageBackend，reader_pipeline=None |
...
2. `with_storage_backend()` 设置 storage_backend，reader_pipeline 默认为 None
3. 可选链式 `.with_reader_pipeline(pipeline)` 启用 hedged read

### 6.4 数据路由：8 个数据操作函数改为 async trait 路由

**chunk_id 生成规则**（实测 `s3_server.rs:166-170`）：
```rust
fn object_chunk_id(bucket: &str, key: &str, version_id: &str) -> String {
    // 格式：obj:{bucket}:{key}:{version_id}
    // 覆盖写入时 chunk_id 不变，put_chunk 自然覆盖
    // 删除标记不创建 chunk（chunk_id 为空字符串）
}
```
...
| 函数 | 路由方法 | 说明 |
|------|---------|------|
| `read_object_data(storage_backend, chunk_id)` | `storage_backend.get_chunk(&ChunkId::new(chunk_id))` | chunk_id 为空（删除标记）时返回空 Vec |
| `write_object_data(storage_backend, bucket, key, version_id, data)` | `storage_backend.put_chunk(&ChunkId::new(&chunk_id), data)` | 生成 chunk_id 后写入，返回 chunk_id |
| `delete_object_data(storage_backend, chunk_id)` | `storage_backend.delete_chunk(&ChunkId::new(chunk_id))` | chunk_id 为空时跳过 |
...
| 4 | `op_head_object` | 元数据操作，不路由数据（仅检查 chunk_id 非空） | HEAD 不读数据 |
...
### 6.5 Send 问题修复：parking_lot MutexGuard 作用域
...
let result = backend.put_chunk(&id, &data).await;  // guard 仍持有，!Send
...
let result = backend.put_chunk(&id, &data.as_ref().unwrap()).await;  // 安全
```
...
| `capabilities` | range_read=true, atomic_write=true, consistency=Strong, max_chunk=128MB | 目标能力声明 |
...
**启用方式**：
```powershell
cargo build -p mox-cloud-s3-svc --features rustfs_ecstore_backend
...
pooled.as_mut().copy_from_slice(data);
// ... 使用 pooled ...
// RAII：pooled drop 时自动归还缓冲池

// 读取路径
let mut pooled = self.buffer_pool.acquire(size);
// ... 读入 pooled ...
```
...
| 方法 | 实现 |
|------|------|
| `read_shard(shard_index)` | 忽略 shard_index（S3 对象读无分片概念），调用 `backend.get_chunk(&ChunkId::new(&self.chunk_id))` |
...
pub fn with_reader_pipeline(mut self, pipeline: Arc<S3ReaderPipeline>) -> Self {
    self.reader_pipeline = Some(pipeline);
    self
}
```

**op_get_object 路由逻辑**（实测）：
```rust
// 优先通过 reader_pipeline 读取（hedged read，多后端取最快）
if let Some(pipeline) = &state.reader_pipeline {
    if pipeline.backend_count() > 0 {
        data = pipeline.read_object(&chunk_id).await?;
    }
}
// 回退：单后端 storage_backend.get_chunk()
if data.is_empty() && !chunk_id.is_empty() {
    data = read_object_data(&state.storage_backend, &chunk_id).await?;
}
```
...
use mox_cloud_kernel::backpressure::BackpressureMonitor;
// ... 等 10 个模块
```
...
| # | 任务 | 说明 | 依赖 |
|---|------|------|------|
| 1 | **RustFS ecstore 实际 FFI/进程对接** | 实现 `RustFsEcstoreBackend` 的 put_chunk/get_chunk/delete_chunk 数据面操作，通过 Unix Socket 或 gRPC 与独立部署的 RustFS ecstore 进程通信 | 阶段四骨架已就绪 |
...
| 6 | **CAS 背压扩展到 read_chunk 和批量操作** | 阶段三仅接入 write_chunk，阶段五扩展到读路径和批量删除/复制 |
| 7 | **ShardReader/ShardWriter trait 落地实现** | 将 volume-svc 现有 hedged_reader/multi_writer 适配为 domain-traits 的 ShardReader/ShardWriter trait |
