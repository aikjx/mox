...
| **新增源码模块**            | 3 个  | 6 个   | +3（buffer\_pool/reader\_capability/config + scanner/s3-config/volume-config） |
...
### 2.2 CAS 背压接入 volume\_server 写入主路径（P0）
...
| **新增测试** | 5 个（acquired\_on\_write /rejects\_when\_full/config\_custom /default\_constructor/consecutive\_writes）                                                        |
...
| **新增测试**     | 11 个 reader\_capability 测试 + 2 个 hedged\_reader 集成测试                                                                                     |
...
| **修改文件**          | `s3-svc/lifecycle.rs`（集成 scan\_budget）、`s3-svc/lib.rs`、`volume-svc/lib.rs`                                                                                                                        |
...
| **Feature Flags** | 新增 6 个（rustfs\_ecstore\_backend /rustfs\_rio\_backend/buffer\_pool\_enabled /scan\_budget\_enabled/backpressure\_enabled /hedged\_read\_enabled 等），共 10 个                                         |
...
| 修改 | `platform/domains/cloud/svc/mox-cloud-volume-svc/src/volume_server.rs`              | 背压接入 write\_chunk 主入口                   |
...
3. **CAS 背压仅接入 write\_chunk**：read\_chunk 和批量操作尚未接入，阶段四应扩展
