...
| **璇玑**（保留，与「璇玑 RelGraph」不混用） | Xuánjī Engine / `mox-expert` crate（技术代号） | 归一化 IR 驱动的**元调度诊断引擎实体**：双璇玑十四维并行诊断 → 裁决 → flow-ai 求解 → ⛨验证网关 → 治理闸门 → 出码/出图。注意：「璇玑 RelGraph」是**整个产品名**；「璇玑」单独出现时指 `mox-expert` 中的最高权限引擎。 | `platform/services/mox-expert/` · ⛨璇玑验证网关 |
