...
| I-15 | **mox 模块化系统架构维度开发完成（里程碑）**：双璇玑十四维 + 四闸门 + 流程图归一化 + mox 模块化系统架构自动化处理，全部经测试验证 | `07`/`08`/`09` | 双璇玑契约测试 14 维齐全、越权被拦、端到端跑通、workspace 全绿 | ✅ 已达成（见 `09`） |
...
| ID | 项 | 说明 |
|----|----|------|
| I-12 | 灾备：快照 + 混沌工程（对齐 `docs/architecture/architecture.md` §16） |
...
- [ ] **mox 模块化系统架构双收口（铁律，见 `07`）**：任何功能须先过 `07` §3 双闸门——①业务流程图归一化（唯一 `FlowGraph` + 六维绑定 + 过 G0）；②开发璇玑mox 模块化系统架构分析验证（`run_experts` 14 专家无 `veto` + 璇玑 `vetoed=false` + 治理 `approved=true` + `primiflow_fusion::full_gate` 全绿）。**未经双收口的功能一律退回，禁止无验证扩展、禁止模糊需求、禁止给闸门加旁路**。
