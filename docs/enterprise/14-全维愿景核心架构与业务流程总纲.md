# 璇玑 RelGraph · 全维愿景 · 核心架构与业务流程总纲（Enterprise Charter · 对齐 18 TOP-MASTER）
...
> **承载底座**：`GR-STD-V1.0` 关图（高维需求关系图）＋ `PT-STD-V1.0` 六维绑定（物理底座）＋ `AA-STD-V1.0` 全维需求流程（业务事实基准）
...
| ⑤ | 全维通过 AI 整理，不断地优化架构与算法 | **AI 全维整理**：双璇玑十四维并行诊断＋归一化裁决＋已验证最优求解，持续打磨架构与算法 |
...
>
> **M5 · AI 全维整理**：双璇玑十四维（业务七维＋开发七维）并行诊断，归一化裁决（硬约束优先），flow-ai 已验证最优求解（CPM/RCPSP/Dijkstra/冲突修复/CodeGen）。
>
...
> **用一张高维、可机读、全关联的需求关系图作为系统的唯一事实基准，让 AI 在全维约束下持续整理与优化架构算法，把人类从"原地重复"中解放出来，只做真正的创新。**

---

## 1 · 全维工程方法论落地（How We Engineer）
...
- **璇玑（协作治理域 · `xuanji-system`）**：组建璇玑 → 专家入璇玑 → 任务派发 → 协同推进 → 全程留痕。
- **璇玑融合（算子融合域 · `xuanji-expert`）**：归一化 → 双璇玑十四维会诊 → 冲突消解 → 治理裁决 → 产出可复用优化算子并上架。
...
4. 安全验收：跨租户提升路径被阻断；鉴权失败可审计；`xuanji bench` 加速比 ≥2.32×。
...
| 应用 Application | OUS 五层映射；模块单一职责；`xuanji-system`/`xuanji-expert`/`flow-ai` 为核心 |
...
| 部署 Deployment | 多后端持久化（`XUANJI_PERSIST=true`＋`STRICT` fail-fast）；可观测性/灾备入路线图 |
...
|------|--------|-------------------------------|
| 1 | 璇玑主核（协作治理 + 融合治理） | `platform/services/xuanji-system`（成员/任务/权限/通信/RBAC/多后端 SQLite+PG+MySQL repo）；`platform/services/xuanji-expert`（双璇玑十四维治理 / 归一化 / 裁决 / ⛨璇玑验证 / 审计三汇 / RBAC） |
| 2 | 算子内核与执行 | `platform/services/operator-core`（算子代数/守恒律/类型核心）；`platform/services/operator-wasm`（WASM 字节码沙箱） |
| 3 | 图与知识 / AI | `platform/services/graph-algorithms`（**八大算法家族 A1~A8**：CNM/Brandes/Harmonic/PageRank/激活扩散/RRF/CEM/CPM·RCPSP）；`platform/services/kg-hub`（混合索引+URN+本体/摄入/推理/治理/影响/热点/闭环8段/5连接器）；`platform/services/ai-agent`（多阶段 Engine/LLMClient 路由/浏览器自动化/需求编译器/BPMN/MultiAgent/ProviderRegistry）；`platform/services/optimizer`（CPM/RCPSP 调度）；`platform/services/flow-ai`（9 模块：冒险/CPM/冲突/调度/拓扑/代码gen/流水线/原语/可视化） |
| 4 | 归一化 / 融合 / 编排 / 市场 / 桥接 / 目录 | `platform/services/primiflow-core`（PrimiFlow 解析/代码生成/8 类骨架模板/执行/持久化）；`platform/services/primiflow-fusion`（六维融合/守恒闸门/Registry/平台编排/12Factor+可观测）；`platform/services/template-market`（市场发布/列表/加载/评分/排序/Fork/2 种子）；`platform/services/hermes-flow-bridge`（Hermes Agent 桥接：normalize/recorder/router/拦截注入）；`platform/services/business-catalog`（7 预置 FlowGraph + TopologyGraph：政务/法院/财务/客服/ETL/MCP/螺旋） |
| 5 | 聚合网关 & 前端（统一入口） | `platform/gateway/runtime/`（15 crate 聚合网关：routes/handlers/Cordis5 子模块/RBAC 中间件/市场 DSL/迁移/治理/OpenAPI/operator-server + 统一 AI 四端点 /ai/engine/{process,analyze,capabilities,metrics}）；`frontend-ui/`（用户端单应用 28 视图 + `/admin` 系统管理 5 面板：融合工作台 XuanjiFusionView、监控台 MonitorView、AdminView 访问/审计/HITL/存储/总览） |
...
> 完整治理见 `00-INDEX.md`（唯一治理入口）与 `docs/README.md`（关图/全维专题分区索引）。本节为其**总纲式映射**。

### 4.1 `docs/` 归一化布局

| 分区 | 等级 | 内容 |
|------|------|------|
| `enterprise/` | 🟢 权威治理中心（本文集 `00~14`） | 需求/架构/设计/业务/路线图/映射/全维/归档/交付 |
| `specs/` | 🟢 权威规范 | `GR-STD`（关图）/ `PT-STD`（六维绑定）/ `OUS`（业务规划） |
| `full-dimensional/` | 🟡 全维专题 | AA-STD 流程基准 / 关图骨架 / 治理台 API / 过程稿归档 |
| `graph/` | 🟡 机读产物 | `graph.json` / `graph.enterprise.json` / `guantu.req.json` |
| 根级 `*.md` | 🟢/🟡 | AA-STD（融合域权威）、`xuanji-expert-*`、过程稿、可视化 `*.html`/`*.mmd` |

### 4.2 编号归一基准

- 全维流程阶段 **S1~S8**；闸门 **G0~G3**；护栏 **G-A~G-E**；规范 **GR-STD / PT-STD / AA-STD**。
...
```mermaid
flowchart LR
    A["需求录入<br/>(关图 REQ 根)"] --> B["AI 全维诊断<br/>(双璇玑十四维)"]
    B --> C["归一裁决<br/>(硬约束优先)"]
...
> 完整证据见 `09-企业级全维度完成归档` / `10-企业级交付清单` / `11-全维测试验证优化修复报告` / `12-RBAC审计全链路闭环验收报告` / `13-禁伪代码真并行零死代码闭环验收报告`。

| 维度 | 证据 | 数值 |
|------|------|------|
| 测试基线 | workspace 全量回归（实测 2026-08-18） | **644 passed / 0 failed / 6 ignored**（58 测试二进制） |
| 静态质量 | `cargo clippy` | 零告警，workspace 0 warning |
| 性能 | `xuanji bench` 融合加速比 | ≥ **2.32×**（实测 2.32×） |
...
- **先读人话版**：本文 `§0 北极星` → `07-全维需求明确书`（铁律）→ `08-全维自动化处理明确书`（铁律怎么自动跑）。
...
| 版本 | 说明 |
|------|------|
| v1.0 (ENT) | 首版顶层总纲：把"高维需求关系图/描述互链/关联关系全明确/快速判重/AI 全维整理/持续优化/人类聚焦创新"七原则整理为方法论，并向下收敛到 `01/02/00-INDEX/AA-STD`；含 mermaid 业务流程图与优化飞轮。 |
...
| 双璇玑 / 四闸门 / 主流程 | `璇玑-全维需求业务处理流程图-归一化企业级.md`（AA-STD-V1.0） |
