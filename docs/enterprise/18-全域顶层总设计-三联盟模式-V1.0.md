...
| 事实 | 载体（溯源路径） |
|---|---|
| 双璇玑十四维维度模型（业务 7 + 开发 7）已代码化落地 | `platform/services/mox-expert/src/experts/{business,algorithm,permission,resource,security,data,observability,architecture,security_code,code_quality,performance,testing,documentation,maintainability}.rs`（参见 `docs/enterprise-architecture-analysis.md` §2） |
| ⛨璇玑验证网关 + 治理 8 闸门（G1~G8）+ 归一化裁决 reconcile | `platform/services/mox-expert/src/verify/mod.rs` + `platform/services/primiflow-fusion/src/unified.rs::full_gate` + `platform/services/mox-expert/src/reconcile.rs` |
...
| 多后端持久化（SQLite / PostgreSQL / MySQL 零代码切换 + fail-fast） | `platform/services/mox-system/src/repo/{sqlite,postgres,mysql,schema}.rs` + `platform/services/mox-system/src/persistence_provider.rs` |
...
| 共享元 & 常量 | `platform/services/mox-common-meta/` | crate_id 唯一性、lookup、版本常量 |
...
| 璇玑融合引擎 | `platform/services/mox-expert/` | experts(14) + verify(5) + govern + reconcile + audit + rbac + flow_loader |
| 璇玑协作治理域 | `platform/services/mox-system/` | services / orchestrator / rbac / repo(3 方言) / crypto / event / metrics |
...
- **⑤审计事件签名**：事件名 `bp.01.requirement_ingested` / `bp.01.dedup_blocked`；签名算法 HMAC-SHA256（`MOX_AUDIT_SECRET`）；汇入口 = `platform/services/mox-expert/src/audit/sink.rs`（Syslog / S3 / NATS 三汇）。
...
- **锚点**：`docs/璇玑-全维需求业务处理流程图-归一化企业级.md`（AA-STD-V1.0）+ `platform/services/mox-expert/src/ir.rs`（FlowGraph 四维合一）。
...
- **锚点**：`platform/services/mox-expert/src/verify/` + `platform/services/primiflow-fusion/src/unified.rs::full_gate` + `docs/enterprise/11-全维测试验证优化修复报告.md`。
...
- **锚点**：`platform/services/optimizer/src/lib.rs` + `.trae/specs/20260823-mox-storage-distributed-ai-unified-query/review.md`（A+ 报告 rolloutPlan/readinessProbe/warmupRun）。
...
17. 璇玑验证 / 治理闸门实现：`platform/services/mox-expert/src/verify/mod.rs` · `platform/services/primiflow-fusion/src/unified.rs::full_gate`
