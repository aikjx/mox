...
> - `docs/璇玑-全维需求业务处理流程图-归一化企业级.md`（AA-STD-V1.0，融合域需求事实基准，§四）  
> - `docs/standards/expert-alliance-flow-standard.md`（EAF-STD-V1.2，专家联盟流程，§四.4 + §五.二）  
> **产品手册（对外介绍）**：`docs/xuanji-relgraph-product-handbook-v3.md`（本文件为顶层设计，对外宣讲以产品手册为准，两者若有冲突以本文件的架构硬约束部分为最终裁决）  
...
| 事实 | 载体（溯源路径） |
|---|---|
| 双璇玑十四维维度模型（业务 7 + 开发 7）已代码化落地 | `platform/services/xuanji-expert/src/experts/{business,algorithm,permission,resource,security,data,observability,architecture,security_code,code_quality,performance,testing,documentation,maintainability}.rs`（参见 `docs/enterprise-architecture-analysis.md` §2） |
| ⛨璇玑验证网关 + 治理 8 闸门（G1~G8）+ 归一化裁决 reconcile | `platform/services/xuanji-expert/src/verify/mod.rs` + `platform/services/primiflow-fusion/src/unified.rs::full_gate` + `platform/services/xuanji-expert/src/reconcile.rs` |
| 图谱算法归一（CNM / Brandes / Harmonic / PR 转置 / 激活扩散） | `platform/services/graph-algorithms/src/lib.rs` + `platform/backend-node/src/lib/graph-algos.js` + 对账脚本 `platform/services/graph-algorithms/scripts/reconcile_7x8.js`（Δ≤1e-6 已锁定） |
...
| 多后端持久化（SQLite / PostgreSQL / MySQL 零代码切换 + fail-fast） | `platform/services/xuanji-system/src/repo/{sqlite,postgres,mysql,schema}.rs` + `platform/services/xuanji-system/src/persistence_provider.rs` |
...
| 算子内核：6 大公理 + 范畴论 + 希尔伯特空间 + 单子 + 守恒律 | `platform/services/operator-core/src/{category,monad,conservation,state,kernel,kernel_ext}.rs` |
...
graph TD
    subgraph L6 [L6 产品应用层 · 用户端统一 frontend-ui 单应用]
        ...
        A2[GraphView · FlowGraph · XuanjiFusionView 图谱与融合]
    ...
    subgraph L3 [L3 算法推理层 · 璇玑融合 + 专家联盟 + CEM]
        D1[xuanji-expert · 双璇玑 14 维 / reconcile 裁决 / ⛨验证 / 审计链]
    ...
    subgraph L2 [L2 Rust 自研工程底座 · 唯一真相源]
        E1[xuanji-common-meta · 共享元 & 常量]
        E2[operator-core + operator-wasm · 算子内核 + WASM 沙箱]
        E3[xuanji-system · 协作治理（成员/任务/RBAC/审计/3 方言）]
...
| 共享元 & 常量 | `platform/services/xuanji-common-meta/` | crate_id 唯一性、lookup、版本常量 |
| 算子内核 + WASM | `platform/services/operator-core/` + `platform/services/operator-wasm/` | 范畴论/守恒律/希尔伯特/单子 + 热加载插件 |
| 图算法归一 | `platform/services/graph-algorithms/` | 导出公式 `export_formula.rs` + 对账 reconcile_7x8.js |
| 图谱中枢 | `platform/services/kg-hub/` | api / ingest / ontology / govern / loop_engine / urn |
| 通用优化器（CEM） | `platform/services/optimizer/` | RCPSP + CEM 统一接口 |
| 流程 AI（CodeGen 等） | `platform/services/flow-ai/` | pipeline / topology / schedule / conflict |
| AI Agent（对话/工作流/浏览器自动化） | `platform/services/ai-agent/` | conversation / workflow_engine / llm_client / plugin_bus |
| 璇玑融合引擎 | `platform/services/xuanji-expert/` | experts(14) + verify(5) + govern + reconcile + audit + rbac + flow_loader |
| 璇玑协作治理域 | `platform/services/xuanji-system/` | services / orchestrator / rbac / repo(3 方言) / crypto / event / metrics |
| PrimiFlow 执行与持久化 | `platform/services/primiflow-core/` | executor / runner / persistence / gen(C1~C8) |
| PrimiFlow 融合（8 闸门） | `platform/services/primiflow-fusion/` | unified.rs full_gate G1~G8 + sixdim + ptdoc + platform |
| 外部流桥接 | `platform/services/hermes-flow-bridge/` | bridge + router + recorder + plugin + state |
| 螺旋业务目录 | `platform/services/business-catalog/` | spiral.rs + catalog bin |
| 模板市场（算子商城） | `platform/services/template-market/` | 资产包（需求+流程图+功能点）上架、克隆、版本 |
| 存储归一（📋 规划中） | `platform/services/storage-engine/` | ADR-DOC-003：统一 StorageProvider + ChunkBackend + CDC（现状功能在 backend-node 物理实现，规划中迁移） |
...
| EB-16 | `audited_by` | 某组件/流程被哪套审计链监控 | 是（审计汇路径） | Workflow(L3) / Crate(L2) → AuditSink(xuanji-expert audit/*) |
...
- **⑤审计事件签名**：事件名 `bp.01.requirement_ingested` / `bp.01.dedup_blocked`；签名算法 HMAC-SHA256（`XUANJI_AUDIT_SECRET`）；汇入口 = `platform/services/xuanji-expert/src/audit/sink.rs`（Syslog / S3 / NATS 三汇）。
...
- **代码锚点**：`platform/services/kg-hub/src/{ingest,govern,api}.rs` + `platform/services/graph-algorithms/src/lib.rs` + `docs/specs/GR-STD-信息关联关系图开发规范-V1.0.md`。
...
- **锚点**：`docs/璇玑-全维需求业务处理流程图-归一化企业级.md`（AA-STD-V1.0）+ `platform/services/xuanji-expert/src/ir.rs`（FlowGraph 四维合一）。
...
- **锚点**：`platform/services/graph-algorithms/src/flow_graph.rs` + `platform/services/optimizer/src/lib.rs` + `docs/modules/algorithm-verification.md`（AV-STD L1~L3）。
...
- **锚点**：`platform/services/xuanji-expert/src/verify/` + `platform/services/primiflow-fusion/src/unified.rs::full_gate` + `docs/enterprise/11-全维测试验证优化修复报告.md`。
...
- **锚点**：`platform/services/optimizer/src/lib.rs` + `.trae/specs/20260823-xuanji-storage-distributed-ai-unified-query/review.md`（A+ 报告 rolloutPlan/readinessProbe/warmupRun）。
...
- **锚点**：`platform/services/template-market/` + `platform/gateway/runtime/src/market_*.rs` + `docs/market-module.md`。
...
- **锚点**：`platform/backend-node/src/storage/{index,chunk-backend}.js` + `platform/services/kg-hub/src/{govern,loop_engine}.rs` + ADR-DOC-010（统一契约）。
...
| ⑧ 调度优化 | CPM 关键路径 + RCPSP（资源受限）+ Dijkstra 最短 + 伪依赖剪除 | 加速比 ≥ 2.32×（`xuanji bench` 基线）；绝不允许资源超配额调度（hard-cap） | gov_pii_graph 基准 ≥ 2.32×；`test-enterprise-slo-capacity-tco.js` 全通过 |
...
1. **后端 100% Rust 全维自研**（HC-14）：统一入口网关、路由语义表、AI 路由、Sidecar 通信、可观测探针、灰度权重、图谱算法核心 **全部 Rust Axum 实现**，`unsafe=0` 护栏（Cargo.toml workspace censor：不允许引入需要 unsafe 的 C 绑定依赖，WASM 走 wasmer 官方封装）。不使用重度第三方框架（如 Leptos/Actix-web 全家桶），保持 OUS-Cordis 插件内核 ctx 可控。
2. **严格对标 AIS 企业级项目架构体系**：Spring-Boot 式分层（Controller → Service/Engine → Repository/Store）。Rust 中：Controller=`platform/gateway/runtime/{routes,handlers}`；Service/Engine = `platform/services/*/src/{services,engine,orchestrator}.rs`；Repository = `repo/` / `persistence.rs` / `store.rs`。**禁止越层调用**：如 handler 内直接写 SQL、直接碰 rusqlite 连接（越层直接由 governance G8 审计链打 Blocked）。
...
| P3 · storage 未归一 | StorageProvider 与 ChunkBackend 实现在 backend-node 与 Node 文件 store 分离 | 历史阶段 Node 实现能力强，后 Rust 没迁移 | ADR-DOC-003：📋 M1 新增 crate `platform/services/storage-engine/`，作为三后端 + 两块 Chunk 的唯一真相源；Node 仅保留接口适配（edge-node） |
...
│   │   ├── xuanji-common-meta/     # 共享元 & 常量（shared/rust 的源派生源，📋 提供 schema 宏导出）
...
│   │   ├── xuanji-expert/          # 璇玑融合：双璇玑14维/⛨验证5项/治理8闸门/裁决/审计汇/RBAC
│   │   ├── xuanji-system/          # 协作治理：成员/任务/RBAC/通信/审计/3方言 repo fail-fast
...
│   │   ├── graph/  GraphView · FlowGraph · XuanjiFusionView
...
│   ├── rust/                       # 由 xuanji-common-meta 派生（宏生成）
...
└── docs/                           # 文档总根
    ...
    ├── xuanji-relgraph-product-handbook-v3.md     # 🟢 对外产品手册
    ├── README.md                   # 导航首屏（TOP-MASTER / 00-INDEX 双链接）
    └── 璇玑-全维需求业务处理流程图-归一化企业级.{md,html}
```
...
members = [
  ...
  "platform/services/storage-engine",        # ← 📋 ADR-DOC-003 M1
...
Phase 1 图谱心脏（graph-algorithms → kg-hub）
   ...
   ▼
Phase 2 治理（xuanji-system → primiflow-core → primiflow-fusion → xuanji-expert）
...
| M3 | 璇玑融合全栈 | 🔴 P0 · 四周 | xuanji-expert 双璇玑 14 维并行 rayon 真并行；reconcile 裁决 + ⛨ 5 项 + G1~G8 统一；审计链三汇（S3/Syslog/NATS） | L0：融合加速比 ≥ 2.32×；G8 通过率 ≥ 95%；三汇 0 丢包 |
...
| R8 | 多后端方言差异导致上线 bug | 中 | 高 | xuanji-system `sea-query` 方言归一 + 三方言 E2E；新增任何 SQL 必须先跑三方言集成，未通过不得合入。（开发联盟） |
...
2. 北极星总纲（老版本 v1.0，已被本文 §一/§二 收敛）：`docs/enterprise/14-全维愿景核心架构与业务流程总纲.md`
3. 产品契约 P1~P10 十大原则：`docs/enterprise/15-产品规范标准-人人爱用全自动全维-V1.0.md`
...
9. 璇玑归一化规范（三层收口 + 四条不变式）：`docs/modules/xuanji-expert-normalization.md`
10. 璇玑产品手册 v3（对外宣讲）：`docs/xuanji-relgraph-product-handbook-v3.md`
11. 全维需求事实基准 AA-STD：`docs/璇玑-全维需求业务处理流程图-归一化企业级.md`
...
16. 算法对账脚本：`platform/services/graph-algorithms/scripts/reconcile_7x8.js`（Δ≤1e-6 事实基线）
17. 璇玑验证 / 治理闸门实现：`platform/services/xuanji-expert/src/verify/mod.rs` · `platform/services/primiflow-fusion/src/unified.rs::full_gate`
