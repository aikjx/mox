...
| SE-11 | 全维双验收联动门禁（任务 Done ∧ 璇玑 Pass）| ❌ | ❌ | ❌ | ❌ | ❌ | ✅ | `/mox/publish` 强制双验收 AND 闭合；对应 Plan D 已交付 |
...
### 7.1 内部代码 + 文档事实
- 15 Crates：`platform/services/*/`（mox-expert / graph-algorithms / kg-hub / mox-system 等 15 份），每份 README 有三常量 + 模块结构 + 测试命令
