# 23 · 璇玑 RelGraph · 竞品全维功能对比 × 可用性判定报告（DOC-COMPETITIVE-ANALYSIS-V1.0）
...
| DM-08 | 全局唯一标识符 GUID / URN | ✅ elementId | ✅ VID(64bit) | ✅ _key | ✅ global_id | ✅ NeptuneID | ✅ | kg-hub `urn.rs` urn:xuanji:<type>:<uuid> |
...
| AL-16 | 图神经网络 GNN 训练 | ✅ Neo4j GenAI + GDS | ✅ Nebula + DGL | ❌ | ✅ TigerGraph GraphStudio ML | ✅ + Bedrock Titan | 📋 | CEM 交叉熵（xuanji-expert verify/cem.rs）是寻优而非 GNN 训练 |
| AL-17 | CPM 关键路径 RCPSP | ❌（非图库原生）| ❌ | ❌ | ❌ | ❌ | ✅ | 独有：flow-ai `pipeline::optimize` A8 算法家族 |
| AL-18 | CEM 交叉熵寻优（高维配置）| ❌ | ❌ | ❌ | ❌ | ❌ | ✅ | 独有：xuanji-expert verify/cem.rs γ=0.1 N=2000 iters=80 |
| AL-19 | 多目标加权评估（0.55 质量+0.20 速度+0.10 效率+0.15 稳定）| ❌ | ❌ | ❌ | ❌ | ❌ | ✅ | 独有：xuanji-expert `reconcile` 裁决加权；验收基线 649+ |
...
| AI-09 | AI 寻优 CEM 跨熵自动化 | ❌ | ❌ | ❌ | ❌ | ❌ | ✅ | xuanji-expert `verify/cem.rs`（test-multi-objective-eval-cem.js） |
...
| AI-12 | 专家评估 + 裁决 + 治理闸门 | ❌ | ❌ | ❌ | ❌ | ❌（最多 policy）| ✅ | xuanji-expert 7+7 14 专家并行 → reconcile 归一化裁决 → govern 闸门 → verify 璇玑否决 四道 G0~G3 |
...
| # | 功能点 | Neo4j | Nebula | ArangoDB | TigerGraph | Neptune | 璇玑 | 璇玑证据 |
|---|--------|:-----:|:------:|:--------:|:----------:|:-------:|:----:|---------|
| AR-01 | 水平分片（Sharding / Shared-Nothing）| ❌ CE / ✅ EE Fabric | ✅ GraphD 计算 + StorageD 分离 | ✅ SmartGraphs（3.12）| ✅ 原生分布式 | ✅ 自动扩容（云）| ❌ | 当前 kg-hub + xuanji-system 均为 SQLite 单进程；分片在 05 M1 路线图 |
...
| AR-04 | 存储计算分离 | ❌ CE / ✅ Aura | ✅ | ❌ | ❌ | ✅ | ❌ | 当前 kg-hub + xuanji 单体 crate；storage 目录存抽象在 Node 侧 |
...
| AR-11 | 多租户策略分层 | ✅ Fabric（EE）| ✅ Space 分区 | ✅ | ✅ | ✅ IAM + Neptune DB clusters | ✅ | xuanji-expert `tenant_policy.rs` I-06 租户策略分层已交付 |
...
| AR-14 | 全自研无重框架依赖（AIS 标准）| ❌ 大量开源依赖 | ❌ RocksDB / MetaRaft | ❌ | ❌ Boost 等 | ❌ | ✅ | operator-core T7 0 外部依赖单测通；xuanji-expert 仅必要 serde/tokio/utoipa 少量 |
...
| SE-03 | 审计链（可查询 hash-chain）| ✅ EE Query Log | ✅ | ✅ Audit Log | ✅ | ✅ CloudTrail | ✅ | xuanji-expert audit 3 sink（File/Syslog/S3）+ `GET /api/audit` + T14.4 hash_chain 180 天 TTI verify_ok=true |
| SE-04 | 敏感字段识别（PII/SSN/手机号）| ✅ + Bloom 分类 | ✅ 规则 | ❌ 需自研 | ✅ 分类标签 | ✅ + Macie | ✅ | kg-hub sensitivity.rs SSOT const 3 套（fields/patterns/kw） |
| SE-05 | STRIDE 威胁建模 + 安全专家 | ❌ 需客户自己做 | ❌ | ❌ | ❌ | ✅ + AWS Security Hub | ✅ | xuanji-expert security.rs STRIDE 6 类 + 14 专家 Security 维度 |
| SE-06 | 数据血缘 Lineage | ❌ 需 Neo4j 自建 | ❌ | ❌ | ✅ 内置 | ✅ 用 Glue 补 | ✅ | experts/data.rs `data lineage` 全维分析 + 图谱 L5 血缘层 |
...
| SE-09 | 治理闸门 8 条（守恒/孤儿/合规/…）| ❌（仅 Neo4j Semarchy 数据治理）| ❌ | ❌ | ❌ | ❌ | ✅ | primiflow-fusion `full_gate_with_baseline` G1-G8 全部；xuanji-expert `govern` 14 专家校验器 |
| SE-10 | 代码质量治理（死代码/clippy 0 警告）| ❌（非图库职责）| ❌ | ❌ | ❌ | ❌ | ✅ | 验收棘轮 13 `allow(dead_code) ≤ 8` + 验收 11 clippy 0 warning workspace |
| SE-11 | 全维双验收联动门禁（任务 Done ∧ 璇玑 Pass）| ❌ | ❌ | ❌ | ❌ | ❌ | ✅ | `/xuanji/publish` 强制双验收 AND 闭合；对应 Plan D 已交付 |
...
| SE-14 | 产物来源追溯 ProvenanceMetrics（溯源加速比）| ❌ | ❌ | ❌ | ❌ | ❌ | ✅ | xuanji-expert `ProvenanceMetrics(source_flow_id, dual_acceptance, speedup_ratio, conflict_count, expert_avg)` |
...
| DT-06 | xuanji_optimize 全 8 步自动化无旁路 | ❌ | ❌ | ❌ | ❌ | ❌ | ✅ | pipeline.rs Parse→Normalize→Bind→Analyze→Verify→Optimize→Reconcile→Govern；前端三步闭环 |
...
| DT-09 | 双验收联动（任务∧治理）双 AND | ❌ | ❌ | ❌ | ❌ | ❌ | ✅ | `/xuanji/publish` 强制 AND；10 §4 边界清单 I-05 已交付 |
...
| DT-18 | 可视化十四维雷达 + 采纳建议 | N/A | N/A | N/A | N/A | N/A | ✅ | MonitorView ECharts 雷达；`/api/xuanji/health` 14 维分数 JSON |
| DT-19 | 融合工作台（蓝图→治理→上架 3 步闭环）| N/A | N/A | N/A | N/A | N/A | ✅ | XuanjiFusionView 蓝图画布 + POST optimize + publish 三步 |
...
| workspace 测试总量基线（`cargo test --workspace`）| （**仍在编译**，2026-08-23 16:xx 已启动；以 2026-08-23 归档基线为准）| 649+ passed / 0 failed / 6 ignored | 2026-08-23 基线 **649+ passed / 0 failed**（xuanji-expert 146 / primiflow-fusion 44 / runtime 33 / primiflow 53）| ✅ 基线 GREEN（等 cargo 完成若回归需修正） |
...
### 7.1 内部代码 + 文档事实
- 15 Crates：`platform/services/*/`（xuanji-expert / graph-algorithms / kg-hub / xuanji-system 等 15 份），每份 README 有三常量 + 模块结构 + 测试命令
