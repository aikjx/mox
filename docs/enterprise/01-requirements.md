...
> **适用范围**：`crates/alliance-system`（协作治理域）、`crates/expert-alliance`（融合引擎域）、`frontend/`
> **权威来源**：业务规则以 `docs/expert-alliance-business-requirements.md` 为基线；本文件将其提升为结构化 SRS 并补充可度量 NFR 与验收。
>
...
定义专家联盟系统的功能性与非功能性需求，作为架构设计、编码、测试与验收的唯一需求基准。系统承载两条互补业务主线：

- **专家联盟（协作治理）**：组建联盟 → 专家入盟 → 任务派发 → 协同推进 → 全程留痕（`alliance-system`）。
- **联盟融合（算子融合）**：归一化 → 七维会诊 → 冲突消解 → 治理裁决 → 产出可复用优化算子并上架（`expert-alliance`）。

### 1.2 术语

| 术语 | 含义 |
|------|------|
| 联盟（Alliance） | 多租户隔离的协作单元，权限与作用域的最小边界 |
| 成员（Member） | 联盟内的参与者，持有角色绑定（RoleBinding） |
| 任务（Task） | 受状态机约束的工作单元，可被分派给多名专家 |
| 角色（Role） | Admin/Coordinator/Expert/Member/Auditor，含继承 |
| 作用域（Scope） | 权限生效范围：Global / Alliance / Task |
| 领域事件（DomainEvent） | 写操作产出的不可变事实，是审计与实时推送的唯一数据源 |
| 反应器（Reactor） | 订阅事件总线、将事件转译为系统消息与通知的组件 |
| 璇玑（Xuanji） | `expert-alliance` 的算法最高权限校验，不可被治理覆盖 |
...
| 用户类 | 业务目标 | 关键关注 |
|--------|----------|----------|
| 联盟管理员 AllianceAdmin | 治理联盟、任免、审计 | 全局管控、合规留痕 |
...
4. 通信机制（联盟/任务/私信频道、消息、通知、WebSocket 实时推送）。
5. 审计留痕（领域事件、鉴权拒绝留痕、审计查询）。
6. 联盟融合治理（双联盟十四维、一票否决、不变式验证、可视化）。
...
| ID | 需求 | 优先级 | 状态 | 落点 |
|----|------|:--:|:--:|------|
| FR-MEM-01 | 创建联盟并确立首位管理员（状态直接 Active） | P0 | ✅ | orchestrator.bootstrap |
| FR-MEM-02 | 邀请专家入盟，默认授 `Expert@Alliance`（最小权限） | P0 | ✅ | services.invite |
| FR-MEM-03 | 同联盟同 email 邀请幂等（忽略大小写/空格）→ Conflict | P1 | ✅ | services.invite |
...
| FR-TASK-03 | 分派身份三重校验（存在/同联盟/Active） | P0 | ✅ | services.validate_assignees |
...
| FR-TASK-06 | 依赖图须为 DAG（拒自依赖/成环/跨联盟） | P1 | ✅ | services.add_dependency |
...
| FR-PERM-03 | 三级作用域 Global/Alliance/Task | P0 | ✅ | rbac.Scope |
...
| ID | 需求 | 优先级 | 状态 | 落点 |
|----|------|:--:|:--:|------|
| FR-COMM-01 | 联盟大厅（公共频道）惰性创建 | P1 | ✅ | orchestrator.bootstrap |
| FR-COMM-02 | 联盟/任务/私信三类频道 | P2 | ✅ | model.Channel |
...
| ID | 需求 | 优先级 | 状态 | 落点 |
|----|------|:--:|:--:|------|
| FR-FUSE-01 | 双联盟十四维治理（业务七维 + 开发七维 CodeIR 驱动） | P0 | ✅ | expert-alliance |
...
| ID | 类别 | 需求 | 目标值 | 现状 |
|----|------|------|--------|------|
| NFR-01 | 多租户隔离 | 所有查询按 alliance_id 过滤；跨联盟引用拒绝 | 100% 拦截 | 查询侧 ✅；写入侧已修 GAP-2 |
| NFR-02 | 安全 | 最小权限：受邀成员仅 `Expert@单联盟` | 默认满足 | ✅ |
| NFR-03 | 可移植性 | 存储层接口/实现分离，可换 SQLite/分布式 KV | 接口稳定 | ✅ 内存实现 |
| NFR-04 | 解耦 | 领域动作与通信经事件总线解耦 | 全量覆盖 | ✅ |
| NFR-05 | 代码质量 | crate `cargo clippy` 零告警 | 0 warning | ✅ |
| NFR-06 | 可回归 | 全量单测/集成测试一键回归 | 全绿 | ✅ 145+ 通过 |
...
- **假设 A-1**：当前为单进程内存态，重启即失忆；生产需 NFR-03 持久化。
...
| FR-TASK-03 | `03-design` §分派校验 | `br07_assign_validates_assignee_identity` | 不存在/跨联盟/非Active→拒绝 |
| FR-TASK-05 | `03-design` §DoD 门禁 | `br10_done_gate_blocks_incomplete_work` | 子任务/依赖未完成→InvalidState |
| FR-TASK-06 | `03-design` §DAG | `br11_dependency_graph_is_dag` | 自依赖/环/跨联盟拒绝 |
| FR-PERM-06 | `03-design` §审计 | `br18_authz_denial_is_audited` | 越权恰好 1 条记录；正常 0 条 |

> 完整 21 条业务规则追踪见 `docs/expert-alliance-business-requirements.md` §6。
...
2. **回归验收**：`cargo test -p alliance-system -p expert-alliance -p flow-ai` 全绿。
3. **静态验收**：相关 crate `cargo clippy` 零告警。
4. **安全验收**：跨租户权限提升路径（GAP-2 攻击链）被阻断；鉴权失败可审计。
5. **性能验收**：`alliance bench` 加速比 ≥ 2.32×。
...
| 版本 | 说明 |
|------|------|
| v1.0 (ENT) | 首版企业级 SRS：结构化 FR/NFR、可度量目标、追踪矩阵、验收标准；与 `expert-alliance-business-requirements.md` 对齐并提升格式标准。 |
