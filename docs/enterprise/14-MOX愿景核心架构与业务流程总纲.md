﻿# 璇玑 RelGraph · MOX愿景 · 核心架构与业务流程总纲（Enterprise Charter · 对齐 18 TOP-MASTER）
...
> **承载底座**：`GR-STD-V1.0` 关图（高维需求关系图）＋ `PT-STD-V1.0` 六维绑定（物理底座）＋ `AA-STD-V1.0` MOX需求流程（业务事实基准）
...
| ⑤ | MOX通过 AI 整理，不断地优化架构与算法 | **AI MOX整理**：双璇玑十四维并行诊断＋归一化裁决＋已验证最优求解，持续打磨架构与算法 |
...
>
> **M5 · AI MOX整理**：双璇玑十四维（业务七维＋开发七维）并行诊断，归一化裁决（硬约束优先），flow-ai 已验证最优求解（CPM/RCPSP/Dijkstra/冲突修复/CodeGen）。
>
...
> **用一张高维、可机读、全关联的需求关系图作为系统的唯一事实基准，让 AI 在MOX约束下持续整理与优化架构算法，把人类从"原地重复"中解放出来，只做真正的创新。**

---

## 1 · MOX工程方法论落地（How We Engineer）
...
> 完整治理见 `00-INDEX.md`（唯一治理入口）与 `docs/README.md`（关图/MOX专题分区索引）。本节为其**总纲式映射**。

### 4.1 `docs/` 归一化布局

| 分区 | 等级 | 内容 |
|------|------|------|
| `enterprise/` | 🟢 权威治理中心（本文集 `00~14`） | 需求/架构/设计/业务/路线图/映射/MOX/归档/交付 |
| `specs/` | 🟢 权威规范 | `GR-STD`（关图）/ `PT-STD`（六维绑定）/ `OUS`（业务规划） |
| `full-dimensional/` | 🟡 MOX专题 | AA-STD 流程基准 / 关图骨架 / 治理台 API / 过程稿归档 |
| `graph/` | 🟡 机读产物 | `graph.json` / `graph.enterprise.json` / `guantu.req.json` |
| 根级 `*.md` | 🟢/🟡 | AA-STD（融合域权威）、`mox-expert-*`、过程稿、可视化 `*.html`/`*.mmd` |

### 4.2 编号归一基准

- MOX流程阶段 **S1~S8**；闸门 **G0~G3**；护栏 **G-A~G-E**；规范 **GR-STD / PT-STD / AA-STD**。
...
```mermaid
flowchart LR
    A["需求录入<br/>(关图 REQ 根)"] --> B["AI MOX诊断<br/>(双璇玑十四维)"]
    B --> C["归一裁决<br/>(硬约束优先)"]
...
> 完整证据见 `09-企业级MOX维度完成归档` / `10-企业级交付清单` / `11-MOX测试验证优化修复报告` / `12-RBAC审计全链路闭环验收报告` / `13-禁伪代码真并行零死代码闭环验收报告`。
...
- **先读人话版**：本文 `§0 北极星` → `07-MOX需求明确书`（铁律）→ `08-MOX自动化处理明确书`（铁律怎么自动跑）。
...
| 版本 | 说明 |
|------|------|
| v1.0 (ENT) | 首版顶层总纲：把"高维需求关系图/描述互链/关联关系全明确/快速判重/AI MOX整理/持续优化/人类聚焦创新"七原则整理为方法论，并向下收敛到 `01/02/00-INDEX/AA-STD`；含 mermaid 业务流程图与优化飞轮。 |
