...
> **新增 v1.5 2 大术语（对齐 27 企业级测试主控提示词 = 8 大类≥48 规范标准题库 + MOX自动化测试判定体系）**：**璇玑企业级 8 大类≥48 规范标准评测题库（T0 18 烟测硬门槛 + 严格单次 + Δ≤1e-6 + SHA-256 全程留痕 6 字段齐全）** · **璇玑MOX自动化测试流程与判定体系（6 阶段 + T1~T7 7 份交付物 + T6 7 条 RELEASE_L2 / L1 条件 / REJECT 3 选 1 判定树）**。
...
| **AA-STD**（保留） | MOX需求业务处理流程图-归一化企业级 | 融合域**需求事实基准**，承载 REQ→FUN→BIZ→ALG→TSK→COD 五向绑定的归一化主流程。 | `docs/璇玑-MOX需求业务处理流程图-归一化企业级.md` |
...
| **MOX**（保留） | full-dimensional | 覆盖需求/架构/设计/业务/测试/验收/归档的MOX维度工程视图。 | `docs/architecture/full-dimensional/` |
