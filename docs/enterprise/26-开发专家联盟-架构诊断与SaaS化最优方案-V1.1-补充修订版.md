...
> **文档编号**：EA-DOC-061 ｜ **权威等级**：🟢权威 ｜ **最后更新**：2026-08-31
>
...
| 能力 | 状态 | 代码位置 |
|------|------|---------|
| 6 内置角色 | ✅ 完成 | [policy.rs#L133-L158](platform/domains/mox-expert/src/rbac/policy.rs#L133-L158)：admin→editor→viewer 继承链 + safety_approver(仅审批生产) + operator(运维) + auditor(审计只读) |
| 资源级权限（通配符） | ✅ 完成 | [Permission.matches()](platform/domains/mox-expert/src/rbac/policy.rs#L33-L48)：`write:db:prod/*` 匹配 `db:prod/citizen_info`，支持 `/*` 尾缀通配 + `*` 全匹配 |
| 继承链展开（防循环） | ✅ 完成 | [BuiltinRoles::resolve_impl()](platform/domains/mox-expert/src/rbac/policy.rs#L93-L119)：visited HashSet 防循环继承 DAG |
| 失败自动审计 | ✅ 完成 | [rbac/mod.rs#L12](platform/domains/mox-expert/src/rbac/mod.rs#L12)：`check_with_audit` 失败自动打审计 |
...
|----------------------|-------------|------|
| `POST /ai/engine/process`（自动意图识别→能力路由） | [AIEngineCore.process()](platform/backend-node/src/ai-engine-core.js#L191) → `detectIntentByGraph()` → `_dispatch()` | ✅ |
| `POST /ai/engine/analyze`（显式能力执行） | [executeCapability()](platform/backend-node/src/ai-engine-core.js#L221)：校验 capability ∈ 能力矩阵 | ✅ |
| `GET /ai/engine/capabilities`（能力矩阵自描述） | [getCapabilities()](platform/backend-node/src/ai-engine-core.js#L162)：返回 6 能力 + 关键词 + pipeline + 4 不变式 | ✅ |
| `GET /ai/engine/metrics`（成功率/降级率/延迟） | `_recordMetric()` + `engine_core_metrics.json` 持久化（[ai-engine-core.js#L125-L140](platform/backend-node/src/ai-engine-core.js#L125-L140)），接口路由已在 `routes/ai-engine.js` | ✅ |
