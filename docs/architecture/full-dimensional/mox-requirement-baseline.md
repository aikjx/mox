...
| --- | --- | --- |
| 关图骨架定义.md | `docs/_archive/2026-08-16/关图骨架定义.md`（原始稿，已归一承载于 `docs/architecture/full-dimensional/guantu-skeleton.md` GR-STD-V1.0） | 已建，独立承载 REQ 根 + 六维绑定骨架 + 偏离检测 |
...
| `docs/architecture/full-dimensional/guantu-skeleton.md` | REQ 根（D01-D13/R01-R08）+ 六维绑定骨架 + 偏离检测（GR-E6）+ CI 门禁 | 关图承载 |
| `docs/modules/mox-expert-alliance-fusion-flows.md` | 联盟融合流程（业务维度） | 业务扩展 |
...
- **「需求有根」** → `docs/architecture/full-dimensional/guantu-skeleton.md` §3（REQ:D04 Bind 到 `crates/mox-expert/src/lib.rs`）。
