...
| 项目 | 类型 | 说明 |
|------|------|------|
| `melody2score/` | Rust+Python 桌面应用 | **哼唱旋律转歌谱**端到端应用：录音/音频→提取旋律→生成简谱/musicxml。PC 原型 + 开发板移植（`board/`）+ 信息图谱融合（`graph/`，领域子图 D13，可被 tools/info-graph 加载）。目录：app/audio/board/build/core/dist/docs/architecture/graph/lib/results/tests/video |
...
| `mox-official-site/` | 官网 | `projects/mox-official-site/docs/需求.md`（官网需求文档） |
...
| `prototypes/expert-alliance-design/` | expert-alliance-design.html | 专家联盟平台MOX维度设计方案（Element Plus 规范） |
| `prototypes/kg-workflow-guide/` | kg-workflow-guide.html | 知识图谱数据处理工作流（采集到入库全链路，含 kg-pipeline-flow.yaml） |
| `prototypes/mox-enterprise-optimization/` | mox-enterprise-optimization.html | 璇玑（MOX）企业级MOX分析与优化设计报告 |
| `reports/html/xuanji-ux-redesign/` | xuanji-ux-redesign.html | 璇玑系统 UX 重设计规划（信息架构与布局） |
| `prototypes/data-vis/` | MOX分析流程.html + flow_data.json | MOX分析需求业务处理流程图可视化 |
