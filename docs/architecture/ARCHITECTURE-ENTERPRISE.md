...
> ⚠️ **设计稿参考**：三端融合视角设计稿，与当前实现互补但非数值权威；143 crate/12 域/网关 :3080 以 `docs/architecture/NORMALIZED_ARCHITECTURE.md`（v2.0）为准。
...
**璇玑 RelGraph · 算子统一系统（OUS）** 是一个企业级多专家智能编排平台，通过"开发专家联盟"模式，将多个领域专家（安全、权限、算法、架构、性能、测试等）组织为协作联盟，对用户需求进行全维维度分析、辩论、合成与质量门禁，最终输出企业级可交付成果。

### 1.2 核心价值

- **全维分析**：14维归一化评估，覆盖安全、权限、算法、架构、性能、可维护性等
