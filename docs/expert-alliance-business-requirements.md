# 专家联盟 · 联盟融合 —— 企业级业务处理流程需求规格

> 文档类型：业务处理流程需求规格（BRD + 领域规则 + 需求追踪矩阵）
> 适用系统：`crates/alliance-system`（协作治理域）、`crates/expert-alliance`（璇玑融合引擎域）、`frontend/src/views/AllianceFusionView.vue`（融合工作台）
> 配套流程图：[`expert-alliance-alliance-fusion-flows.md`](./expert-alliance-alliance-fusion-flows.md)
...
| 主线 | 业务语义 | 承载域 |
|---|---|---|
| **专家联盟（协作治理）** | 把跨组织的领域专家组织成受治理的协作体：组建联盟 → 专家入盟 → 任务派发 → 协同推进 → 全程留痕 | `alliance-system` |
| **联盟融合（算子融合）** | 把专家提交的业务流程做归一化、七维专家会诊、冲突消解、治理裁决，产出可复用的优化算子并上架 | `expert-alliance` |

两条主线在业务上串联：**专家联盟产出「谁来做、做什么、是否通过」的组织决策；联盟融合产出「怎么做得更快、是否可信」的技术决策。**
...
角色实现见 `crates/alliance-system/src/rbac.rs:63`，继承链 `Coordinator → Expert → Member`（见 `rbac.rs:112`）。

| 角色 | 业务职责 | 继承 |
|---|---|---|
| `AllianceAdmin` 联盟管理员 | 联盟最高治理权：成员管理、任务全权、审计查看 | — |
...
- `Scope::Global` —— 跨联盟生效，仅 bootstrap 首位管理员持有（`orchestrator.rs:123`）
- `Scope::Alliance(id)` —— 仅在指定联盟内生效，受邀专家默认获此作用域（`services.rs:51`）
...
| 步骤 | 动作 | 实现 |
|---|---|---|
| 1.1 | 创建联盟实体（多租户隔离单位） | `orchestrator.rs:102` |
| 1.2 | 创建首位管理员，状态直接 `Active`（无需邀请自环） | `orchestrator.rs:109` |
| 1.3 | 授予 `AllianceAdmin` 全局角色绑定 | `orchestrator.rs:122` |
| 1.4 | 惰性创建「联盟大厅」公共频道 | `orchestrator.rs:126` |
| 1.5 | 签发访问令牌 | `orchestrator.rs:128` |

**业务规则**
- **BR-01** 联盟必须在创建成员/任务之前存在，否则拒绝（`services.rs:31`、`services.rs:209`）。
...
sequenceDiagram
    ...
    participant S as AllianceSystem
    participant St as Store
    participant P as PermissionService
    C->>S: bootstrap(联盟名, 管理员名, 邮箱)
    S->>St: create_alliance()
    S->>St: create_member(status=Active, tier=Principal)
    S->>P: assign_role(Global/AllianceAdmin)
    S->>St: ensure_alliance_channel() 联盟大厅
    S->>St: set_token(tok)
    S-->>C: (Alliance, Member, Token)
```
...
**业务目标**：把外部专家纳入联盟，赋予最小必要权限，并完成状态激活。

| 步骤 | 动作 | 实现 |
|---|---|---|
| 2.1 | 鉴权：调用者需 `member:invite` @ 该联盟 | `orchestrator.rs:139` |
| 2.2 | 校验联盟存在 | `services.rs:31` |
| 2.3 | 创建成员，状态 `Invited` | `services.rs:45` |
| 2.4 | 授予 `Expert` 角色，作用域限本联盟（最小权限原则） | `services.rs:51` |
| 2.5 | 发布 `MemberInvited` 事件 | `services.rs:53` |
| 2.6 | 反应器：向被邀请人推送通知 + 联盟大厅播报 | `orchestrator.rs:243` |
| 2.7 | 成员激活：`Invited → Active` | `services.rs:61` |

**业务规则**
- **BR-03** 受邀成员默认角色为 `Expert`，作用域**必须**为 `Scope::Alliance`，不得为 `Global`（防止跨联盟越权）。
- **BR-04** 【`GAP-1`】同一联盟内同一 email 不得重复产生成员实体。当前 `invite` 无幂等校验，重复调用会生成多个 member。
  - 验收断言：同联盟同 email 连续 invite 两次，第二次返回 `Conflict` 错误，且 `list_members` 数量不增。
...
| 步骤 | 动作 | 实现 |
|---|---|---|
| 3.1 | 鉴权：`task:create` @ 联盟 | `orchestrator.rs:154` |
| 3.2 | 创建任务，初始状态 `Draft`，无被分派者 | `services.rs:218` |
| 3.3 | 发布 `TaskCreated` → 联盟大厅播报 | `services.rs:229`、`orchestrator.rs:270` |
...
**业务规则**
- **BR-07** 【`GAP-2` · 安全 P0】被分派者必须**逐一校验**：① 成员存在 ② `member.alliance_id == task.alliance_id` ③ `member.status == Active`。
  - **风险说明**：`rbac.rs:228` 的所有权判定完全基于 `task.assignees` 成员表。当前 `services.rs:237 assign()` 对 `assignees` **零校验**，因此可写入任意字符串——包括其他联盟的成员 ID。该成员随后即可凭 `task:transition:own` / `task:edit:own` 操作本不属于其联盟的任务，构成**跨租户权限提升**。
  - 验收断言：① 分派不存在的成员 → `BadRequest`；② 分派他联盟成员 → `Forbidden`（跨联盟）；③ 分派 `Invited`/`Suspended`/`Left` 成员 → `InvalidState`；④ 合法分派后 `assignees` 精确等于入参。
...
- **BR-11** 【`GAP-4` · P1】任务依赖图必须是 DAG：拒绝自依赖、拒绝成环、拒绝跨联盟依赖。当前 `add_dependency` 仅去重（`services.rs:322`）。
  - 验收断言：`A→A` 拒绝；`A→B` 后 `B→A` 拒绝；跨联盟依赖拒绝；合法链路 `A→B→C` 通过。
...
流程细节与真实端点映射见配套流程图文档 [`expert-alliance-alliance-fusion-flows.md`](./expert-alliance-alliance-fusion-flows.md)。核心链路：

```
AllianceFusionView.vue
  → POST /api/alliance/optimize
...
- **BR-17** 发布产物必须携带来源追溯信息（联盟 ID、任务 ID、优化前后指标）。
...
| 编号 | 需求 | 现状 |
|---|---|---|
| **NFR-01** 多租户隔离 | 所有查询按 `alliance_id` 过滤；跨联盟引用一律拒绝 | 查询侧已过滤（`store.rs:52/117/135`）；写入侧存在缺口 → `GAP-2`（BR-07）、`GAP-4`（BR-11） |
| **NFR-02** 最小权限 | 受邀成员仅得 `Expert` @ 单联盟作用域 | ✅ `services.rs:51` |
...
| **NFR-05** 静态质量 | 目标 crate `cargo clippy` 零告警 | ✅ expert-alliance / alliance-system / flow-ai 均 0 告警 |
| **NFR-06** 测试可回归 | 全量单测/集成测试可一键回归 | ✅ 当前 145 项通过 |
| **NFR-07** 性能可度量 | 融合优化有可复现基准 | ✅ `alliance bench`，7 场景实测 |
...
| 需求 | 类别 | 优先级 | 状态 | 位置 / 缺口 |
|---|---|:--:|:--:|---|
| BR-01 联盟前置存在校验 | 完整性 | P1 | ✅ 已实现 | `services.rs` `invite`/`create` 前置校验 |
| BR-02 写操作统一鉴权 | 安全 | P0 | ✅ 已实现 | `orchestrator.rs::require` |
| BR-03 受邀成员最小权限 | 安全 | P0 | ✅ 已实现 | `services.rs::invite` 授 `Expert@Alliance` |
...
|---|---|:--:|---|---|
| `GAP-2` | BR-07 分派身份校验 | 🔴 安全 P0 | `validate_assignees`：不存在→`BadRequest`；跨联盟→`Forbidden`；非 Active→`InvalidState`；名单重复→`BadRequest` | `br07_assign_validates_assignee_identity` |
...
| `GAP-4` | BR-11 依赖 DAG | 🟠 完整性 P1 | 拒自依赖 / 直接环 / 间接环 / 跨联盟依赖；重复依赖幂等 | `br11_dependency_graph_is_dag` |
| `GAP-1` | BR-04 邀请幂等 | 🟠 一致性 P1 | 同联盟 email 唯一（忽略大小写与首尾空格）→ `Conflict`(409) | `br04_invite_is_idempotent_per_email` |
...
rbac::authorize 的所有权判定 ← 完全依赖 task.assignees
                        ...
                        assign() 此前对 assignees 零校验
                                      ↓
        写入他联盟成员 ID → 该成员获得 task:transition:own / task:edit:own
                                      ↓
                          跨租户操作本不属于其联盟的任务
```
...
2. **回归验收**：`cargo test -p alliance-system -p expert-alliance -p flow-ai` 全绿，无既有用例被破坏。
3. **静态验收**：三个 crate `cargo clippy` 保持零告警。
4. **集成验收**：`cargo check -p runtime` 通过，HTTP 层不被破坏。
5. **性能验收**：`alliance bench` 平均加速比不低于既有基线 2.32×。
