...
> 权威文档：`docs/enterprise/18 TOP-MASTER / 09-企业级MOX维度完成归档.md`（L0 最高权威）
...
| **AC-6** | 六维绑定覆盖率 | REQ/FUN/BIZ/ALG/TSK/COD ≥ 90% rubric | 企业自归档：649+ passed / 0 failed（L0 §3 声明） | `docs/enterprise/09-企业级MOX维度完成归档.md §3` |
...
2. **AC-6 六维绑定**：企业文档 `09-企业级MOX维度完成归档.md` 声明 649+ passed / 0 failed，为了本仓库内可回归测试，建议在 `scripts/` 下新增 `binding_coverage_reconcile.js`，通过解析 `REQ-FUN-BIZ-ALG-TSK-COD` 六维 ID 映射，输出 `≥ 90%` 断言脚本（建议下一期 spec）。
