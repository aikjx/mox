...
- **顶栏**（`.chat-header`）：
  - 左："🧠 璇玑系统"、"新建对话"按钮；
  - 中：面包屑/会话标题（已有）保留；
  - 右：按顺序排列"清空 / 导出 / 导入 / 转任务 / 创建项目 / MOX分析（primary）"6 按钮（新建已在左边），中间 φ 间距 10px。
...
### TR
- rule：顶栏按指定顺序包含"新建对话、清空、导出、导入、转任务、创建项目、MOX分析"7 个交互按钮（允许 1 两个位置合并但文字齐全）。
...
- rubric（MOX分析布局质感 ≥4/4）：顶栏 86px 固定、聊天体 809、输入面板 164 三段比例清晰；快捷卡片悬停出现柔和品牌阴影（多层柔边），卡片文字与图标对齐。
