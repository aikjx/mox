...
> 评审对象：Spec `20260823-mox-storage-distributed-ai-unified-query`
...
| AC-11 SDK/灰度/预热/就绪探针 | 三语言 SDK (graph.list / ask) + LRU+backoff；Canary 1→10→50→100 + rollback 0；ready = warmup_complete ∧ pg_hit_rate ≥ 0.85；warmup 三步闭环 | `test-sdk-gray-warmup-summary.js` createMoxClient + rolloutPlan/readinessProbe/warmupRun | 22/22 (sync + async driver) | ✅ |
...
| # | 交付物 | 路径 | 状态 |
|---|---|---|---|
| 1 | Spec 文档 | `.trae/specs/20260823-mox-storage-distributed-ai-unified-query/spec.md` | ✓ |
...
| 10 | 三语言 SDK 契约 + 灰度脚本 + 就绪探针 + 预热 | `test-sdk-gray-warmup-summary.js`（含 createMoxClient 参考实现 + rolloutPlan/readinessProbe/warmupRun） | ✓ |
