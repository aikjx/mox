# 规格：璇玑 mox v4 · 需求-架构-业务处理 MOX维度企业级闭环架构与全功能开发

> 规格语言：中文
> 治理中枢：璇玑归一化知识图谱（需求 → 架构 → 业务流程 → 模块 → 文档 → 代码 六层归一化双向绑定）
> 技术底座约束：后端 Rust MOX自研（Rust Gateway + 16 crate Services + Node EAF 编排），图谱唯一真相源，AIS DIP 依赖反转分层
...
**技术底座约束（红线）**：
1. 后端 Rust MOX自研（Gateway + 16 crate）；Node 仅做编排层（EAF）
2. 图谱唯一真相源：6 层归一化绑定
