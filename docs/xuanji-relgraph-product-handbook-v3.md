...
> 验证报告：`.trae/specs/20260823-xuanji-storage-distributed-ai-unified-query/review.md`（A+ 评级）
...
├─ SDK & 编排层
│   ├─ G41 createXuanjiClient 三语言（Node/Python/Rust）契约（graph.list & ask）
...
[![A+ Review](https://img.shields.io/badge/Enterprise%20Review-A%2B-19B26E.svg)](./.trae/specs/20260823-xuanji-storage-distributed-ai-unified-query/review.md)
...
```bash
# 推荐：Helm3 部署企业增强版
helm repo add xuanji https://charts.xuanji.local
helm install xuanji xuanji/xuanji -n xuanji --create-namespace \
...
```bash
# 严格按 先节点→后边 顺序！！！
curl -X POST http://xuanji.corp.local/ai/engine/process \
...
```js
import { createXuanjiClient } from '@xuanji/sdk';
const xj = createXuanjiClient({ base: 'https://xuanji.corp.local' });
...
#### 第 1 步：Fork & 运行最小验证
```bash
git clone https://github.com/your/xuanji-relgraph
cd xuanji-relgraph/platform/backend-node
...
- **最佳实践**：遵循"核心能力做厚，周边能力做轻"—— 你的 Azure 实现应可单独打包成 `@xuanji/chunk-backend-azure` 插件，不要侵入核心仓库。
...
| A+ 独立评审报告 | [review.md](file:///d:/a10/aikjx/gitcode/infotopograph/.trae/specs/20260823-xuanji-storage-distributed-ai-unified-query/review.md) |
