...
>
> **文档编号**：EA-DOC-066 ｜ **权威等级**：🟢权威 ｜ **最后更新**：2026-08-31

> **文档身份**：专家联盟全域业务流程的「**归一化唯一真相源**」。
> 本手册将分散在 47份文档（2026-08-31盘点）中的专家联盟业务流程统一收敛为一套**可迭代、可追踪、可关联**的标准化流程集，
...
|--------|--------|----------|----------|--------|
| **EAF 专家联盟管线** | PHASE-0~6 | 「AI 多专家咨询」的编排：组队→辩论→合成→门禁→学习 | `POST /ai/engine/alliance/full`（SSE） | Rust `platform/domains/alliance/core/mox-alliance-scheduler-core/` |
...
> **参考实现**：`platform/domains/alliance/core/mox-alliance-scheduler-core/`（EAF 6阶段Rust实现；Node.js参考实现见 `platform/backend-node/src/expert-alliance-engine.js`）
...
> **参考实现**：mox-expert crate `pipeline.rs::mox_optimize`（路径待验证，原引用 `platform/domains/mox-expert/` 已随架构迁移失效）
...
|--------|------|---------------|-----------|----------|
| **EAF 6 阶段** | PHASE-0~6 全管线 | `platform/domains/alliance/core/mox-alliance-scheduler-core/`（EAF Rust实现） | `POST /ai/engine/alliance/full` (SSE) | `alliance.ts runAllianceFullSSE` |
| | 阶段常量与公式 | `platform/domains/alliance/core/mox-alliance-scheduler-core/`（PHASE_NAMES, QUALITY_FORMULA） | `GET /ai/engine/alliance/capabilities` | `alliance.ts getAllianceCapabilities` |
| **MOX 融合 8 步** | XOPT-1~8 mox_optimize | mox-expert crate `pipeline.rs:42`（路径待验证） | `POST /api/mox/optimize` | `MoxFusionView.vue` |
| | 14 专家全清单 | mox-expert crate `experts/mod.rs all_experts()`（路径待验证） | — | — |
| | ⛨验证 5 项不变式 | mox-expert crate `verify/mod.rs`（路径待验证） | — | — |
| | 治理闸门 G1~G8 | mox-expert crate `govern.rs` + `tenant_policy.rs`（路径待验证） | — | — |
...
| | 审计三汇（Syslog/S3/NATS） | mox-expert crate `audit/{sink,syslog,s3}.rs`（路径待验证） | `GET /api/audit` | `AdminView.vue 审计面板` |
