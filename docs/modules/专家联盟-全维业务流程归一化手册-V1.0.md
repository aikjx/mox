...
| **XUANJI 璇玑融合管线** | XOPT-1~8 | 「蓝图治理」的优化：归一化→会诊→裁决→求解→验证→治理 | `POST /api/xuanji/optimize` + `XuanjiFusionView.vue` | Rust `pipeline.rs` |
| **BP 璇玑协作流程** | BP-01~10 | 「组织协作」的闭环：组建→入队→立项→派发→推进→验收→上架 | `XuanjiSystem orchestrator.rs` + 前端 5 视图 | Rust `xuanji-system` |
...
> **参考实现**：`platform/services/xuanji-expert/src/alliance/{mod.rs,gate.rs,intent.rs,team.rs,debate.rs}`
...
> **参考实现**：`platform/services/xuanji-expert/src/pipeline.rs::xuanji_optimize`
> **HTTP 入口**：`POST /api/xuanji/optimize`（`runtime/main.rs:503`）
> **前端入口**：`frontend-ui/src/views/XuanjiFusionView.vue`（三栏：蓝图 / 治理 / 上架）
...
> **参考实现**：`platform/services/xuanji-system/src/{orchestrator,services,model,rbac}.rs`
> **配套流程图**：见 [`xuanji-expert-alliance-fusion-flows.md`](./xuanji-expert-alliance-fusion-flows.md)
...
|----|------|---------------|-------------|
| **BP-1** | 璇玑组建 | 多租户隔离单位创建 + Global XuanjiAdmin 唯一无鉴权入口 bootstrap 一次调用 | 璇玑 Bootstrapped 状态 |
| **BP-2** | 专家入队 | 邀请→授 Expert@Xuanji（最小权限禁止 Global，BR-03）→ 激活；BR-04 同 email 幂等 | `Invited → Active`（成员 FSM） |
...
| **EB-ALL-10** | BP-5 协同推进 | BP-6 融合 | 任务必须 InReview/Done 才能触发融合 | Task.status ∉ {InReview, Done} → /api/xuanji/optimize 412 Precondition Failed |
...
| **F-02** | `xuanji_opt_step` | L3 算法层 | `step_id (1..=8), name, expert_scores` | XOPT-1~8 每步 → 1 节点；专家打分写入 attrs |
...
|--------|------|---------------|-----------|----------|
| **EAF 6 阶段** | PHASE-0~6 全管线 | `xuanji-expert/src/alliance/{mod.rs, gate.rs, intent.rs, team.rs, debate.rs}` | `POST /ai/engine/alliance/full` (SSE) | `alliance.ts runAllianceFullSSE` |
| | 阶段常量与公式 | `xuanji-expert/src/alliance/constants.rs`（PHASE_NAMES, QUALITY_FORMULA） | `GET /ai/engine/alliance/capabilities` | `alliance.ts getAllianceCapabilities` |
| **XUANJI 融合 8 步** | XOPT-1~8 xuanji_optimize | `xuanji-expert/src/pipeline.rs:42` | `POST /api/xuanji/optimize` | `XuanjiFusionView.vue` |
| | 14 专家全清单 | `xuanji-expert/src/experts/mod.rs all_experts()` | — | — |
| | ⛨验证 5 项不变式 | `xuanji-expert/src/verify/mod.rs` | — | — |
| | 治理闸门 G1~G8 | `xuanji-expert/src/govern.rs` + `tenant_policy.rs` | — | — |
| **BP 璇玑协作** | 协作闭环 orchestrator | `xuanji-system/src/orchestrator.rs`（5 角色入口） | `POST /api/xuanji/bootstrap/invite/task/...` | `ExpertCenterView.vue` / `ExpertEnterpriseView.vue` |
| | RBAC + 作用域校验 | `xuanji-system/src/rbac.rs`（BR-07 三重校验） | — | — |
| | 成员 FSM + 任务 FSM | `xuanji-system/src/{services,model}.rs` | — | — |
...
| | 审计三汇（Syslog/S3/NATS） | `xuanji-expert/src/audit/{sink,syslog,s3}.rs` | `GET /api/audit` | `AdminView.vue 审计面板` |
| | 璇玑融合视图端到端 | `gateway/runtime/src/main.rs:502-504` | `/api/xuanji/{optimize,publish}` | `XuanjiFusionView.vue` 三栏 |
...
| **璇玑融合全管线**（19 测试） | `cargo test -p xuanji-expert --test alliance` | 19/19 PASS：`phase_order`、`security_veto_irreversible`、`govern_respects_algo_veto`、`accelerate_ratio_ge_2_32x` | `16-P9 判重闸门验收报告 §4` |
| **璇玑协作流程**（33 测试） | `cargo test -p xuanji-system` | 33/33 PASS：`br07_triple_check_cross_tenant_blocked`、`fsm_done_immutable`、`audit_hash_chain_continuous` | `12-RBAC审计全链路闭环验收报告` |
