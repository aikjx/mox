...
| 内存降级丢持久化 | `new_in_memory` 在 SQLite 不可用时静默降级 → 对话不落库 | 软 | 生产环境应 fail-fast（`MOX_STRICT_PERSIST` 同款语义），而非静默丢数据 |
...
| FR-14 | 验证网关与循环护栏 | mox-expert | `verify` 5 项 + `check_loops` | Security_Code/Algorithm |
...
- **跨 crate Clippy 漂移**：`--all-targets` 下 `mox-system`(测试 ×4)、`mox-expert`(×1)、`runtime`(×2+2) 仍有预存警告，与文档「188→0」口径不符，建议下一轮清扫。
