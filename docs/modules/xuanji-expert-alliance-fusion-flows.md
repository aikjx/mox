...
> 配套代码：`crates/xuanji-expert`（璇玑/璇玑引擎）、`crates/xuanji-system`（璇玑协作系统）、
> `frontend/src/views/XuanjiFusionView.vue`（璇玑融合视图）、`crates/runtime/src/main.rs`（HTTP 入口）。
...
入口：`xuanji_expert::pipeline::xuanji_optimize(raw: &FlowGraph, ctx: &GovernContext) -> GovernanceReport`
（`crates/xuanji-expert/src/pipeline.rs:41`）。
...
（定义见 `crates/xuanji-expert/src/lib.rs` `DIM_PRIORITY` / `DIM_THRESHOLD`）
...
## 2. 璇玑融合视图端到端流程（传媒融合 / Xuanji Fusion）

前端：`frontend/src/views/XuanjiFusionView.vue`；后端入口：`crates/runtime/src/main.rs:502-504`。

```mermaid
sequenceDiagram
    participant U as 用户(融合视图)
    participant V as XuanjiFusionView.vue
    participant API as /api/xuanji/*
    participant EA as xuanji-expert 引擎
    participant M as 算子市场/插件平台

    U->>V: 粘贴蓝图 FlowGraph JSON
    V->>V: 点「全维归一化(双璇玑十四维+璇玑)」
    V->>API: POST /api/xuanji/optimize { flow }
    API->>EA: xuanji_optimize_handler → pipeline::xuanji_optimize
    EA-->>API: GovernanceReport(评分/闸门/优化流程图)
    API-->>V: 治理结论(评分/闸门/优化指标/算法)
    V->>V: 渲染优化后节点与有向边
    U->>V: 填包名 + 自然语言需求
    V->>API: POST /api/xuanji/publish { flow,name,requirement }
    API->>M: 归一化上传算子包
    M-->>V: 包 ID + 节点/边 + 治理评分
    V-->>U: 已上传成功提示
```

三栏对应（`XuanjiFusionView.vue`）：
...
## 3. 璇玑系统协作闭环（XuanjiSystem + Reactor）

门面：`crates/xuanji-system/src/orchestrator.rs`；事件反应器：`Reactor::handle`。
...
flowchart LR
    subgraph FE[前端]
        ...
        A2[XuanjiFusionView 璇玑融合]
    end
    subgraph RT[运行时 runtime/Axum]
        B1[API 网关 鉴权+限流]
        B2[/api/xuanji/optimize]
        B3[/api/xuanji/publish]
    end
    subgraph EA[璇玑 璇玑]
        C1[归一化+七专家]
        C2[裁决+flow-ai 求解]
...
| 流程环节 | 真实代码/端点 | 状态 |
|---------|--------------|------|
| 归一化流水线 | `xuanji_expert::pipeline::xuanji_optimize` (`pipeline.rs:41`) | ✅ |
| 璇玑最高权限验证 | `verify()` (`verify.rs:43`) | ✅ |
| 治理闸门 | `govern()` (`govern.rs`) | ✅ |
| 融合归一化端点 | `POST /api/xuanji/optimize` (`runtime/main.rs:503`) | ✅ |
| 融合上传端点 | `POST /api/xuanji/publish` (`runtime/main.rs:504`) | ✅ |
| 前端三栏视图 | `XuanjiFusionView.vue` | ✅ |
| 璇玑协作闭环 | `XuanjiSystem` + `Reactor` (`orchestrator.rs`) | ✅ |
