...
> 代码落点：`crates/mox-expert`（含 `harness` 插件运行时、`govern` 治理、`rbac` 权限、`audit` 审计、`pipeline` MOX管线、`flow_loader` 流程外部化）
...
璇玑是算子统一系统（OUS）的**最高权限MOX业务编排内核**。它把"算法归一化、资源调度、数据血缘、权限安全、可观测性、业务建模、浏览器自动化"七个领域的专家抽象为**可热插拔、可审计、可拦截**的一等公民插件，在统一共享上下文（`HarnessCtx`）中协同求解任意业务流程，并通过**璇玑验证网关 + 治理闸门**保证产出"可上线、可审计、不越权"。
