...
flowchart TD
    ...
    C -->|MOX求解| G[SUPER_EXPERT 调度中枢 §19]
...
| `POST /api/mox/optimize` | P-13 MOX | mox-expert | — |
| `POST /api/mox/publish` | MOX融合发布 | mox-expert → market | 归一化→优化图→上传算子市场 |

---

## 8. MOX融合总线（归一化 · 融合 · 打通 · 上传平台）
...
**前端入口**：导航栏「MOX融合」→ `/mox-fusion`，提供：① 编辑业务蓝图 → ② MOX归一化（治理评分/闸门/优化指标）→ ③ 一键上传算子市场。
