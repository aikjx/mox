# 璇玑（Expert-Xuanji）产品需求 · 架构 · 业务流程设计书

> 版本：v1.0（生产级实现对齐）
> 参考范式：[DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) —— **"Everything is a Plugin"（一切皆插件）**
> 代码落点：`crates/xuanji-expert`（含 `harness` 插件运行时、`govern` 治理、`rbac` 权限、`audit` 审计、`pipeline` 全维管线、`flow_loader` 流程外部化）
...
璇玑是算子统一系统（OUS）的**最高权限全维业务编排内核**。它把"算法归一化、资源调度、数据血缘、权限安全、可观测性、业务建模、浏览器自动化"七个领域的专家抽象为**可热插拔、可审计、可拦截**的一等公民插件，在统一共享上下文（`HarnessCtx`）中协同求解任意业务流程，并通过**璇玑验证网关 + 治理闸门**保证产出"可上线、可审计、不越权"。
...
| 项 | 验证方式 | 状态 |
|----|----------|------|
| 七位专家并行分析产出结构化观点 | `pipeline::tests::xuanji_end_to_end_runs` | ✅ |
