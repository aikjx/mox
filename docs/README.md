...
功能与源码入口：[统一模块导航](modules/README.md) · [自动生成的全仓代码目录](modules/CODE-CATALOG.md)。按业务域查看后端依赖、前端页面与路由、独立项目入口。
...
| **[API-REGISTRY.md](API-REGISTRY.md)** | **API 注册表（权威·接口↔实现一一对应）** — 223 条注册路由逐域映射实现源码、46 域描述符、独立服务矩阵、治理规则 | 🟢 权威 |
| **[ROADMAP-DOMAINS.md](ROADMAP-DOMAINS.md)** | **域路线图（权威）** — 46 域已全部 ready（0 stub）；五阶段排产表保留为历史落地路线，每域验收标准一一对应 | 🟢 权威 |
