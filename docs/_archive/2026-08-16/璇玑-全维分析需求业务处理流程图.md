...
> 企业级规范 · 归一化整理版 V1.0
> 主责 crate：`xuanji-expert`（D04 全维治理/璇玑）+ `xuanji-system`（D10 璇玑系统）
> 配套基准：`关图骨架定义.md`、`PT-Primi-架构规范-V1.0-完整版.md`、`OUS-业务功能规划与架构数据关系分析.md`
> 落点代码：`crates/xuanji-expert/src/{pipeline,ir,verify,govern,reconcile,programming}.rs`
...
`programming_pipeline`（十步编排）与 `xuanji_optimize`（内核七步）中的逻辑**归一为一条端到端流程**，
...
> 编排器 `programming_pipeline` 包裹内核 `xuanji_optimize`。任一闸门失败按护栏 G‑E 回退最近安全点，
> **绝不将错就错**。
...
flowchart TD
    ...
    subgraph G2[③-⑤ 内核 xuanji_optimize]
        E[/③ 七专家并行审查<br/>业务七维 /] --> R
...
> 注：当前 `xuanji_optimize` 默认装载**业务七维**专家；开发七维经 `CodeIR` 并行分析，
> 双璇玑十四维契约由 `xuanji_double_league_fourteen_dimensions` 测试守护。
...
| # | 原实现问题 | 归一化整理后 |
| --- | --- | --- |
| 1 | 两套入口（`programming_pipeline` 十步 / `xuanji_optimize` 七步）职责边界模糊 | 明确双层：**编排层①-⑩** 包裹 **内核②③-⑤-⑥-⑨**，单一基准 |
| 2 | 开发七维仅在 `ir.rs` 枚举，未进入 `xuanji_optimize` 默认装载 | 双璇玑十四维契约由测试守护，业务/开发并行分析同一引擎 |
...
# 2) 跑全维处理（①-⑩，内部含 xuanji_optimize）
cargo test -p xuanji-expert            # 含十四维契约 / 敏感写拦截 / 草稿护栏
