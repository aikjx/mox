...
| --- | --- | --- |
| 关图骨架定义.md | `guantu-skeleton.md`（GR-STD-V1.0） | 已建，独立承载 REQ 根 + 六维绑定骨架 + 偏离检测 |
...
| 璇玑-…-业务处理流程图.md | `璇玑-全维需求业务处理流程图-归一化企业级.md`（AA-STD）+ `xuanji-expert-normalization.md`（归一化规范） | 并入既有基准与规范书 |
...
| `xuanji-expert-normalization.md` | 归一化规范：单图多维铁律、reconcile 细节、IN-/OUT-/KB-* 契约、P0-P2 落地 | 设计规范书 |
| `xuanji-expert-验证总结-20260816.md` | 验证报告：164 项测试全绿、clippy 0 告警、修复记录 | 验证事实 |
| `xuanji-expert-business-requirements.md` | 业务需求 SRS + 21 BR/9 NFR 追踪矩阵 | 需求事实 |
| `guantu-skeleton.md` | REQ 根（D01-D13/R01-R08）+ 六维绑定骨架 + 偏离检测（GR-E6）+ CI 门禁 | 关图承载 |
| `xuanji-expert-alliance-fusion-flows.md` | 联盟融合流程（业务维度） | 业务扩展 |
...
- **「需求有根」** → `guantu-skeleton.md` §3（REQ:D04 Bind 到 `crates/xuanji-expert/src/lib.rs`）。
- **「六维绑定可追溯」** → AA-STD §3（REQ→FUN→BIZ→ALG→TSK→COD）+ `crates/primiflow/trace_matrix.md`（PT-STD）。
- **「全维分析流程」** → AA-STD §1（S1-S8）+ `xuanji-expert-normalization.md` §1-§2。
- **「测试验证通过」** → `xuanji-expert-验证总结-20260816.md`（164 项 0 失败）。
- **「企业级治理」** → AA-STD §4（G0-G3 + 护栏 G-A~G-E + 审计链）+ `xuanji-expert-normalization.md` §1.4。
