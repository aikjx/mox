...
> 企业级规范 · 归一化整理版 V1.0
> 主责 crate：`expert-alliance`（D04 全维治理/专家联盟）+ `alliance-system`（D10 专家联盟系统）
> 配套基准：`关图骨架定义.md`、`PT-Primi-架构规范-V1.0-完整版.md`、`OUS-业务功能规划与架构数据关系分析.md`
> 落点代码：`crates/expert-alliance/src/{pipeline,ir,verify,govern,reconcile,programming}.rs`

---

## 0. 文档定位与口径

本图是「算法联盟」承接 **全维分析需求** 的**唯一业务处理基准**。它把散落在
`programming_pipeline`（十步编排）与 `alliance_optimize`（内核七步）中的逻辑**归一为一条端到端流程**，
...
| 全局 维度归一 | 业务七维 + 开发七维 | 双联盟十四维统一评分/裁决/验证/闸门 | `ir::Dimension` + 单一 `Expert` 引擎 |
...
> 编排器 `programming_pipeline` 包裹内核 `alliance_optimize`。任一闸门失败按护栏 G‑E 回退最近安全点，
> **绝不将错就错**。
...
flowchart TD
    ...
    subgraph G2[③-⑤ 内核 alliance_optimize]
        E[/③ 七专家并行审查<br/>业务七维 /] --> R
...
> 注：当前 `alliance_optimize` 默认装载**业务七维**专家；开发七维经 `CodeIR` 并行分析，
> 双联盟十四维契约由 `alliance_double_league_fourteen_dimensions` 测试守护。
...
| # | 原实现问题 | 归一化整理后 |
| --- | --- | --- |
| 1 | 两套入口（`programming_pipeline` 十步 / `alliance_optimize` 七步）职责边界模糊 | 明确双层：**编排层①-⑩** 包裹 **内核②③-⑤-⑥-⑨**，单一基准 |
| 2 | 开发七维仅在 `ir.rs` 枚举，未进入 `alliance_optimize` 默认装载 | 双联盟十四维契约由测试守护，业务/开发并行分析同一引擎 |
...
# 2) 跑全维处理（①-⑩，内部含 alliance_optimize）
cargo test -p expert-alliance            # 含十四维契约 / 敏感写拦截 / 草稿护栏
# 3) 治理报告消费（GovernanceReport）：expert_scores / optimization / algo / gate / audit
# 4) 出码产物仅在 safe_to_emit==true 时存在（编程报告 code 字段）
```

> 本图为「算法联盟」全维分析需求处理的**企业级归一基准**；任何实现须满足 §4 璇玑最高权限
