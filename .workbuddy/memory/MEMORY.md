...
## 系统术语
- **璇玑 = 璇玑 (Xuánjī) 系统** = `mox-expert` crate：归一化 IR 驱动、双璇玑十四维（业务7+开发7）并行诊断 → 裁决 → flow-ai 求解 → ⛨璇玑验证网关(最高权限) → 治理闸门 → 出码/出图。承载于关图 GR-STD（`REQ→FUN→BIZ→ALG→TSK→COD` 六维绑定）。
...
## 已知架构缺口（已修复，留痕）
- `mox-system/tests/integration.rs` 的 `temp_db()` 曾仅用 pid 致并行串扰（已改 AtomicU64+pid 唯一目录）。
