...
echo "=== check full alliance domain ==="
cargo check -p mox-alliance-api -p mox-alliance-common-proto -p mox-alliance-executor-proto -p mox-alliance-scheduler-proto -p mox-alliance-boot-config -p mox-alliance-config-core -p mox-alliance-core -p mox-alliance-executor-core -p mox-alliance-scheduler-core -p mox-alliance-http-sdk -p mox-alliance-sdk -p mox-alliance-executor-svc -p mox-alliance-scheduler-svc --all-targets --message-format short 2>&1 | grep -E "^(error|warning: unused|platform\\\\domains\\\\alliance)" | head -40
echo "=== END CHECK ==="
