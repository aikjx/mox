...
| 全维治理 / 专家联盟 | expert-alliance | 十四维专家并行诊断、归一裁决、璇玑验证网关、RBAC、审计链、治理闸门 | ⚠️ 库已实现，HTTP 服务可独立跑；runtime 路由未挂载（见 §2.5） |
...
| 专家联盟系统 | alliance-system | 成员/任务/权限/通信，独立 HTTP+WS 服务 | ✅ 已实现（独立二进制） |
| 模板市场（草莓多） | template-market / ai-agent(caomei) | 系统模板发布/复用/评分，自动生成电商 DDL+Vue | ✅ 已实现（孤立资产层，仅 dev 依赖） |
| 可视化拓扑前端 | frontend（PrimiFlow 规格） | 拓扑画布、图谱、自动化、商城、联盟融合等视图 | ✅ 前端已实现；PrimiFlow 规格为上层产品待开发 |
...
**G. 算子商城 / 自动化中枢 / 模板市场 / 联盟系统 / 桥接**：接口与能力见 §2.3 与外部对接层。
...
| expert-alliance | 璇玑十四维专家+治理 | flow-ai |
| hermes-flow-bridge | Hermes 零侵入注入插件 | flow-ai, expert-alliance |
| business-catalog | 7 业务建模+六维关系网 | flow-ai, expert-alliance |
| ai-agent | AI 智能体八大能力 | operator-core, operator-graph（prod）；flow-ai, template-market（dev） |
| template-market | 草莓系统模板市场 | —（孤立资产层） |
| runtime | Axum 总入口+商城+自动化+Cordis | operator-core/graph/wasm, optimizer, ai-agent, business-catalog, expert-alliance, flow-ai |
| alliance-system | 专家联盟系统（独立） | —（完全独立） |
...
| expert-alliance 独立服务 | HTTP /api/optimize /api/ingest（feature=live 推送） | ⚠️ 默认关闭 | `expert-alliance/src/server.rs` |
| alliance-system 独立服务 | HTTP+WS（/api/members,/tasks,/ws…） | ✅ 独立二进制 | `alliance-system/src/server.rs` |
| SQLite（operator_dialogue.db / alliance.db） | rusqlite 本地持久化 | ✅ | `ai-agent/dialogue_graph.rs`, `alliance-system/store.rs` |
| 审计 Sink（Syslog/S3/Kafka/NATS/RabbitMQ） | expert-alliance audit 外部化 | ⚠️ 需额外配置 | `expert-alliance/src/lib.rs` |

### 2.4 分层架构映射（对齐 README 全业务流程图）

```
接入层:  frontend(Vue3/Three.js) · REST(/api/*) · WS(仅 alliance-system)
运行时:  runtime(Axum) 鉴权/限流/观测/Cordis/MCP/OpenAPI
编排层:  flow-ai(拓扑/DAG/CPM/冲突/排程/codegen) · expert-alliance(多专家融合/治理) · ai-agent(工作流/对话)
...
旁路:    hermes-flow-bridge(外部注入) · alliance-system(独立联盟)
```
...
3. **runtime 主路由无 WebSocket**：`/api/governance/ws` 已定义但未挂载；仅 alliance-system 提供 WS。
...
| GovernanceReport / AuditChain / AuditEvent | expert-alliance/src/pipeline.rs:18, govern.rs:50,37 | 内存 + 可选外 Sink | 治理报告/审计链 |
...
| Alliance 领域模型 | alliance-system/src/model.rs | alliance.db(SQLite) + 内存 | 成员/任务/通信 |
...
FlowGraph(flow-ai) ◀──────────── 被 5 方共享
   ├──▶ expert-alliance::alliance_optimize ──▶ GovernanceReport(AuditChain)
...
- **内存**：operator-core、operator-graph、flow-ai、expert-alliance（AuditChain 内存哈希链）、runtime 治理台状态。
- **文件 JSON**：market 算子包（`$OUS_HOME/market`）、automation 资产、template-market（`templates/`）。
- **SQLite**：`operator_dialogue.db`（对话+图谱）、`alliance.db`（联盟，write-through）。
- **外部 Sink（可选）**：expert-alliance audit → Syslog/S3/Kafka/NATS/RabbitMQ。
...
- `alliance-system::event`：领域事件总线 `EventBus`（`event.rs:80`），写后发布→`Reactor` 翻译为通知（`orchestrator.rs:325`）。
- `expert-alliance::executor`：`ExecEvent` 时间轴回放（`executor.rs:33`）。
...
- 单 crate 构建：`cargo build -p runtime -p flow-ai -p expert-alliance`。
...
- 覆盖率：`cargo tarpaulin`（或 `cargo llvm-cov`），目标核心算法 crate（flow-ai / operator-core / expert-alliance）≥ 阈值（建议 70%）。
...
- **已实现**：算子内核、知识图谱、流程图优化 AI（含 κ‑τ 内核）、专家联盟治理库、业务目录、AI 智能体、商城/自动化/模板市场、联盟系统、前端视图——功能面覆盖极广。
- **关键缺口（规划待办，按优先级）**：
