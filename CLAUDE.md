...
关图（算子统一mox 模块化系统架构分析系统）是一套「需求 → 归一化 IR → 双璇玑十四维诊断 → 裁决 → flow-ai 求解 → ⛨验证网关 → 治理 8 闸门 → 出码/出图」的 AI 自动化中枢，面向企业级mox 模块化系统架构治理与合规出码。后端为 Rust（Axum + Tokio）模块化单体（6层8域DDD矩阵 · 73 crate workspace），前端为 Vue3 管理控制台。
...
- 修改璇玑验证、治理 8 闸门、RBAC、租户策略、审计汇，优先提示爆炸半径（影响mox 模块化系统架构出码与合规）。
