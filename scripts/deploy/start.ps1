...
.DESCRIPTION
  · 自动检测 py/python 解释器
  · 内部调用 scripts/service/server-manage.py bootstrap（预检 → 清理残留 → 可选启动服务 → 管理面板）
...
$($C.C)============================================================
  璇玑 Mox · 架构数字孪生中台  一键启动（Windows PowerShell）
...
"@
  ...
  function Invoke-Manage([string[]]$ManageArgs) {
    $all = @($PY.ArgsPrefix) + @("$RepoRoot\scripts\service\server-manage.py") + @($ManageArgs)
