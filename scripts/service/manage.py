...
本文件仅为向后兼容保留，把所有参数原样转发给 scripts/service/server-manage.py：
  1. 防旧命令 / 旧脚本断链（start.sh、start-all.ps1、stop-all.ps1、deploy/start.ps1 等）；
  2. 新代码一律调用 scripts/service/server-manage.py（单一权威文件名）。

用法：
  python scripts/service/manage.py <action> [args...]     # 等价于 python scripts/service/server-manage.py ...
"""
import os
import subprocess
import sys
...
def main() -> int:
    if not os.path.exists(_AUTHORITATIVE):
        ...
        print(
            "[ERROR] scripts/service/manage.py 为兼容别名，仅当 scripts/service/server-manage.py 存在时可用。",
            file=sys.stderr,
        )
        return 1
