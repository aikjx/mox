...
> **自然语言**：中文  
> **适用范围**：Rust 全量实现专家联盟 6 阶段全维分析引擎、`platform/gateway/runtime` 路由集成、`xuanji-expert` 升级（14 维专家 × 辩论合成 × 质量门禁 × 指标学习）、前端 `ChatView.vue` 全维流程桥接、以及语音对话 ASR/TTS/桌面小白 端到端交付。  
...
| 层 | 已有能力 | 现状路径 |
|---|---|---|
| L3 算法推理 | `xuanji-expert` 14 维专家 trait + 并行 dispatch + reconcile 裁决 + RBAC + 审计链 | `platform/services/xuanji-expert/` |
...
| # | 问题 | 影响（企业级严重性） |
|---|---|---|
| P1 | **双真相漂移**：专家联盟 6 阶段流程在 `backend-node/` 用 Node.js 实现、14 维专家在 `xuanji-expert/` 用 Rust 实现——两套实现长期并行会产生算法漂移（如辩论参数不一致）、违反 HC-14（后端 100% Rust）。 | 与 TOP-MASTER §六 HC-14 冲突；L2 企业基线不通过。 |
...
| 开发联盟 Owner | R | Rust Controller→Service→Repo 分层零越层；unsafe=0；`cargo test -p xuanji-expert` 全绿。 |
...
- **G1（Rust 单一真相）**：专家联盟 6 阶段流程（意图→组队→并行→辩论→合成→门禁→学习）**100% 在 Rust `xuanji-expert` crate 中实现**，Node `backend-node/expert-alliance/` 只保留薄代理层（302 重定向到 Rust 端点，或 sidecar 透传）。
...
- **NG1**：本次不重写 `xuanji-expert` 已有 14 位专家的 `analyze()` 逻辑（业务 7 + 开发 7 共 14 个 `*.rs`），只在其上封装联盟流程管线。新增的专家/维度需要先经 ADR。
...
| HC-5 | 后端 100% Rust 全维自研：新端点 handler 只能调 Rust service，禁止直连 Node/外部进程做业务判断 | gateway/runtime 新增路由 → `subservers.rs` 注册 → 调 `xuanji-expert::alliance` crate 内部函数 |
...
- Rust workspace 成员：`xuanji-expert`（核心）、`graph-algorithms`（HC-2 spread）、`ai-agent`（若需 LLM 辅助辩论）、`runtime`（路由挂载）
...
### 4.1 Rust 核心层（FR-CORE-n，xuanji-expert 新增模块）

| 编号 | 需求 | 说明 |
|---|---|---|
| FR-CORE-01 | 新增 `alliance` 模块：6 阶段管线编排器 | `xuanji-expert/src/alliance/mod.rs` 对外导出 `AllianceEngine::run_full_analysis(req) -> SSE Stream<AllianceEvent>`，6 阶段 = IntentClassify → TeamOptimize → ParallelDebate → Synthesize → QualityGate → SkillLearn。 |
...
| FR-CORE-09 | 审计桥接：所有 6 阶段事件独立走 `audit::*` 汇，事件名 `alliance.01.intent_classified`…`alliance.07.skill_learned` 共 7 条 | 每条事件附 HMAC-SHA256 签名（XUANJI_AUDIT_SECRET）；安全类命中事件独立汇 1 份。 |
...
| 编号 | 维度 | 需求 | 验收度量 |
|---|---|---|---|
| NFR-01 | 分层合规（HC-分层） | Controller(runtime routes) → Service(xuanji-expert::alliance) → Repo(图谱/审计) 零越层；禁止 handler 内写业务逻辑 | `grep -rn "fn analyze\|dispatch\|reconcile" platform/gateway/runtime/src/handlers/` 命中 = 0（只做参数校验 + 调 service） |
| NFR-02 | Rust 安全护栏（HC-14） | 新代码 `unsafe` 块计数 = 0；`cargo deny -L error` 全通过；无 C 绑定重依赖 | `grep -rn "unsafe " platform/services/xuanji-expert/src/alliance/` = 0 |
...
| NFR-04 | 性能基线（60 场景） | 空查询（纯本地无 AI）全维分析 P50 ≤ 400ms；P95 ≤ 1.5s；14 专家并行 rayon 加速比 ≥ 3.5×（对比串行） | `cargo bench -p xuanji-expert --bench alliance_full` 基准结果 |
...
| NFR-07 | 企业级稳定性 | 1000 次全维分析调用（含错误输入/空查询/超时）→ 进程不崩溃（0 panic）；内存泄漏率 ≤ 5MB/1000 次 | `cargo test -p xuanji-expert --test alliance_stress -- --test-threads=1 --ignored` |
...
| AC 编号 | 条件 | 证据来源 |
|---|---|---|
| AC-01 | `cargo build -p xuanji-expert -p runtime` 编译通过，0 warning（deny warnings） | CI 构建日志 |
| AC-02 | `cargo test -p xuanji-expert alliance_ --nocapture` 全通过（≥ 25 测试：6 阶段 + 降级 + 幂等 + 审计 + 安全） | cargo test 报告 |
...
| AC-18 | Rust `unsafe` 计数：`grep -rc "unsafe " platform/services/xuanji-expert/src/alliance/` = 0 | 代码扫描 |
