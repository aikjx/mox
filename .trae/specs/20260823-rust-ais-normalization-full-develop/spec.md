...
| **AIS 架构对齐（DIP 依赖反转）** | A-01 | `xuanji-system/orchestrator.rs (L2)` 直接 `use crate::services::*`，未通过 domain 抽象 trait 反转依赖 | 1 处反向依赖 |
...
| AIS 服务拆分（同级直接结构体耦合） | A-03 | `hermes-flow-bridge/src/bridge.rs` 直接 `use xuanji_expert::{xuanji_optimize, GovernContext}` 具体结构体；`business-catalog/src/lib.rs` 直接 `use xuanji_expert::context::{GovernContext, Principal, Tenant}` 具体 struct，均未通过抽象 trait | 2 处同级硬耦合 |
| AIS 依赖治理（版本漂移） | A-04 | ① `primiflow-core` dev-deps reqwest 固定 0.11，workspace 统一 0.12 漂移；② `xuanji-expert / hermes-flow-bridge / business-catalog` 三 crate 未用 workspace=true 继承依赖（serde/tokio/thiserror 等 8+ 项硬编码版本号）；③ `primiflow-fusion` criterion 配置与 operator-core 不一致 | 3 类版本漂移 |
| AIS 依赖治理（框架依赖扩散） | A-05 | 框架级持久化依赖 `rusqlite` 仅允许在 Infra 实现层（xuanji-system），实际 **扩散到 L3 业务域 ai-agent**（Cargo.toml:24 rusqlite workspace=true）与 **L4 核心域 primiflow-core**（Cargo.toml:16 rusqlite workspace=true） | 2 crate 违规扩散 |
...
| 架构文档代码不一致 | B-04 | `docs/enterprise/02-architecture.md` 仅描述 xuanji-system 单 crate 的 AIS 5 层，其余 15 个 crate 的分层、职责、边界在企业架构文档中完全缺失 | 1/16 文档覆盖率 |
...
- ❌ **不涉及**：将 `sea-query`、`sqlx` 从 xuanji-system 替换为自研查询构建器（AIS 允许 Infra 层用方言库，仅禁止扩散）
...
- **rule**：A-01 消除：`xuanji-system/orchestrator.rs` 改为依赖某域 trait（`trait MemberService` / `trait TaskService` / `trait PermissionService`）而非 `use crate::services::*` 通配。可在 `domain/traits.rs` 新增抽象或直接 `pub trait` 于 model/services.rs 顶层，orchestrator 仅 import trait，具体实现注入（构造参数 Arc<dyn Trait> 或全局 fn 指针）。
...
- **rule**：A-03 消除：hermes-flow-bridge / business-catalog 两 crate 不得直接 `use xuanji_expert::*` 具体 struct。新增 `xuanji-expert` 的 `src/traits.rs`（或已有 traits 模块）暴露 `pub trait Optimize`、`pub trait GovernContextRead`、`pub trait PrincipalLike`；bridge/catalog 改为依赖这些 trait（通过泛型参数 `<C: GovernContextRead>` 或 Arc<dyn Trait>），具体实现在 runtime（聚合层）做 feature-gated 注入或通过 bridge `register_xuanji_optimize_fn(fn)` 钩子回调。
...
- **rule**：A-05 消除：rusqlite 框架级依赖只允许在 xuanji-system（Infra 层 crate）出现。ai-agent 和 primiflow-core 两 crate 对 rusqlite 的 direct Cargo.toml 依赖必须移除。解法：新增 **`xuanji-system` 的 PersistenceProvider trait** + adapter，ai-agent / primiflow-core 通过 **`dyn PersistenceProvider` 抽象**进行 SQLite 操作（在 runtime 聚合时把 xuanji-system 的 SQLiteRepo impl 注入进去）；如果 ai-agent / primiflow-core 内部只在少数测试或单文件内使用 rusqlite 打开 db，可替换为 `std::fs + serde_json` 持久化（满足 AIS 禁止框架级持久化进 L3/L4）。
...
- **rule**：`xuanji-expert/tests/gap_p2_perf_boundaries.rs::boundary_ultra_deep_chain_with_data_deps` 测试通过：500 级深链（带真实数据依赖）的 xuanji_optimize 耗时 ≤ 10,000 ms。验证：`cargo test -p xuanji-expert --test gap_p2_perf_boundaries boundary_ultra_deep_chain_with_data_deps` exit 0。
...
- **rule**：重应用框架类依赖仅在允许的边界内：
  - Axum (Web 框架)：仅 runtime / xuanji-system / primiflow-core / primiflow-fusion / xuanji-expert 的 L1 HTTP server 入口文件允许；L4 算法层 crate（operator-core/graph-algorithms/optimizer/flow-ai）Cargo.toml 不得出现 axum。
  - sea-query / sqlx：仅 xuanji-system（Infra 持久化层）Cargo.toml 允许；其他 15 个 crate 不得出现。
  - rusqlite：仅 xuanji-system 允许（FR-RUST-04 已覆盖）。
...
- **rule**：`rusqlite` 从 ai-agent / primiflow-core 移除后，原功能（如 ai-agent dialogue session 持久化、primiflow-core SQLite 输出）仍可在 runtime 聚合时通过 xuanji-system PersistenceProvider 注入获得等价行为。等价性：以 ai-agent 现有 tests `caomei_e2e.rs`、primiflow-core `tests/*integration*` exit 0 验证。
...
5. **性能硬约束**：500 深链数据依赖 xuanji_optimize ≤ 10 s（P-01）。
...
- **假设**：
  - 允许新增 `xuanji-expert/src/traits.rs` / `xuanji-system/src/domain_traits.rs` / `operator-core/src/kernel.rs` + `kernel_ext.rs` 等薄抽象层，不改变 crate 的顶层结构（lib.rs 导出不变）。
  ...
  - P-01 的性能回退来自 xuanji-expert 的 4 类 verify 拓扑/代码生成等 O(n^2) 操作，可通过剪枝/缓存/并行（已用 rayon）之外的局部优化（例如减少 500 深链的无谓拓扑重算）达到 <10 s 的目标。
...
| Q3 | ai-agent rusqlite 用途若超过"会话持久化小量 1-2 表"，**完全抽象 PersistenceProvider** 工作量大，是否允许保留 ai-agent `rusqlite = { workspace = true, optional = true, default-features = false }` 作为可选特性但默认不启用？ | 推荐：**完全移除依赖**，把持久化逻辑全部抽到 xuanji-system PersistenceProvider 抽象；若因实现困难退回可选 feature，必须在 README + AIS 违规表明确标注"非默认启用 + 框架级依赖隔离在 feature gating" | 可选特性兼容 |
...
| AC-08 | rule | FR-RUST-03 rule3：hermes-flow-bridge / business-catalog Cargo.toml 不出现 `xuanji-expert = ...` direct concrete dep（仅依赖 xuanji-expert 的 traits feature 或通过 abstraction 间接） | grep 两 crate Cargo.toml + Cargo.lock |
...
| AC-11 | rule | FR-RUST-04 rule3：rusqlite 仅出现在 xuanji-system Cargo.toml（16 crate 中 1 处） | grep 16 Cargo.toml |
...
| AC-18 | rule | FR-RUST-08 rule1：500 深链测试 exit 0 | `cargo test -p xuanji-expert --test gap_p2_perf_boundaries boundary_ultra_deep_chain_with_data_deps` |
