...
1. **云盘 T10**：`xuanji-cloud-drive-s3/master/volume/filer` 4 crate 仅实现 S3 API 骨架；`SecurityManager` 的 IAM Policy 为 Mock 形态，无 Bucket Policy 10 条真实矩阵验证；STS AssumeRole 的 TTL=900s（15 min）硬约束未在 SDK/后端联合验证；Quota 429 响应（含 `Retry-After` + `X-Quota-*` 头）未接入 API 层；**等保三级 hash_chain 审计**（国密合规基线）在 `xuanji-standards/dengbao_skeleton` 中仍为 placeholder，与 `xuanji_expert::audit::AuditContext` 未打通，无法满足 GB/T 22239-2019 三级可追溯要求。
2. **关系图 T11 R4**：`xuanji-graph-storage::cdc_source` 实现了 Raft → CDC 事件，但 Flink CDC Source Connector 10 万级无丢重语义（at-least-once + idempotent writer = exactly-once 可验证）仅为 trait 契约未验证；**Spark Connector 读写**（GraphX DataFrame → bulkUpsert → GraphX）无实现；**子图 Projection 20 算子**（按 type/community/attr/hop 等条件投影 + 20 种组合）仅 concept 层；**AC-15 故障注入 14**（断链/重复/乱序/字节翻转等 14 故障矩阵）缺少完整测试。
3. **官方 SDK T17**：`xuanji-sdk-cloud` / `xuanji-sdk-graph` (Rust) 仅为 1 行 stub；Node.js / Python SDK 目录不存在；**30 示例 × 3 语言 = 90 示例**（cloud×15 + graph×15 每语言）为零；测试覆盖 ≥80 的基线差距巨大。
...
- DEP-1：`xuanji-standards` (sigv4 / fips_hmac / rfc5424) → T10 STS 签名、hash_chain
- DEP-2：`xuanji-domain-abstractions` (IamProvider, QuotaProvider, CdcPublisher, GraphQueryProvider) → T10/T11 实现所依赖的 trait 契约（**不允许改签名**）
- DEP-3：`xuanji-expert::audit` (AuditContext / MultiSink / S3Sink) → T10 hash_chain 的 WORM Sink 承载
- DEP-4：`xuanji-graph-storage::cdc_source` → T11 Flink / Spark Connector 的事实源
...
- **FR-T10-13 独立验证器**：`cargo run -p xuanji-standards --example verify-hash-chain <path>` 输出 `{blocks:N, integrity:true/false, broken_at:idx?}`；不破坏数据只读
- **FR-T10-14 与现有 audit 集成**：`xuanji_expert::audit::AuditContext` 新增 `.with_dengbao_chain(chain)` 钩子；每条 ExtAuditEvent 同步 append 到 hash_chain，写失败整体返回 error（不可旁路）
...
- **FR-T17-1 Rust SDK Cloud 实现**：`xuanji-sdk-cloud` 实现 `CloudClient { config, list_buckets, create_bucket, put_object, get_object, delete_object, upload_part, complete_mpu, sts_assume_role, iam_evaluate }`；SigV4 自签名（复用 xuanji-standards::sigv4）
- **FR-T17-2 Rust SDK Graph 实现**：`xuanji-sdk-graph` 实现 `GraphClient { get_graph, stats, centrality, communities, pagerank, bulk_upsert, project, cdc_stream, search }`；与 `/graph/*` 端点 1:1 对齐
- **FR-T17-3 Node.js SDK Cloud**：`platform/sdk/nodejs/xuanji-sdk-cloud/`（index.js + package.json），API 形状 = `class CloudClient { constructor({endpoint,accessKey,secretKey,sessionToken?}); async listBuckets(); async putObject(key,buf,opts?); ... }`；零外部 aws-sdk 依赖，使用内置 https + SigV4
- **FR-T17-4 Node.js SDK Graph**：`platform/sdk/nodejs/xuanji-sdk-graph/` 同构
- **FR-T17-5 Python SDK Cloud**：`platform/sdk/python/xuanji_sdk_cloud/`（`__init__.py` + `client.py` + `pyproject.toml`），基于标准库 `urllib` + `hmac` + `hashlib`；零第三方依赖
- **FR-T17-6 Python SDK Graph**：`platform/sdk/python/xuanji_sdk_graph/` 同构
...
- **FR-E-1 T12 Helm DR Chart**：`deploy/helm/xuanji-dr/` 含 Chart.yaml + values.yaml + templates/{master,volume,s3,graph-gateway}; replicaCount=3; antiAffinity=hard; 自动 failover = liveness probe 失败 → k8s 重建主
...
- **FR-F-3 T20 Helm 一键 Chart**：`deploy/helm/xuanji/` 含完整一键部署（云盘+关图+网关+前端+minio-ha）；`helm install xuanji ./xuanji` 即 0 配置可起
- **FR-F-4 T20 Helm 可模板化测试**：`helm template --dry-run xuanji ./xuanji > /tmp/rendered.yaml` 必须成功；模板输出包含 ≥10 Deployment / ≥1 Service / ≥1 ConfigMap
...
- **[rule] AC-T10-1**：运行 `cargo test -p xuanji-cloud-drive-s3 --test t6_m2_s3_service lifecycle_hot_warm_cold` → exit 0；断言 ≥3 子用例（HOT→WARM 自动迁移、WARM 读回温、COLD restore）全部通过
- **[rule] AC-T10-2**：运行 `mocha platform/backend-node/test/test-t10-m4-lifecycle.js`（新增）→ 12/12 绿；覆盖写入、迁移、回温、restore 路径
- **[rule] AC-T10-3**：`IamProvider::authorize_policy` 对 10 条标准 policy × 3 场景 = 30 子断言，执行 `cargo test -p xuanji-domain-abstractions --test t1_t2_t3_red_green iam_10_policies` → 30/30 通过
...
- **[rule] AC-T10-11**：`xuanji-standards::dengbao` 不再是 skeleton，存在 `HashChain::append` / `verify` / `iter` 三个 pub fn
- **[rule] AC-T10-12**：生成 N=1000 hash chain 块 → verify 返回 true；随机篡改第 500 块 1 字节 → verify 返回 false，且 `broken_at` 字段 = 500
- **[rule] AC-T10-13**：`cargo run -p xuanji-standards --example verify-hash-chain <chain.json>` 输出 JSON 含 `integrity: true/false`；exit code 0 仅当 integrity=true
...
- **[rule] AC-T10-19**：`cargo clippy -p xuanji-standards -p xuanji-cloud-drive-s3 -p xuanji-domain-abstractions -p xuanji-expert -- -D warnings` → 0 warning（NFR-13）
...
- **[rule] AC-T11-1**：`FlinkCdcSource` 实现存在（Rust 新模块 `xuanji-graph-streams::flink_source.rs`），含 `new(Arc<CdcSource>) → Self; fn resume(offset) → Result<(),_>; fn next_blocking() -> Option<CdcEvent>`
...
- **[rule] AC-T11-11**：`cargo test -p xuanji-graph-storage -p xuanji-graph-service --test t7_r2_storage t9_r3_graph_service` → 0 失败
...
- **[rule] AC-T17-2**：Node.js SDK 两目录存在：`platform/sdk/nodejs/xuanji-sdk-cloud/package.json` + `xuanji-sdk-graph/package.json`；`exports.main` 指向 `index.js`
- **[rule] AC-T17-3**：Python SDK 两目录存在：`platform/sdk/python/xuanji_sdk_cloud/pyproject.toml`（setuptools）+ `xuanji_sdk_graph/pyproject.toml`；`python -c "import xuanji_sdk_cloud as c; print(c.__version__)"` 成功
- **[rule] AC-T17-4**：Rust Cloud SDK 15 示例文件存在（`examples/ex_c1.rs` 到 `examples/ex_c15.rs` + `examples/ex_g1.rs` 到 `examples/ex_g15.rs`）；`cargo test -p xuanji-sdk-cloud --examples` 编译通过
...
- **[rule] AC-T17-8**：Rust SDK 测试 `cargo test -p xuanji-sdk-cloud -p xuanji-sdk-graph` → passed ≥ 30
...
- **[rule] AC-E-1**：T12 Helm DR `deploy/helm/xuanji-dr/Chart.yaml` + `values.yaml` + 至少 4 个模板存在；`helm lint ./deploy/helm/xuanji-dr`（无 helm 则模板 yaml 解析通过）→ exit 0
...
- **[rule] AC-F-3**：T20 Helm 一键 Chart 存在 `deploy/helm/xuanji/Chart.yaml`；模板渲染输出 YAML ≥ 10 Deployment / ≥ 1 Service / ≥ 1 ConfigMap
...
- **[rule] AC-F-8**：`helm template --dry-run xuanji ./deploy/helm/xuanji > /tmp/xuanji-render.yaml` exit 0（有 helm 环境时）；无 helm 时 `yq eval-all` 模板语法检查通过
...
| 交付物 ID | 路径 | 说明 |
|-----------|------|------|
| D-T10-LC | `platform/services/xuanji-cloud-drive-s3/src/lifecycle.rs`（新） | 冷热分层引擎 |
| D-T10-IAM10 | `platform/services/xuanji-domain-abstractions/src/iam_standard_policies.rs`（新） | IAM 10 条标准 policy 常量 + evaluate |
| D-T10-STS | `platform/services/xuanji-domain-abstractions/src/sts_ttl900.rs`（新） | STS TTL=900 硬约束 + verify |
| D-T10-QUO | `platform/backend-node/src/middleware/quota429.js`（新） | HTTP Quota 429 头中间件 |
| D-T10-HC | `platform/services/xuanji-standards/src/dengbao_hash_chain.rs`（替换 dengbao_skeleton） | 等保三级 hash chain + WORM |
| D-T10-TEST | `platform/backend-node/test/test-t10-m4-{lifecycle,iam10,sts,quota429,dengbao}.js` ×5 新 | T10 ≥60 测试主体 |
| D-T11-CDC100K | `platform/services/xuanji-graph-streams/`（新 crate，workspace Cargo.toml 注册） | Flink Source + 10 万 harness |
| D-T11-SPARK | `platform/services/xuanji-graph-spark/`（新 crate） | Spark Connector Reader/Writer |
| D-T11-PROJ | `platform/services/xuanji-graph-service/src/projection_20.rs`（新） | 子图 Projection 20 算子 |
| D-T11-AC15 | `platform/services/xuanji-graph-service/src/ac15_faults.rs`（新） | AC-15 14 故障注入 |
| D-T11-TEST | `platform/backend-node/test/test-t11-r4-{cdc100k,spark,proj20,ac15}.js` ×4 新 | T11 ≥40 测试主体 |
| D-T17-RUST | `platform/sdk/rust/xuanji-sdk-cloud/src/lib.rs` + `xuanji-sdk-graph/src/lib.rs`（重写）+ `examples/ex_{c,g}{1-15}.rs` ×30 | Rust SDK + 30 示例 |
| D-T17-NODE | `platform/sdk/nodejs/{xuanji-sdk-cloud,xuanji-sdk-graph,test,examples}`（新目录） | Node SDK + 30 示例 |
| D-T17-PY | `platform/sdk/python/{xuanji_sdk_cloud,xuanji_sdk_graph,test,examples}`（新目录） | Python SDK + 30 示例 |
| D-T17-COUNT | `platform/backend-node/scripts/count_sdk_artifacts.js`（新） | 90 示例 / 80 测试计数器 |
| D-E-T12 | `deploy/helm/xuanji-dr/{Chart.yaml,values.yaml,templates/*,scripts/dr-failover.*}` | T12 Helm DR |
...
| D-F-T20 | `deploy/helm/xuanji/` + `deploy/helm/canary-{step1-4}.*` + `values-canary-*.yaml` | Helm 一键 + 4 阶段灰度 |
