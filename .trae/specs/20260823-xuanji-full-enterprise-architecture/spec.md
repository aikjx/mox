# 规格：璇玑 RelGraph v4 · 需求-架构-业务处理 全维度企业级闭环架构与全功能开发
...
| Rust AIS 同级硬耦合 | R-03 | `hermes-flow-bridge` / `business-catalog` 直接 `use xuanji_expert::*` 具体 struct；`xuanji-system/orchestrator.rs` 反向依赖 services 实现 | 3 处 DIP 违反 |
...
| 框架依赖扩散 | R-05 | `rusqlite` 本应仅 Infra（xuanji-system），实际扩散至 L3 `ai-agent` 和 L4 `primiflow-core` Cargo.toml | 2 crate 违规扩散 |
...
| 文档-代码不一致 | E-02 | 架构文档 `02-architecture.md` 仅描述 xuanji-system 单 crate 5 层；16 crate README 覆盖率=2/16=12.5% | 14 crate README 缺失 |
| 版本治理漂移 | E-03 | primiflow-core dev-deps reqwest=0.11（workspace=0.12）；xuanji-expert/hermes/business-catalog 3 crate 未 workspace=true 继承 8+ 依赖 | 3 类漂移 |
...
2. AIS DIP 合规：L6 operator-core kernel 100% std-only；3 处同级硬耦合全部 trait 反转；rusqlite 仅存在于 xuanji-system
...
│   L4.1 xuanji-expert     治理专家引擎（GovernContext trait）   │
...
│   0 外部 crate —— AIS 红线                                    │
├───────────────────────────────────────────────────────────────┤
│ L7 INFRASTRUCTURE（xuanji-system ONLY impl rusqlite / remote ）│
...
C. `rusqlite`、`tokio-postgres`、`rusoto_s3` 等 IO crate 只出现在 L7 xuanji-system impl（禁止 4/5/6 层出现）
...
Response 关键：
  ...
  checks: [
    ...
    { id: "dip_traits_bound",     ok: true, detail: "hermes/bridage/business-catalog/xuanji-system 全部走 trait" }
    { id: "frame_dep_not_spread", ok: true, detail: "rusqlite 仅在 xuanji-system" }
...
| **可部署** | 开源版：`helm install xuanji --set tier=oss` ≤ 10 min；企业版：`--set tier=enterprise` + 3 节点 MinIO + 3 节点 Nebula + Gateway HPA ≤ 30 min | CI deploy smoke |
...
| FR-05 | xuanji-system orchestrator 依赖 L5 Member/Task/Permission trait（非 concrete） | 2.1 B + T6 | rule |
| FR-06 | hermes-flow-bridge / business-catalog 改依赖 xuanji-expert trait（非 struct） | 2.1 B + T8 | rule |
| FR-07 | rusqlite 仅在 xuanji-system（L7）；ai-agent / primiflow-core 移除 rusqlite | 2.1 C 约束 | rule |
...
| FR-14 | 500 深链 xuanji_optimize P99 ≤ 10,000 ms | 2.5 性能 | rule |
...
| AC-07 | hermes-flow-bridge / business-catalog Cargo.toml `xuanji-expert` 下 **无** concrete struct 依赖路径；lib.rs 仅 use trait | grep |
| AC-08 | ai-agent / primiflow-core Cargo.toml `rusqlite` 行不存在；xuanji-system 存在 | grep Cargo.toml |
