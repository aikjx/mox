# 任务队列：璇玑 RelGraph v4 · 全维度企业级架构全功能开发

> 语言：中文 · Spec: [spec.md](file:///d:/a10/aikjx/gitcode/infotopograph/.trae/specs/20260823-xuanji-full-enterprise-architecture/spec.md) · 治理：TDD RED→GREEN + 每任务完成后图谱反向同步（6 层节点+边）
...
| **T5** | rusqlite 收拢 xuanji-system 单 crate（ai-agent / primiflow-core 移除 + PersistenceProvider trait） | HIGH | T2 | AC-08 | pending |
| **T6** | DIP 反转 xuanji-system orchestrator → L5 Member/Task/Permission trait | HIGH | — | AC-06 | pending |
| **T7** | DIP 反转 operator-core L6 kernel/kernel_ext 双层（kernel 0 extern crate） | HIGH | — | AC-04 | pending |
| **T8** | DIP 反转 hermes / business-catalog → xuanji-expert 抽象 trait | HIGH | — | AC-07 | pending |
...
```rust
pub const CRATE_ID: &str = "uuid v5(ns=DNS, name=crate.rs.xuanji.infotopograph/<name>)";
pub const ENGINE_NAME: &str = "xuanji::<name>";
...
3. xuanji-system.layer = L7Infrastructure

### TRs
#### rule
- **TR 2.1**: 每个 lib.rs 含 `pub const CRATE_ID: &str` / `ENGINE_NAME` / `CRATE_META`；16/16
- **TR 2.2**: 所有 CRATE_ID uuid v5 **互异**；`ENGINE_NAME` = `xuanji::<crate 目录名>`（下划线=短横线一致）
- **TR 2.3**: `crate_meta_lookup()` 在 `xuanji_common_meta`（新建 crate：platform/services/xuanji-common-meta）中返回 16 crate 记录
...
```
2. Node 端 `GraphFormulas` 仅保留：① 输入标准化；② 调用 `child_process::spawn` 或 FFI 调用 Rust 二进制 `xuanji-graph-algos-cli`（新建二进制 target）；③ 输出标准化；核心循环完全移除
...
## Task 5: rusqlite 收拢 xuanji-system 单 crate
...
### 范围
1. L5 新建 `pub trait PersistenceProvider`（xuanji-common-meta 或新 crate `xuanji-domain-abstractions`）：
```rust
...
4. xuanji-system 独一的 `RusqlitePersistenceProvider implements PersistenceProvider`
...
- **TR 5.3**: xuanji-system RusqlitePersistenceProvider 集成测试 20 条 CRUD GREEN

---

## Task 6: DIP 反转 xuanji-system orchestrator → L5 trait
...
### 范围
1. 在 L5 crate（xuanji-domain-abstractions）定义：
```rust
...
```
2. xuanji-system orchestrator.rs：将 `use crate::services::{XuanjiMember, XuanjiTask, PermissionService}`（具体 struct）全部替换为 `use crate::domain::{MemberProvider, TaskProvider, PermissionProvider}`；结构体字段改为 `Box<dyn MemberProvider>`
...
## Task 8: DIP 反转 hermes-flow-bridge / business-catalog → xuanji-expert 抽象 trait
...
```
2. hermes-flow-bridge：将 `use xuanji_expert::xuanji_optimize; use xuanji_expert::context::GovernContext;`（concrete）替换为 `use xuanji_expert::domain::{GovernExpert, GovernContext};`（trait）；注入 `Arc<dyn GovernExpert>`
3. business-catalog：同上，只 depend GovernContext trait，不依赖具体 struct

### TRs
#### rule
- **TR 8.1**: grep hermes / business-catalog lib.rs 无 `use xuanji_expert::xuanji_optimize`（concrete fn）；只有 trait use
...
### 范围
1. `xuanji_optimize` 500 深链算法：增加 2 个优化
