...
| **AC-2** | Rust 测试全绿 | `cargo test --workspace` exit=0，≥400 tests | exit=0，doc-tests 1 + 单元/集成 批量 | `Doc-tests xuanji_expert 1/1 ok + xuanji_e2e 2/2 + runtime_integration + …… 全 ok` |
...
| **AC-11** | t6_dip Member 结构体契约 | Tier::Senior/Lead/Associate + name/email/title/expertise | `t6_dip_orchestrator.rs` 重构通过 | `cargo test -p xuanji-system exit=0` |
...
1. **Clippy T2 清零**（16 crate × 全 target）
   - `xuanji-expert/src/verify/cem.rs`：将 `(i+round)%10==0` 统一改为 `.is_multiple_of(10)`。
   - `xuanji-expert/src/domain/mod.rs`：移除 `format!` 嵌套，直接嵌入 `write!`。
   - `xuanji-expert/examples/cem_probe.rs`、`tests/t9_deep_chain_p99.rs`、`examples/profile_deep_chain.rs`：`CemConfig::default()` 直接结构体初始化，移除不必要 `&format!`。
   - `business-catalog/tests/t8_business_dip.rs`：`Error::new(Other,...)` → `Error::other`，`map_or` → `is_none_or` / `is_some_and`。
   - `hermes-flow-bridge/tests/t8_hermes_dip.rs`、`xuanji-expert/tests/t8_dip_*.rs`：`any(|a|*a==x)` → `contains(&x)`。
...
2. **Rust 运行时 + 业务契约修复**
   ...
   - `xuanji-system/tests/t6_dip_orchestrator.rs`：`Member` 改用正确字段 `name/email/title/expertise/tier`，Tier 变体替换为 `Senior/Lead/Associate`。
   - `primiflow-core/src/generate.rs`：`schemas={schemas}` → `schemas={schemas_str}`（消除 `Vec<String>` Display 编译错误）。
   - `xuanji-expert/src/verify/cem.rs`：`use flow_ai::model::OptimizationReport;`（缺失导入）。
...
| xuanji_expert Doc 测试 | 1/1 (audit) | ≥ 1 | ✅ |
...
3. **Cargo future-incompat**：本次 xuanji_e2e 运行提示 `sqlx-postgres v0.8.0` 未来 Rust 版本将被拒绝，可升级依赖避免后续升级失败。
...
>
> 报告生成：企业级 T9 汇总证据包 · 签名 `xuanji::runtime + ai_agent::T9_reporter_v3` · 可追溯、可重跑。
