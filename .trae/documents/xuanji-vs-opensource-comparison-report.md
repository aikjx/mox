# 璇玑 · 自研 vs 开源竞品全维对比分析报告 v1.0

> 生成日期：2026-08-23
> 基准版本：Xuanji v3.0.0-ai-powered（AIS-Autopilot 分层架构 · 璇玑全域知识图谱 21 crate · 23 Rust 域 · Node 双栈 100% 自研核心）

## 0. 摘要

璇玑（Xuanji）与当前通用开源 AI 基础设施（Dify、LangGraph、Flowise、LangChain、AnythingLLM、Supabase、Weaviate 等）相比，核心差异在于：
...
| 维度 | 璇玑 Xuanji（自研） | Dify / Flowise | LangGraph (LangChain) | LangChain Core |
|---|---|---|---|---|
...
| 意图识别单源 | ✅ xuanji-common-meta 单意图分类器 + 统一 Intent 枚举 | ❌ 至少 4~8 套独立意图分类（Dify intent、LangChain AgentOutputParser、自定义 router 等）结果不统一 |
