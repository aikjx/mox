...
- 归一化铁律：
  ...
  - DIP：高层（xuanji-system / orchestrator）只依赖 trait，不依赖具体 services crate。
...
| L2 公共元 xuanji-common-meta | crates/xuanji-common-meta | 仅依赖 L1 |
| L3 算法内核 graph-algorithms/primiflow-core | platform/services/graph-algorithms | 仅依赖 L1/L2，不依赖 L5 |
| L4 服务层 xuanji-expert/runtime/... | platform/services/* | 依赖 trait 抽象（L3），不依赖实现 |
| L5 编排层 xuanji-system orchestrator | xuanji-system | 通过 `dyn Trait` 依赖 L4 抽象，禁止 `use crate::services::*` |
