...
1. **流程管理台**：`http://localhost:30400/` 可视化控制台，统一编排「需求 → 录制 → 流程 → 测试开发 → 执行 → AI自愈 → 运维」全生命周期，含流程编排 / 模块库 / 测试开发 / 智能运维四个舞台面板。
2. **流程模块化**：流程 = 有序步骤，每步引用注册表模块（goto/click/fill/press/wait/assert_text/extract/screenshot/custom，schema `RPA-FLOW-V1`）；流程可编译为 Python 脚本、可从录制脚本反向拆解为步骤（模块编译与分步执行共用同一代码生成，单一事实源）。
3. **测试开发**：分步调试执行器在同一浏览器会话内逐步执行流程，支持起始步/截止步、失败即停并自动截图，产出步骤级测试报告。
4. **录制**：`playwright codegen` 录制浏览器操作，直接产出 **Python 脚本**并按任务版本化存档。
5. **AI 改脚本（自愈）**：页面变动导致脚本报错（定位器失效/超时）时，自动抓取失败现场页面快照，调用 LLM 重写脚本并复跑；LLM 不可用时退化为离线模糊定位器修复。
6. **智能运维**：内置调度器（间隔任务）、失败自动重跑/自愈闭环、运行留痕（runs/events）、健康聚合端点。
7. **需求归一化**：自然语言需求经 `intake` 归一化为结构化 `TaskSpec`（schema `RPA-TASKSPEC-V1`），字段别名、URL 抽取、意图词表统一映射。
...
│   ├── flow.py         # 流程模块化：步骤模型/模块注册表/脚本⇄流程双向转换
│   ├── devtest.py      # 测试开发：流程分步调试执行器（失败即停+截图）
...
├── web/index.html      # 流程管理台（可视化控制台，挂载于 /）
...
| GET | `/events` | 最近运维事件 |
| GET | `/` | **流程管理台**（可视化控制台） |
| GET | `/flow-modules` | 步骤模块注册表 |
| GET/POST/PUT/DELETE | `/flows` `/flows/{id}` | 流程 CRUD（`RPA-FLOW-V1`） |
| POST | `/flows/{id}/import-script` | 录制脚本拆解为流程步骤 |
| GET | `/flows/{id}/script` | 流程编译为 Python 脚本预览 |
| POST | `/flows/{id}/attach-task` | 编译下发为任务脚本新版本 |
| POST | `/flows/{id}/test` | 分步调试（start/stop_at/headless，失败截图） |
