...
# 同音短休止合并阈值（拍）：低于此值的同音间休止被视为"量化伪影"，
# 前后同音合并为一个长音。0.03125 = 1/32 拍（三十二分休止），
# 仅合并网格对齐产生的微小缝隙，不会吞掉真实的同音重复演奏
# （如"小星星"的 1 1 / 5 5，相邻间隙通常 ≥ 60ms = 0.125 拍@130BPM）。
_SAME_PITCH_REST_MERGE_THRESH = 0.03125

# 极短休止吸收阈值（拍）：低于此值的休止被吸收到前一个音符，
# 避免谱面出现"一个三十二分休止"这种几乎无音乐意义的细碎标记。
_TINY_REST_ABSORB_THRESH = 0.03125
